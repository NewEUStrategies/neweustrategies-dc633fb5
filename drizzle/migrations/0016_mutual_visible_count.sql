...
COMMENT ON FUNCTION public.connection_statuses(uuid[]) IS
  'Stan relacji + STOPIEŃ ODDALENIA (0/1/2/3) i most 1. stopnia dla partii profili. Most nazywamy tylko dla własnych kontaktów z opt-inem discoverable; środkowy węzeł ścieżki 3. stopnia pozostaje anonimowy. mutual_count to fakt grafu (stopień, ranking), mutual_visible_count to zbiór, który wołający zobaczy w mutual_connections (interfejs pokazuje tę drugą).';
...
COMMENT ON FUNCTION public.connection_suggestions(integer) IS
  '„Osoby, które możesz znać" + STOPIEŃ ODDALENIA (0/2/3) i most 1. stopnia, z odsiewem świadomie odrzuconych (connection_suggestion_dismissals) oraz zbieżnością intencji i kompletnością profilu w rankingu. Trzeci stopień dokłada punkt, ale nigdy nie przebija wspólnego kontaktu. Ranking liczy mutual_count (fakt grafu); karta pokazuje mutual_visible_count.';
