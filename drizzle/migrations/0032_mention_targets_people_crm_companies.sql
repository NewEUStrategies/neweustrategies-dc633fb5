...
AS $$
  ...
  companies AS (
    SELECT
      ...
      c.id,
      'org-' || regexp_replace(regexp_replace(unaccent(lower(btrim(c.name))), '[^a-z0-9]+', '-', 'g'), '(^-+|-+$)', '', 'g') AS slug,
      c.name AS label,
...
AS $$
  ...
  SELECT 'organization'::text AS kind,
    ...
    CROSS JOIN wanted
   WHERE c.tenant_id = ctx.tid
     AND 'org-' || regexp_replace(regexp_replace(unaccent(lower(btrim(c.name))), '[^a-z0-9]+', '-', 'g'), '(^-+|-+$)', '', 'g') = wanted.slug
  LIMIT 1;
$$;
