...
function MediaSettings() {
  ...
  const Pair = ({ label, w, h }: { label: string; w: keyof Media; h: keyof Media }) => (
    <Field label={label}>
      <div className="flex items-center gap-3">
        ...
        <label className="text-xs text-muted-foreground">wys.</label>
  ...
  return (
    <div>
      ...
      <Pair label="Miniatura" w="thumbnail_w" h="thumbnail_h" />
      <Pair label="Średni rozmiar" w="medium_w" h="medium_h" />
      <Pair label="Duży rozmiar" w="large_w" h="large_h" />
