        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as FollowTargetType)}>
          <TabsList>
            <TabsTrigger value="author">
              {t("profile.follows.tabAuthors")} ({ids.author.length})
            </TabsTrigger>
            <TabsTrigger value="category">
              {t("profile.follows.tabCategories")} ({ids.category.length})
            </TabsTrigger>
                ...
                {(authorsQ.data ?? []).map((a) => (
                  <li key={a.id} className="flex items-center gap-3 py-3">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        toggle.mutate({ targetType: "author", targetId: a.id, on: false })
                      }
                    >
                ...
                {(categoriesQ.data ?? []).map((c) => (
                  <li key={c.id} className="flex items-center gap-3 py-3">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        toggle.mutate({ targetType: "category", targetId: c.id, on: false })
                      }
                    >
                      {t("profile.follows.unfollow")}
                    </Button>
                ...
                {(tagsQ.data ?? []).map((tg) => (
                  <li key={tg.id} className="flex items-center gap-3 py-3">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        toggle.mutate({ targetType: "tag", targetId: tg.id, on: false })
                      }
                    >
                      {t("profile.follows.unfollow")}
                    </Button>
