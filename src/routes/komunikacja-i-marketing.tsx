...
export const Route = createFileRoute("/komunikacja-i-marketing")({
  ...
  loader: async ({ context }) => {
    ...
    const [seo, document] = await Promise.all([
      loadResilient(
        context.queryClient,
        staticPageSeoQueryOptions("komunikacja-i-marketing"),
        NO_STATIC_SEO,
        {
          deadlineAt,
          label: "legal-seo:komunikacja-i-marketing",
        },
      ),
      loadResilient(
        context.queryClient,
        legalDocumentQueryOptions("communications"),
        NO_LEGAL_DOCUMENT,
        {
          deadlineAt,
          label: "legal-doc:communications",
        },
      ),
    ]);
