...
function Page() {
  ...
  const filtered = useMemo(() => {
    ...
    return (rows ?? []).filter((r) => {
      ...
      if (!q) return true;
      return r.title_pl?.toLowerCase().includes(q) || r.title_en?.toLowerCase().includes(q) || r.slug?.toLowerCase().includes(q);
    });
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Zarządzanie odcinkami, transkrypcjami i publikacją</p>
            </div>
          </div>
          {!editing && (
          ...
          <>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <div className="relative flex-1 min-w-[220px]">
                ...
                <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Szukaj odcinka…" className="pl-8" />
              </div>
              <div className="flex items-center gap-1 p-1 rounded-md bg-muted/60 border border-border">
                {(["all", "published", "draft", "archived"] as const).map((s) => (
                  ...
                  >
                    {s === "all" ? "Wszystkie" : s === "published" ? "Opublikowane" : s === "draft" ? "Szkice" : "Archiwum"}
                  </button>
                ))}
              </div>
            </div>
                ...
                <tbody>
                  {filtered.map((r) => (
                    <tr key={r.id} className="border-b border-border/60 hover:bg-muted/30 transition-colors">
                      <td className="p-2">
                        {r.cover_image_url ? (
                          <img src={r.cover_image_url} alt="" className="w-12 h-12 rounded-md object-cover border border-border" />
                        ) : (
                      ...
                      <td className="p-3">
                        <button className="hover:underline text-left font-medium" onClick={() => loadOne.mutate(r.id)}>{r.title_pl || r.title_en || r.slug}</button>
                        <div className="text-xs text-muted-foreground font-mono">{r.slug}</div>
                      </td>
                      <td className="p-3 text-xs tabular-nums">{r.season ? `S${r.season}` : "-"}{r.episode_number ? ` E${r.episode_number}` : ""}</td>
                      ...
                      <td className="p-3 text-right">
                        <button onClick={() => remove.mutate(r.id)} className="text-xs text-destructive hover:underline inline-flex items-center gap-1">
...
function StatusBadge({ status }: { status: PodcastStatus }) {
  ...
  const label: Record<PodcastStatus, string> = { published: "Opublikowany", draft: "Szkic", archived: "Archiwum" };
  return <span className={`text-xs px-2 py-0.5 rounded font-medium ${map[status]}`}>{label[status]}</span>;
}

type StatTone = "default" | "success" | "warning";
function StatCard({ icon: Icon, label, value, tone }: { icon: typeof Mic; label: string; value: string; tone: StatTone }) {
      ...
      <section className="bg-card border border-border rounded-lg p-5 space-y-5 min-w-0">
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <div className="sm:col-span-2">
            ...
            <Input value={d.audio_url} onChange={(e) => upd({ audio_url: e.target.value })} placeholder="https://…" />
          </div>
          <div>
        ...
        <div className="rounded-md border border-border bg-muted/30 p-3 flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground">Ustawia status na „Opublikowany" i datę publikacji na teraz.</div>
          </div>
          <Switch
            checked={d.status === "published"}
            onCheckedChange={(v) => upd({
      ...
      <aside className="xl:sticky xl:top-4 space-y-3">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1">
            <button type="button" onClick={() => setPreviewLang("pl")} className={`px-2 py-1 text-xs rounded ${previewLang === "pl" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>PL</button>
            <button type="button" onClick={() => setPreviewLang("en")} className={`px-2 py-1 text-xs rounded ${previewLang === "en" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>EN</button>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-border bg-gradient-to-br from-muted/40 to-muted/10 p-4 space-y-4">
          <div className="bg-card border border-border rounded-xl overflow-hidden shadow-sm">
            <div className="relative aspect-square bg-muted">
              ...
              <span className="absolute top-3 right-3"><StatusBadge status={d.status} /></span>
            </div>
            <div className="p-4 space-y-2">
              <h3 className="font-display text-lg leading-tight">{previewTitle || (previewLang === "pl" ? "Tytuł odcinka" : "Episode title")}</h3>
              {previewExcerpt && <p className="text-sm text-muted-foreground line-clamp-3">{previewExcerpt}</p>}
          ...
          <div>
            <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold mb-1.5">Odtwarzacz</div>
            {d.audio_url ? (
              <PodcastPlayer
                src={d.audio_url}
          ...
          {previewNotes && (
            <div>
              <div className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold mb-1.5">Show notes</div>
              <div
