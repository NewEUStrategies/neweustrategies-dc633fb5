  ...
  const { data } = useQuery({
    ...
    queryFn: async (): Promise<MediaItem[]> => {
      const { data, error } = await supabase
        .from("media")
        .select(
          "id, storage_path, public_url, filename, mime_type, size_bytes, uploader_id, created_at",
        )
        ...
        <Select value={typeFilter} onValueChange={(v) => setTypeFilter(v as TypeFilter)}>
          ...
          <SelectContent>
            <SelectItem value="all">
              {t("admin.list.media.allTypes", { defaultValue: "Wszystkie typy" })}
            </SelectItem>
            <SelectItem value="image">
              {t("admin.list.media.images", { defaultValue: "Obrazy" })}
            </SelectItem>
            <SelectItem value="video">
              {t("admin.list.media.videos", { defaultValue: "Wideo" })}
            </SelectItem>
            <SelectItem value="audio">
              {t("admin.list.media.audio", { defaultValue: "Audio" })}
            </SelectItem>
            <SelectItem value="other">
              {t("admin.list.media.other", { defaultValue: "Inne" })}
            </SelectItem>
          </SelectContent>
        </Select>
              ...
              >
                  ...
                  {m.mime_type?.startsWith("image/") ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-[10px] text-muted-foreground gap-1 p-1 text-center">
                      ...
                      <span className="truncate w-full">
                        {m.mime_type || m.filename.split(".").pop()?.toUpperCase()}
                      </span>
                    </div>
                  )}
                </button>
                <div className="p-1.5 text-[10px]">
                  ...
                  <div className="flex items-center justify-between text-muted-foreground">
                    ...
                    <span className="truncate ml-1" title={authorLabel(author)}>
                      {authorLabel(author)}
                    </span>
                  </div>
                  <div className="flex justify-between mt-1">
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        <DialogHeader>
                          <DialogTitle className="truncate">{m.filename}</DialogTitle>
                        </DialogHeader>
          ...
          {!!data?.items.length && (
            <ul className="divide-y divide-border border border-border rounded-md overflow-hidden">
              {data.items.map((it) => (
                <li
                  key={`${it.kind}-${it.id}`}
                  className="p-2 flex items-center justify-between gap-3 hover:bg-muted/40"
                >
