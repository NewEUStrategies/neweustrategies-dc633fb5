    ...
    <div className="space-y-6">
            ...
            {slots.map((s) => (
              <tr key={s.id} className="border-b border-border hover:bg-muted/40">
                ...
                <td className="p-3 text-right space-x-2">
                  <Button size="sm" variant="outline" onClick={() => setDraft(s)}>Edytuj</Button>
                  <Button size="sm" variant="ghost" onClick={() => remove(s.id)}><Trash className="w-4 h-4" /></Button>
                </td>
              </tr>
            ))}
            {slots.length === 0 && (
              <tr><td colSpan={5} className="p-6 text-center text-muted-foreground text-sm">Brak slotów. Dodaj pierwszy poniżej.</td></tr>
            )}
          </tbody>
        </table>
      </section>
      ...
      <section className="border border-border rounded-lg bg-card p-5">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <Label>Nazwa</Label>
            <Input value={draft.name ?? ""} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="np. AdSense 728x90 góra" />
          </div>
          <div>
            <Label>Typ</Label>
            <Select value={draft.kind ?? "html"} onValueChange={(v) => setDraft({ ...draft, kind: v as AdSlotKind })}>
          ...
          {draft.kind === "html" && (
            <div className="sm:col-span-2">
              ...
              <Textarea rows={4} value={draft.html ?? ""} onChange={(e) => setDraft({ ...draft, html: e.target.value })} placeholder="<div>…</div>" className="font-mono text-xs" />
            </div>
          )}
          {draft.kind === "script" && (
            <div className="sm:col-span-2">
              <Label>Skrypt (np. AdSense)</Label>
              <Textarea rows={5} value={draft.script ?? ""} onChange={(e) => setDraft({ ...draft, script: e.target.value })} placeholder='<script async src="…"></script>' className="font-mono text-xs" />
            </div>
          )}
          {draft.kind === "image" && (
            <>
              ...
              <div>
                ...
                <Input value={draft.image_link ?? ""} onChange={(e) => setDraft({ ...draft, image_link: e.target.value })} placeholder="https://…" />
              </div>
              <div>
                <Label>Alt (dla dostępności)</Label>
                <Input value={draft.image_alt ?? ""} onChange={(e) => setDraft({ ...draft, image_alt: e.target.value })} />
              </div>
            </>
          )}
          ...
          <div>
            ...
            <Input type="number" value={draft.width ?? ""} onChange={(e) => setDraft({ ...draft, width: e.target.value ? Number(e.target.value) : null })} />
          </div>
          <div>
            <Label>Wysokość (px)</Label>
            <Input type="number" value={draft.height ?? ""} onChange={(e) => setDraft({ ...draft, height: e.target.value ? Number(e.target.value) : null })} />
          </div>

          <div className="flex items-center gap-2">
            <Switch checked={draft.status === "active"} onCheckedChange={(v) => setDraft({ ...draft, status: v ? "active" : "paused" })} />
            <Label className="m-0">Aktywny</Label>
          </div>
          <div className="flex items-center gap-2">
            <Switch checked={!!draft.requires_consent} onCheckedChange={(v) => setDraft({ ...draft, requires_consent: v })} />
        ...
        <div className="flex gap-2 mt-5">
          <Button onClick={save} disabled={busy}>
            <Plus className="w-4 h-4 mr-2" />{draft.id ? "Zapisz" : "Dodaj slot"}
          </Button>
          {draft.id && <Button variant="outline" onClick={() => setDraft(emptySlot())}>Anuluj</Button>}
        </div>
      </section>
    </div>
  );
...
function PlacementsPanel() {
  ...
  const setCfg = (key: string, val: unknown) => setDraft({ ...draft, config: { ...cfg, [key]: val } });
    ...
    <div className="space-y-6">
      <section className="border border-border rounded-lg bg-card overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {placements.length === 0 && (
              <tr><td colSpan={5} className="p-6 text-center text-muted-foreground text-sm">Brak pozycji.</td></tr>
            )}
          </tbody>
        </table>
      </section>
      ...
      <section className="border border-border rounded-lg bg-card p-5">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <Label>Slot</Label>
            <Select value={draft.slot_id ?? ""} onValueChange={(v) => setDraft({ ...draft, slot_id: v })}>
          ...
          <div>
            ...
            <Select value={draft.position ?? "top_of_post"} onValueChange={(v) => setDraft({ ...draft, position: v as AdPosition })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {Object.entries(AD_POSITION_LABELS).map(([k, l]) => (<SelectItem key={k} value={k}>{l}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          <div>
            <Label>Typ strony</Label>
            <Select value={draft.page_type ?? "all"} onValueChange={(v) => setDraft({ ...draft, page_type: v as AdPageType })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {Object.entries(AD_PAGE_TYPE_LABELS).map(([k, l]) => (<SelectItem key={k} value={k}>{l}</SelectItem>))}
              </SelectContent>
            </Select>
          </div>
          <div>
          ...
          {draft.position === "mid_post" && (
            <div>
              ...
              <Input type="number" min={1} value={(cfg.paragraph as number) ?? 4} onChange={(e) => setCfg("paragraph", Number(e.target.value))} />
            </div>
          )}
          {draft.position === "in_feed" && (
            <div>
              <Label>Co N kart</Label>
              <Input type="number" min={1} value={(cfg.every as number) ?? 5} onChange={(e) => setCfg("every", Number(e.target.value))} />
            </div>
          )}
          {draft.position === "footer_slideup" && (
            <>
              <div>
                ...
                <Input type="number" value={(cfg.delay_ms as number) ?? 3000} onChange={(e) => setCfg("delay_ms", Number(e.target.value))} />
              </div>
              <div className="flex items-center gap-2 mt-6">
                <Switch checked={(cfg.dismissible as boolean) ?? true} onCheckedChange={(v) => setCfg("dismissible", v)} />
          ...
          <div>
            ...
            <Input type="datetime-local" value={draft.starts_at?.slice(0, 16) ?? ""} onChange={(e) => setDraft({ ...draft, starts_at: e.target.value ? new Date(e.target.value).toISOString() : null })} />
          </div>
          <div>
            <Label>Aktywne do</Label>
            <Input type="datetime-local" value={draft.ends_at?.slice(0, 16) ?? ""} onChange={(e) => setDraft({ ...draft, ends_at: e.target.value ? new Date(e.target.value).toISOString() : null })} />
          </div>
        ...
        <div className="flex gap-2 mt-5">
          <Button onClick={save} disabled={busy}>
            <Plus className="w-4 h-4 mr-2" />{draft.id ? "Zapisz" : "Dodaj pozycję"}
          </Button>
          {draft.id && <Button variant="outline" onClick={() => setDraft(emptyPlacement())}>Anuluj</Button>}
        </div>
      </section>
    </div>
  );
