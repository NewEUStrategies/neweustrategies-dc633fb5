...
describe("normalizeFeatured (legacy fallback)", () => {
  it("returns defaults when input is null / undefined / empty", () => {
    ...
    expect(normalizeFeatured(undefined)).toMatchObject({
      focalX: 50,
      focalY: 50,
      aspectRatio: "16/10",
    });
  ...
  it("coerces numeric strings", () => {
    const r = normalizeFeatured({
      focalX: "30" as unknown as number,
      focalY: "70" as unknown as number,
    });
  ...
  it("clamps out-of-range and NaN", () => {
    expect(normalizeFeatured({ focalX: -10, focalY: 999 })).toMatchObject({
      focalX: 0,
      focalY: 100,
    });
