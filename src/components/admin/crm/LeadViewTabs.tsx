// Paski zakładek zapisanych widoków w stylu HubSpot dla listy osób CRM
// (`/admin/crm`) - lustrzane wobec CompanyViewTabs dla firm. Łączy widoki
// wbudowane (BUILTIN_LEAD_VIEWS) z widokami użytkownika z tabeli `saved_views`
// (entity = "lead"). "+" tworzy nowy widok z bieżącej konfiguracji; context
// menu (⋮) pozwala zmienić nazwę, udostępnić zespołowi, usunąć.
...
import { BUILTIN_LEAD_VIEWS, parseLeadViewConfig } from "@/lib/crm/leadViews";
import type { LeadViewConfig } from "@/lib/crm/leadViews";

export interface LeadSavedViewRow {
  id: string;
  name: string;
  config: unknown;
  is_shared: boolean;
...
interface Props {
  lang: "pl" | "en";
  activeId: string;
  onSelect: (id: string, config: LeadViewConfig) => void;
  saved: LeadSavedViewRow[];
  currentConfig: LeadViewConfig;
      ...
      {saved.map((v) => {
        ...
        return (
          <div key={v.id} className="group relative flex items-center">
            <button
              type="button"
              onClick={() => onSelect(v.id, parseLeadViewConfig(v.config))}
        ...
        <PopoverContent align="start" className="w-72 space-y-3 p-3">
          <div>
            ...
            <Input
              ...
              placeholder={t("np. Gorące leady z UE", "e.g. Hot EU leads")}
