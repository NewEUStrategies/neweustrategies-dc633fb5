  ...
  return (
    <div className="not-prose rounded-md border border-border bg-card p-3 space-y-2">
      <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-muted-foreground">
        <ShoppingBag className="w-3.5 h-3.5" /> {bt.editor("affiliate", "heading")}
        <label className="ml-auto flex items-center gap-1 normal-case tracking-normal">
          ...
          />
          {bt.editor("affiliate", "sponsored")}
        </label>
      </div>
      <div className="grid grid-cols-2 gap-2">
        <Input
          placeholder={bt.editor("affiliate", "title")}
          value={get("title")}
          onChange={(e) => patch("title", e.target.value)}
        />
        <Input
          placeholder={bt.editor("affiliate", "store")}
          value={get("store")}
          onChange={(e) => patch("store", e.target.value)}
        />
      </div>
      <Input
        placeholder={bt.editor("affiliate", "imageUrl")}
        value={get("image")}
        onChange={(e) => patch("image", e.target.value)}
      />
      <textarea
        placeholder={bt.editor("affiliate", "description")}
      ...
      <div className="grid grid-cols-3 gap-2">
        <Input
          placeholder={bt.editor("affiliate", "price")}
          value={get("price")}
          onChange={(e) => patch("price", e.target.value)}
        />
        <Input
          placeholder={bt.editor("affiliate", "currency")}
          value={get("currency")}
        ...
        <Input
          ...
          max={5}
          step={0.1}
          placeholder={bt.editor("affiliate", "rating")}
      ...
      <div className="grid grid-cols-2 gap-2">
        <Input
          placeholder={bt.editor("affiliate", "ctaLabel")}
          value={get("ctaLabel")}
          onChange={(e) => patch("ctaLabel", e.target.value)}
        />
        <Input
          placeholder={bt.editor("affiliate", "ctaHref")}
          value={get("ctaHref")}
