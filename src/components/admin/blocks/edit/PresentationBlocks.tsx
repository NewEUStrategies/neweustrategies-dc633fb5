    ...
    <Shell label="Liczniki / statystyki">
      <label className="block text-xs text-muted-foreground">
        ...
        <input
          ...
          value={duration}
          onChange={(e) =>
            onChange({ ...block, data: { ...block.data, duration: Number(e.target.value) } })
          }
        />
      </label>
      {items.map((it, idx) => (
        <div key={idx} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2 items-center">
          ...
          <input
            ...
            onChange={(e) => {
              const next = [...items];
              next[idx] = { ...it, suffix: e.target.value };
              update(next);
            }}
          />
          <input
        ...
        {items.map((it, idx) => (
          <div key={idx} className="rounded border border-border p-2 space-y-1.5">
            <textarea
              ...
              onChange={(e) => {
                const next = [...items];
                next[idx] = { ...it, quote: e.target.value };
                update(next);
              }}
            />
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                onChange={(e) => {
                  const next = [...items];
                  next[idx] = { ...it, author: e.target.value };
                  update(next);
                }}
              />
              <input
            ...
            <div className="grid grid-cols-[1fr_auto_auto] gap-2 items-center">
              ...
              <input
                ...
                onChange={(e) => {
                  const next = [...items];
                  next[idx] = { ...it, rating: Number(e.target.value) };
                  update(next);
                }}
              />
              <button
        ...
        <div key={idx} className="rounded border border-border p-2 space-y-1.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <input
              ...
              onChange={(e) => {
                const next = [...plans];
                next[idx] = { ...p, period: e.target.value };
                update(next);
              }}
            />
          </div>
        ...
        <div key={idx} className="rounded border border-border p-2 space-y-1.5">
          <div className="grid grid-cols-[1fr_2fr_auto] gap-2">
            <input
              ...
              onChange={(e) => {
                const next = [...items];
                next[idx] = { ...it, date: e.target.value };
                update(next);
              }}
            />
            <input
