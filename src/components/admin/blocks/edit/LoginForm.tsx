...
type Key =
  | "title_pl" | "title_en" | "subtitle_pl" | "subtitle_en"
  | "submitLabel_pl" | "submitLabel_en"
  | "redirectTo" | "registerHref" | "forgotHref" | "variant";
  ...
  return (
    <div className="not-prose space-y-3 rounded-md border border-border bg-muted/30 p-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <div className="space-y-1">
          <Label className="text-[11px] uppercase text-muted-foreground">Redirect po logowaniu</Label>
          <Input value={s("redirectTo", "/")} onChange={(e) => set("redirectTo", e.target.value)} placeholder="/" />
        </div>
        <div className="space-y-1">
          <Label className="text-[11px] uppercase text-muted-foreground">Link „Rejestracja"</Label>
          <Input value={s("registerHref", "?mode=signup")} onChange={(e) => set("registerHref", e.target.value)} />
        </div>
        <div className="space-y-1 md:col-span-2">
          <Label className="text-[11px] uppercase text-muted-foreground">Link „Zapomniałem hasła"</Label>
          <Input value={s("forgotHref", "?mode=reset")} onChange={(e) => set("forgotHref", e.target.value)} />
        </div>
      </div>
