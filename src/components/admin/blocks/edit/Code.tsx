...
import { isHighlightableLang } from "@/lib/code/highlight";
...
export function CodeBlock({ block, onChange }: Props) {
  ...
  const highlightable = isHighlightableLang(lang);
  return (
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-border bg-muted/60">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="ts"
          />
          {/* Sygnał, czy wpisany język dostanie kolory na froncie. */}
          <span
            className={`inline-block w-1.5 h-1.5 rounded-full ${
              highlightable ? "bg-emerald-500" : "bg-muted-foreground/40"
            }`}
            title={
              highlightable
                ? cc("highlightOn", { defaultValue: "Podświetlanie składni aktywne" })
                : cc("highlightOff", { defaultValue: "Język bez gramatyki - kod bez kolorów" })
            }
            aria-hidden="true"
          />
        </div>
