...
function SidebarStylePicker() {
  ...
  const pick = (style: SidebarStyle) => {
    ...
    save.mutate({
      ...query.data,
      sidebars: { ...(query.data.sidebars ?? {}), style },
    });
  };
...
function renderPreviewBody(slot: GlobalColorSlot, get: (key: string) => string): React.ReactNode {
  ...
  switch (slot.key) {
    ...
    case "sidebar-icon":
    case "sidebar-icon-hover":
    case "sidebar-icon-active": {
      ...
      const Item = ({
        ...
        return (
          ...
          >
            ...
            {state === "hover" && <span className="text-[8px] uppercase opacity-60">hover</span>}
          </div>
        );
      };
      return (
