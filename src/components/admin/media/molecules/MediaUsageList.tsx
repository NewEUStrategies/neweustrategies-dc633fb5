...
export function MediaUsageList({ isLoading, error, items }: MediaUsageListProps) {
  ...
  const areaLabel = (a: MediaUsageArea): string => {
    const map: Record<MediaUsageArea, string> = {
      cover: t("admin.media.usage.cover"),
      excerpt: t("admin.media.usage.excerpt"),
      content: t("admin.media.usage.content"),
      builder: t("admin.media.usage.builder"),
      blocks: t("admin.media.usage.blocks"),
      layout: t("admin.media.usage.layout"),
    };
    return map[a];
  };
  ...
  return (
    <div className="p-3 text-xs">
      ...
      {!isLoading && error == null && list.length === 0 && (
        <p className="text-muted-foreground">{t("admin.media.notUsed")}</p>
      )}
      {list.length > 0 && (
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[10px] text-muted-foreground flex items-center gap-1 flex-wrap">
                    <span className="uppercase font-semibold">
                      {it.kind === "post" ? t("admin.media.post") : t("admin.media.page")}
                    </span>
                    <span>·</span>
