...
const LABELS: Record<RelatedLayout, { pl: string; en: string; desc_pl: string; desc_en: string }> = {
  grid: { pl: "Grid", en: "Grid", desc_pl: "Równa siatka miniatur", desc_en: "Equal thumbnail grid" },
  list: { pl: "Lista", en: "List", desc_pl: "Kompaktowe wiersze", desc_en: "Compact rows" },
  slider: { pl: "Slider", en: "Slider", desc_pl: "Poziome przewijanie", desc_en: "Horizontal scroll" },
  cards: { pl: "Karty", en: "Cards", desc_pl: "Kolor kategorii + ikona", desc_en: "Category color + icon" },
  magazine: { pl: "Magazyn", en: "Magazine", desc_pl: "Hero + lista boczna", desc_en: "Hero + side list" },
  timeline: { pl: "Oś czasu", en: "Timeline", desc_pl: "Chronologia z kropkami", desc_en: "Chronological dots" },
};
      ...
      return (
        <svg viewBox="0 0 120 70" className="w-full h-full">
          {[0, 1, 2].map((c) =>
            [0, 1].map((r) => (
              <g key={`${c}-${r}`}>
                <rect x={6 + c * 38} y={6 + r * 32} width={32} height={18} rx={2} fill={soft} stroke={stroke} />
                <rect x={6 + c * 38} y={26 + r * 32} width={22} height={2} fill={ink} />
                <rect x={6 + c * 38} y={30 + r * 32} width={16} height={2} fill={ink} opacity={0.5} />
              </g>
            )),
          )}
        </svg>
    ...
    case "list":
      return (
        <svg viewBox="0 0 120 70" className="w-full h-full">
          {[0, 1, 2, 3].map((i) => (
            <g key={i}>
              <rect x={6} y={6 + i * 15} width={20} height={11} rx={2} fill={soft} stroke={stroke} />
    ...
    case "slider":
      return (
        <svg viewBox="0 0 120 70" className="w-full h-full">
          {[0, 1, 2, 3].map((i) => (
            <g key={i}>
              <rect x={6 + i * 32} y={10} width={28} height={38} rx={2} fill={soft} stroke={stroke} />
        ...
        <svg viewBox="0 0 120 70" className="w-full h-full">
          {[
            ...
            <g key={i}>
              ...
              <rect x={card.x + 4} y={52} width={10} height={3} rx={1.5} fill={accent} opacity={0.7} />
            </g>
          ))}
        </svg>
      );
    case "magazine":
      return (
        <svg viewBox="0 0 120 70" className="w-full h-full">
          ...
          {[0, 1, 2].map((i) => (
            <g key={i}>
              <rect x={74} y={6 + i * 20} width={16} height={16} rx={2} fill={soft} stroke={stroke} />
