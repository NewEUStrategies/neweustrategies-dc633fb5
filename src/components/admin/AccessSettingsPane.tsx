...
import type { AccessEntityType, AccessMode, AccessPlan, ContentAccessRule } from "@/hooks/useContentAccess";
...
export function AccessSettingsPane({ entityType, entityId }: Props) {
  ...
  const [rule, setRule] = useState<Partial<ContentAccessRule>>({
    mode: "public", plan_ids: [], one_time_price_cents: null, one_time_currency: "PLN",
    teaser_pl: "", teaser_en: "",
  });
  ...
  useEffect(() => {
    (async () => {
      const [{ data: pl }, { data: r }] = await Promise.all([
        ...
        entityId
          ? supabase.from("content_access").select("*").eq("entity_type", entityType).eq("entity_id", entityId).maybeSingle()
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from("content_access").upsert(payload, { onConflict: "entity_type,entity_id" });
    setSaving(false);
    if (error) toast.error(error.message); else toast.success("Zapisano dostęp");
  };
  ...
  return (
    <div className="space-y-4 border border-border rounded-lg p-4 bg-card">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={save} disabled={saving}>Zapisz dostęp</Button>
      </div>
      ...
      {rule.mode === "paid" && (
        <>
          <div>
            ...
            {plans.length === 0 ? (
              <p className="text-xs text-muted-foreground mt-1">Brak planów. Utwórz je w „Paywall → Plany".</p>
            ) : (
              <div className="space-y-1.5 mt-2">
                {plans.map((p) => (
                  <label key={p.id} className="flex items-center gap-2 text-sm">
                    <Switch checked={(rule.plan_ids ?? []).includes(p.id)} onCheckedChange={() => togglePlan(p.id)} />
                    <span>{p.name_pl || p.name_en} · {(p.price_cents / 100).toFixed(2)} {p.currency} / {p.interval}</span>
                  </label>
                ))}
              </div>
            )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input type="number" min={0} value={rule.one_time_price_cents ?? 0}
                onChange={(e) => setRule({ ...rule, one_time_price_cents: Number(e.target.value) || null })} className="h-9" />
            </div>
            <div>
              <Label className="text-xs">Waluta</Label>
              <Input value={rule.one_time_currency ?? "PLN"} onChange={(e) => setRule({ ...rule, one_time_currency: e.target.value })} className="h-9" />
            </div>
          </div>
        </>
      )}
      ...
      {rule.mode !== "public" && (
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Textarea rows={3} value={rule.teaser_pl ?? ""} onChange={(e) => setRule({ ...rule, teaser_pl: e.target.value })} />
          </div>
          <div>
            <Label className="text-xs">Teaser EN (opcjonalny)</Label>
            <Textarea rows={3} value={rule.teaser_en ?? ""} onChange={(e) => setRule({ ...rule, teaser_en: e.target.value })} />
          </div>
        </div>
      )}
