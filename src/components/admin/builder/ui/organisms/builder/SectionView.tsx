...
import { Plus, Trash2, ChevronUp, ChevronDown, Columns2, Copy, Save, Eye } from "@/lib/lucide-shim";
import type {
  SectionNode,
  ColumnNode,
  InnerSectionNode,
...
import {
  ...
  hiddenOnDevice,
} from "../../organisms/widget-view/frame";
...
export function SectionView(p: SectionViewProps) {
  ...
  const children = (Array.isArray(p.section.children) ? p.section.children : []).filter(
    (c): c is ColumnNode | InnerSectionNode => !!c,
  );
  const colsSum =
    children.reduce(
      (a, c) => a + (c.kind === "column" ? resolveSpan(c.span, p.device, 12) : 12),
      0,
    ) || 12;
  ...
  const videoUrl =
    p.section.background?.type === "video"
  ...
  return (
    <div
      ...
      onClick={(e) => {
        e.stopPropagation();
        p.setSelection({ kind: "section", id: p.section.id });
      }}
    >
      ...
      <div
        className={`absolute -top-3 left-3 z-30 flex items-center gap-0.5 bg-background border border-border rounded px-1 py-0.5 text-[10px] shadow-sm transition ${selected ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
      >
        ...
        <IconBtn
          onClick={(e) => {
            e.stopPropagation();
            p.onToggleHidden("section", p.section.id);
          }}
          title={hidden ? `Pokaż na ${p.device}` : `Ukryj na ${p.device}`}
        >
      ...
      <div style={sectionContainerStyle(p.section)}>
        <div
          ...
          style={columnsRowStyle(p.section, colsSum)}
        >
          {children.map((child) => {
              ...
              return (
                ...
                >
                  <InnerSectionView
                    ...
                    onRemoveColumn={p.onRemoveColumn}
                    onDuplicateColumn={p.onDuplicateColumn}
                    onRemoveWidget={p.onRemoveWidget}
                    onDuplicateWidget={p.onDuplicateWidget}
...
function InnerSectionView({
  ...
  return (
    <div
      ...
      onClick={(e) => {
        e.stopPropagation();
        setSelection({ kind: "inner-section", id: inner.id });
      }}
    >
      <div
        className={`absolute -top-2.5 left-3 z-30 flex items-center gap-0.5 bg-background border border-border rounded px-1.5 py-0.5 text-[9px] font-medium shadow-sm transition ${selected ? "opacity-100" : "opacity-0 group-hover/inner:opacity-100"}`}
      >
        <span className="text-muted-foreground uppercase tracking-wider">Sekcja wewn.</span>
        <IconBtn
          onClick={(e) => {
            e.stopPropagation();
            onToggleHidden("inner-section", inner.id);
          }}
          title={hidden ? `Pokaż na ${device}` : `Ukryj na ${device}`}
        >
          <Eye className={`w-3 h-3 ${hidden ? "opacity-40" : ""}`} />
        </IconBtn>
      </div>
      <div
        className="grid gap-2 relative z-10 min-w-0 max-w-full"
        style={{ ...columnsRowStyle(inner, colsSum), padding: `${INNER_SECTION_SAFE_AREA_PX}px` }}
      >
        {columns.map((c) => (
          <div
            key={c.id}
          ...
          >
            <ColumnView
              ...
              onDuplicate={() => onDuplicateColumn(c.id)}
...
function ColumnView({
  ...
  const children = (Array.isArray(column.children) ? column.children : []).filter(
    (w): w is WidgetNode => !!w && isKnownWidgetType(w.type),
  );
  ...
  const isToolbar =
    ...
    (visibleChildren.length > 1 &&
      visibleChildren.every(
        (w) => COMPACT_WIDGET_TYPES.has(w.type) || AUTO_SIZE_WIDGETS.has(w.type),
      ));
  ...
  return (
    <div
      ...
      style={{
        ...
        minHeight: column.style?.minHeight ?? 80,
      ...
      onDrop={(e) => {
        ...
        if (t) {
          e.preventDefault();
          onDropWidget(column.id, t);
        }
      }}
    >
      <div
        className={`absolute -top-2.5 right-2 z-30 flex items-center gap-0.5 bg-background border border-border rounded px-1 py-0.5 text-[10px] shadow-sm transition ${selected || dragOver ? "opacity-100" : "opacity-0 group-hover/col:opacity-100"}`}
      >
        ...
        <IconBtn
          onClick={(e) => {
            e.stopPropagation();
            onToggleHidden("column", column.id);
          }}
          title={hidden ? `Pokaż na ${device}` : `Ukryj na ${device}`}
        >
          <Eye className={`w-3 h-3 ${hidden ? "opacity-40" : ""}`} />
      ...
      <SortableContext items={children.map((w) => w.id)} strategy={verticalListSortingStrategy}>
        {(() => {
          ...
          return (
            <div
              className={`flex ${stacked ? "flex-col" : "flex-row flex-wrap"} gap-2 min-w-0 max-w-full ${hClass} ${vClass}`}
            >
              {groups.map((g, gi) =>
                g.inline && stacked ? (
                  <div
                    key={gi}
                    className={`flex flex-row flex-wrap gap-2 min-w-0 max-w-full w-full ${column.contentAlign === "center" ? "justify-center" : column.contentAlign === "end" ? "justify-end" : "justify-start"} items-center`}
                  >
...
function SortableWidget({
  ...
  return (
    <div
      ...
      style={{ ...style, ...frameStyle, boxSizing: "border-box", padding: 0, overflow: "visible" }}
      {...attributes}
      onClick={(e) => {
        e.stopPropagation();
        onSelect();
    ...
    >
      <div
        className={`absolute -top-2.5 right-2 z-30 flex items-center gap-0.5 bg-background border border-border rounded px-1 py-0.5 text-[10px] shadow-sm transition ${selected ? "opacity-100" : "opacity-0 group-hover/w:opacity-100"}`}
      >
        <span {...listeners} className="cursor-grab text-muted-foreground px-1" title="Przeciągnij">
          ⋮⋮
        </span>
        <IconBtn
          onClick={(e) => {
