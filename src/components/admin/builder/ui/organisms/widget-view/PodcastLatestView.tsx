  ...
  if (variant === "featured") {
    ...
    return (
      <article className="grid lg:grid-cols-2 gap-6 border border-border rounded-xl overflow-hidden bg-card">
        {p.cover_image_url ? (
          <img src={p.cover_image_url} alt="" loading="lazy" className="w-full aspect-square object-cover" />
      ...
      <ul className="divide-y divide-border border border-border rounded-lg bg-card">
        {data.map((p) => (
          <li key={p.id} className="p-4 flex items-center gap-4">
            {p.cover_image_url ? (
              <img src={p.cover_image_url} alt="" loading="lazy" className="w-14 h-14 rounded object-cover shrink-0" />
