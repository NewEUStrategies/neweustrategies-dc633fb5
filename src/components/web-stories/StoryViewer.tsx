...
export function StoryViewer({ pages, lang, onClose, startIndex = 0 }: Props) {
  ...
  useEffect(() => {
    if (paused) {
      elapsedAtPauseRef.current = elapsedAtPauseRef.current + (performance.now() - startedAtRef.current);
    } else {
      startedAtRef.current = performance.now();
    }
  }, [paused]);
  ...
  const posClass =
    cur.text_position === "top" ? "top-16 bottom-auto" :
    cur.text_position === "center" ? "top-1/2 -translate-y-1/2 bottom-auto" :
    "bottom-10 top-auto";
  const alignClass =
    cur.text_align === "center" ? "text-center items-center" :
    cur.text_align === "right" ? "text-right items-end" :
    "text-left items-start";

  return (
    <div className="fixed inset-0 z-[100] bg-black text-white flex items-center justify-center" role="dialog" aria-modal="true" aria-label={lang === "en" ? "Web story" : "Historia"}>
      {/* Background */}
      ...
      <div className="absolute top-6 right-3 flex gap-2 z-20">
        <button
          aria-label={paused ? (lang === "en" ? "Resume" : "Wznów") : (lang === "en" ? "Pause" : "Pauza")}
      ...
      <div className={`absolute left-6 right-6 z-10 flex flex-col gap-2 ${posClass} ${alignClass}`}>
        {title && <h2 className="font-display text-2xl md:text-3xl drop-shadow-lg max-w-xl">{title}</h2>}
        {caption && <p className="text-sm md:text-base text-white/90 max-w-xl drop-shadow">{caption}</p>}
