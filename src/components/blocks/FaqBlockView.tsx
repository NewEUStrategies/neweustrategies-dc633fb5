...
interface Props { items: Item[]; title?: string; lang?: "pl" | "en" }
...
export function FaqBlockView({ items, title, lang = "pl" }: Props) {
  ...
  const L = lang === "pl" ? { fallback: "Najczęstsze pytania" } : { fallback: "Frequently Asked Questions" };
        ...
        {valid.map((it, i) => {
          ...
          return (
            <li key={i}>
              ...
              {isOpen && (
                <div className="px-4 pb-4 text-sm text-muted-foreground whitespace-pre-line">{it.a}</div>
              )}
            </li>
          );
        })}
