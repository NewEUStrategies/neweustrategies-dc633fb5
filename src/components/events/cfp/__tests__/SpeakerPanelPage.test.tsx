...
function mine(items: unknown[], overrides: Record<string, unknown> = {}) {
  return { event_id: "e1", event_slug: "kongres", timezone: "Europe/Warsaw", max_per_submitter: 3, score_max: 5, person: null, items, ...overrides };
}

async function renderPanel(options: { panelData?: unknown; mineData?: unknown; phase?: string } = {}) {
  stub().setData("event_my_speaker_panel", options.panelData === undefined ? panel() : options.panelData);
  stub().setData("event_my_cfp_submissions", options.mineData === undefined ? mine([]) : options.mineData);
...
describe("SpeakerPanelPage - zgłoszenia", () => {
  it("stan, informacja od organizatora, średnia ocen i kroki zgodne ze stanem", async () => {
    ...
    expect(screen.getByRole("link", { name: "eventCfp.speaker.reviewerLink" })).toHaveAttribute("href", "/events/kongres/review");
    // Limit liczy wszystko poza wycofanymi.
    expect(screen.getByText("eventCfp.speaker.submissions.limit(count=5,max=3)")).toBeInTheDocument();
    expect(screen.queryByRole("link", { name: "eventCfp.speaker.submissions.new" })).not.toBeInTheDocument();

    const card = (id: string) => {
      const heading = screen.getByRole("heading", { name: id === "s1" ? "-" : `Wystąpienie ${id}` });
      const li = heading.closest("li");
      if (li === null) throw new Error("test");
      return within(li);
    };
    expect(card("s1").getByRole("link", { name: "eventCfp.speaker.submissions.continueDraft" })).toHaveAttribute(
      "href",
      "/events/kongres/cfp-submit?id=s1",
    );
    expect(card("s1").getByRole("button", { name: "eventCfp.speaker.submissions.deleteDraft" })).toBeInTheDocument();
    ...
    expect(card("s2").getByRole("link", { name: "eventCfp.speaker.submissions.edit" })).toBeInTheDocument();
    expect(card("s3").getByText("eventCfp.speaker.submissions.acceptedHint")).toBeInTheDocument();
    expect(card("s3").getByText("eventCfp.speaker.submissions.reviewsInfo(avg=4,3,count=3,max=5)")).toBeInTheDocument();
    expect(card("s3").getByRole("button", { name: "eventCfp.speaker.submissions.confirm" })).toBeInTheDocument();
    expect(card("s3").queryByRole("button", { name: "eventCfp.speaker.submissions.withdraw" })).not.toBeInTheDocument();
    expect(card("s4").getByText("eventCfp.speaker.submissions.confirmedHint")).toBeInTheDocument();
    expect(card("s4").getByRole("button", { name: "eventCfp.speaker.submissions.withdraw" })).toBeInTheDocument();
  ...
  it("pusta lista i nowe zgłoszenie przy otwartym naborze pod limitem", async () => {
    ...
    expect(screen.queryByRole("link", { name: "eventCfp.speaker.reviewerLink" })).not.toBeInTheDocument();
    cleanup();
  ...
  it("wycofanie i usunięcie szkicu: potwierdzenie, wynik i odmowa", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "eventCfp.speaker.submissions.deleteDraft" }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "eventCfp.speaker.submissions.deleteDraft" }));
    await waitFor(() => expect(h.toastSuccess).toHaveBeenCalledWith("eventCfp.speaker.submissions.deleted"));
    ...
    await waitFor(() => expect(h.toastSuccess).toHaveBeenCalledWith("eventCfp.speaker.submissions.withdrawn"));
    expect(h.confirm.mock.lastCall?.[0 as number]).toMatchObject({ title: "eventCfp.speaker.submissions.withdrawTitle" });
  ...
  it("odpowiedź na przyjęcie: potwierdzenie bez pytania, rezygnacja z pytaniem", async () => {
    ...
    await waitFor(() => expect(h.toastSuccess).toHaveBeenCalledWith("eventCfp.speaker.submissions.confirmed"));
    ...
    fireEvent.click(screen.getByRole("button", { name: "eventCfp.speaker.submissions.decline" }));
    await waitFor(() => expect(h.toastSuccess).toHaveBeenCalledWith("eventCfp.speaker.submissions.declined"));
...
describe("SpeakerPanelPage - profil", () => {
  ...
  it("zapis profilu: błąd przy polu, potem ładunek z tematami i językami jako listy", async () => {
    ...
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.photo"), { target: { value: "http://x.pl/a.jpg" } });
    ...
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.photo"), { target: { value: "https://x.pl/a.jpg" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.headlineEn"), { target: { value: "Expert" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.bioPl"), { target: { value: "Nowy biogram" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.bioEn"), { target: { value: "Bio" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.topicsPl"), { target: { value: "energia, sieci" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.topicsEn"), { target: { value: "grids" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.languages"), { target: { value: "PL, en" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.headlinePl"), { target: { value: "Prezeska" } });
    stub().setData("event_my_speaker_profile_set", {});
    fireEvent.click(screen.getByRole("button", { name: "eventCfp.common.save" }));
    await waitFor(() => expect(h.toastSuccess).toHaveBeenCalledWith("eventCfp.speaker.profile.saved"));
  ...
  it("każda reguła profilu ma swoje zdanie przy polu", async () => {
    ...
    await screen.findByText("eventCfp.speaker.title");
    openTab("eventCfp.speaker.tabs.profile");
    const save = () => fireEvent.click(screen.getByRole("button", { name: "eventCfp.common.save" }));
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.headlinePl"), { target: { value: "x".repeat(201) } });
    save();
    expect(screen.getByText("eventCfp.speaker.profile.validation.headline")).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.headlinePl"), { target: { value: "ok" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.bioPl"), { target: { value: "x".repeat(4001) } });
    expect(screen.getByText("eventCfp.speaker.profile.validation.bio")).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.bioPl"), { target: { value: "ok" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.topicsPl"), { target: { value: "x".repeat(61) } });
    expect(screen.getByText("eventCfp.speaker.profile.validation.topics")).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.topicsPl"), { target: { value: "a" } });
    fireEvent.change(screen.getByLabelText("eventCfp.speaker.profile.languages"), { target: { value: "polski" } });
...
describe("SpeakerPanelPage - wystąpienia", () => {
  it("godzina w strefie wydarzenia, sala, ścieżka, rola i szkic; pusta lista", async () => {
    ...
    expect(within(first).getByText("eventCfp.speaker.sessions.room(room=Sala A)")).toBeInTheDocument();
    expect(within(first).getByText("eventCfp.speaker.sessions.track(track=Energia)")).toBeInTheDocument();
...
describe("SpeakerPanelPage - materiały", () => {
  it("lista: rodzaj, widoczność, stan publikacji i bezpieczny odnośnik", async () => {
    ...
    expect(within(row).getByRole("link", { name: "eventCfp.speaker.materials.open" })).toHaveAttribute(
      "rel",
  ...
  it("nowy materiał: walidacja, ładunek i zamknięcie okna; odmowa bazy", async () => {
    ...
    expect(within(dialog).getByText("eventCfp.speaker.materials.validation.title")).toBeInTheDocument();
    fireEvent.change(within(dialog).getByLabelText("eventCfp.speaker.materials.titlePl"), { target: { value: "Prezentacja" } });
    fireEvent.change(within(dialog).getByLabelText("eventCfp.speaker.materials.titleEn"), { target: { value: "Deck" } });
    fireEvent.change(within(dialog).getByLabelText("eventCfp.speaker.materials.url *"), { target: { value: "ftp://x" } });
    expect(within(dialog).getByText("eventCfp.speaker.materials.validation.url")).toBeInTheDocument();
    ...
    expect(within(selects[2] as HTMLElement).getAllByRole("option").map((o) => o.textContent)).toEqual([
      "eventCfp.speaker.materials.noSession",
      "Sesja otwarcia",
      "Panel roboczy",
    ]);
    ...
    fireEvent.click(within(dialog).getByRole("button", { name: "eventCfp.common.save" }));
    await waitFor(() => expect(h.toastSuccess).toHaveBeenCalledWith("eventCfp.speaker.materials.saved"));
  ...
  it("edycja zaczyna od materiału; anulowanie; wydarzenie bez wystąpień nie pyta o sesję", async () => {
    ...
    const dialog = await screen.findByRole("dialog");
    expect(within(dialog).getByLabelText("eventCfp.speaker.materials.titlePl")).toHaveValue("Slajdy");
    expect(within(dialog).getAllByRole("combobox")).toHaveLength(2);
    stub().setData("event_my_speaker_material_upsert", "m1");
    fireEvent.click(within(dialog).getByRole("button", { name: "eventCfp.common.save" }));
    await waitFor(() => expect(payload("event_my_speaker_material_upsert")).toMatchObject({ id: "m1", session_id: "ses1" }));
    await waitFor(() => expect(screen.queryByRole("dialog")).not.toBeInTheDocument());
    fireEvent.click(within(row).getByRole("button", { name: "eventCfp.common.edit" }));
  ...
  it("zapis w toku blokuje przycisk", async () => {
    ...
    await screen.findByText("eventCfp.speaker.title");
    openTab("eventCfp.speaker.tabs.materials");
    fireEvent.click(screen.getAllByRole("button", { name: "eventCfp.common.edit" })[0] as HTMLElement);
    const dialog = await screen.findByRole("dialog");
    stub().setResponse("event_my_speaker_material_upsert", () => new Promise(() => undefined) as never);
    fireEvent.click(within(dialog).getByRole("button", { name: "eventCfp.common.save" }));
    expect(await within(dialog).findByRole("button", { name: "eventCfp.common.saving" })).toBeDisabled();
  });
  ...
  it("usunięcie materiału: potwierdzenie, wynik i odmowa", async () => {
    ...
    openTab("eventCfp.speaker.tabs.materials");
    h.confirm.mockResolvedValueOnce(false);
    fireEvent.click(screen.getAllByRole("button", { name: "eventCfp.common.delete" })[0] as HTMLElement);
    await waitFor(() => expect(h.confirm).toHaveBeenCalledTimes(1));
    expect(stub().callsFor("event_my_speaker_material_delete")).toHaveLength(0);
    stub().setData("event_my_speaker_material_delete", true);
    fireEvent.click(screen.getAllByRole("button", { name: "eventCfp.common.delete" })[1] as HTMLElement);
    await waitFor(() => expect(h.toastSuccess).toHaveBeenCalledWith("eventCfp.speaker.materials.deleted"));
