...
const { InvoiceRequestDialog } = await import("@/components/events/invoices/molecules/InvoiceRequestDialog");
...
describe("InvoiceRequestDialog", () => {
  it("niepoprawne dane: bledy przy polach, bez zapisu", () => {
    renderWithQueryClient(
      <InvoiceRequestDialog target={TARGET} initial={emptyBuyerDraft()} onClose={vi.fn()} onSaved={vi.fn()} />,
    );
  ...
  it("poprawne dane: zapis prosby i zamkniecie", async () => {
    ...
    const onSaved = vi.fn();
    renderWithQueryClient(<InvoiceRequestDialog target={TARGET} initial={VALID} onClose={vi.fn()} onSaved={onSaved} />);
    fireEvent.click(screen.getByRole("button", { name: "eventInvoices.profile.submit" }));
    await waitFor(() =>
      expect(screen.getByRole("button", { name: "eventInvoices.profile.submitting" })).toHaveProperty("disabled", true),
    );
  ...
  it("odmowa bazy: zdanie kupujacego, okno zostaje", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "eventInvoices.profile.submit" }));
    expect((await screen.findByRole("alert")).textContent).toBe("eventInvoices.errors.requestWindowClosed");
  ...
  it("anuluj zamyka okno", () => {
    const onClose = vi.fn();
    renderWithQueryClient(<InvoiceRequestDialog target={TARGET} initial={VALID} onClose={onClose} onSaved={vi.fn()} />);
