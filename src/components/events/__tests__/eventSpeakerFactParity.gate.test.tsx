...
const CONSUMER_EXCEPTIONS: Record<string, string> = {
  ...
  "src/components/builder/organisms/widget-view/SpeakersWidget.tsx":
    'WIDGET REDAKCYJNY, nie lista jednego wydarzenia - i to jest cały powód, bo ORGANIZACJI JUŻ NIE BRAKUJE: przy źródle „baza" karta bierze ją z kolumny `company` tego samego wiersza RPC (`speakerRowToItem` -> klucz `organization`), a przy wpisie ręcznym z pola „Organizacja" w `SpeakersEditor`; dowód zachowania dla OBU źródeł stoi w `components/builder/organisms/widget-view/__tests__/eventWidgets.test.tsx`. Wyjątek zostaje z dwóch powodów mechanicznych. (1) ŚLADEM FAKTU EKSPERTA jest w tej bramce KLUCZ i18n `eventFront.speakers.expertBadge`, a widget rysuje własną plakietkę z własnym napisem (`lang === "pl"`), bo powierzchnia `widget-view` nie jedzie nakładką frontu wydarzenia - wciągnięcie go tutaj czerwieniłoby `is_expert` na widgecie, choć fakt JEST narysowany. (2) ZESTAW FAKTÓW WIDGETU TO SUMA, NIE PROJEKCJA: to samo pole karty (`SpeakerItem`) wypełniają wpisy wpisane ręcznie w studiu, a ich model treści nie ma ani `has_speaker_profile`, ani kolumny eksperta, więc wiersz „ten sam człowiek na dwóch powierzchniach" dla trybu ręcznego nie istnieje',
};
