...
function SectionCard({ icon, title, empty, action, children, isEmpty }: SectionCardProps) {
  return (
    <section className="rounded-[6px] border border-border bg-card p-4 sm:p-5">
      ...
      {isEmpty ? <p className="text-sm italic text-muted-foreground">{empty}</p> : children}
    </section>
  );
}
...
function AddToggle({
  ...
  return (
    <Button type="button" size="sm" variant="ghost" onClick={onClick} className="h-7 px-2 text-xs">
      ...
      action={
        editable ? (
          <AddToggle
            open={open}
            onClick={() => setOpen((v) => !v)}
            label={t("profile.forms.addExperience")}
          />
        ) : undefined
      }
    >
      <ul className="space-y-3">
        {items.map((e) => (
          ...
          >
            ...
            {editable ? (
              <DeleteBtn onClick={() => remove(e.id)} label={t("profile.actions.remove")} />
            ) : null}
          </li>
        ))}
      </ul>
      {open ? (
        <div className="mt-3 space-y-2 rounded border border-dashed border-border bg-background/40 p-3">
          <MiniField
            label={t("profile.forms.roleTitle")}
            value={form.role_title}
            onChange={(v) => setForm({ ...form, role_title: v })}
          />
          <MiniField
            label={t("profile.forms.company")}
            value={form.company}
          ...
          <div className="grid grid-cols-2 gap-2">
            <MiniField
              type="date"
              label={t("profile.forms.startDate")}
              value={form.start_date}
              onChange={(v) => setForm({ ...form, start_date: v })}
            />
            <MiniField
              type="date"
              label={t("profile.forms.endDate")}
              value={form.end_date}
          ...
          <MiniArea
            label={t("profile.forms.description")}
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button type="button" size="sm" onClick={add} className="h-7 px-3 text-xs">
      ...
      action={
        editable ? (
          <AddToggle
            open={open}
            onClick={() => setOpen((v) => !v)}
            label={t("profile.forms.addEducation")}
          />
        ) : undefined
      }
    >
        ...
        {items.map((e) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-[11px] text-muted-foreground">
                {formatRange(e.start_date, e.end_date, false, t)}
              </p>
            </div>
            {editable ? (
              <DeleteBtn onClick={() => remove(e.id)} label={t("profile.actions.remove")} />
      ...
      {open ? (
        <div className="mt-3 space-y-2 rounded border border-dashed border-border bg-background/40 p-3">
          <MiniField
            label={t("profile.forms.school")}
            value={form.school}
            onChange={(v) => setForm({ ...form, school: v })}
          />
          <div className="grid grid-cols-2 gap-2">
            <MiniField
              label={t("profile.forms.degree")}
              value={form.degree}
              onChange={(v) => setForm({ ...form, degree: v })}
            />
            <MiniField
              label={t("profile.forms.field")}
              value={form.field}
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button size="sm" onClick={add} className="h-7 px-3 text-xs">
              {t("profile.actions.save")}
            </Button>
          </div>
        </div>
...
export function AwardsSection({
  ...
  tenantId: string;
  editable: boolean;
  kind: "award" | "recognition" | "mention";
}) {
  const { t } = useTranslation();
  const qc = useQueryClient();
  ...
  const title =
    kind === "award"
      ? t("profile.sections.awards")
      : kind === "recognition"
        ? t("profile.sections.recognitions")
        : t("profile.sections.mentions");
  const empty =
    kind === "award"
      ? t("profile.sections.awardsEmpty")
      : kind === "recognition"
        ? t("profile.sections.recognitionsEmpty")
        : t("profile.sections.mentionsEmpty");
      ...
      action={
        editable ? (
          <AddToggle
            open={open}
            onClick={() => setOpen((v) => !v)}
            label={t("profile.forms.addAward")}
          />
        ) : undefined
      }
    >
        ...
        {items.map((a) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              {a.issuer ? (
                <p className="truncate text-xs text-muted-foreground">{a.issuer}</p>
              ) : null}
              {a.awarded_at ? (
                <p className="text-[11px] text-muted-foreground">{a.awarded_at}</p>
              ) : null}
              {a.description ? (
                <p className="mt-1 whitespace-pre-wrap text-xs text-foreground/80">
                  {a.description}
            ...
            {editable ? (
              <DeleteBtn onClick={() => remove(a.id)} label={t("profile.actions.remove")} />
            ) : null}
          </li>
        ))}
      </ul>
      {open ? (
        <div className="mt-3 space-y-2 rounded border border-dashed border-border bg-background/40 p-3">
          <MiniField
            label={t("profile.forms.title")}
            value={form.title}
            onChange={(v) => setForm({ ...form, title: v })}
          />
          <MiniField
            label={t("profile.forms.issuer")}
            value={form.issuer}
          ...
          <div className="grid grid-cols-2 gap-2">
            <MiniField
              type="date"
              label={t("profile.forms.awardedAt")}
              value={form.awarded_at}
              onChange={(v) => setForm({ ...form, awarded_at: v })}
            />
            <MiniField
              label={t("profile.forms.url")}
              value={form.url}
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            ...
            {editable ? (
              <button
                ...
                className="inline-flex h-7 items-center gap-1 rounded-[6px] border border-border px-2 text-xs text-destructive hover:bg-destructive/10"
              >
        ...
        <div className="mt-4">
          ...
          <ul className="space-y-1.5">
            {items.map((f) => (
              <li
                key={f.id}
                className="flex items-center justify-between gap-2 rounded border border-border/40 bg-background/30 p-2 text-xs"
              >
                ...
                <span className="flex shrink-0 items-center gap-2 text-[11px] text-muted-foreground">
                  ...
                  {editable && !f.is_current ? (
                    <button
                      type="button"
                      onClick={() => markCurrent(f.id)}
                      aria-label={t("profile.sidebar.retakeTest")}
                  ...
                  {editable ? (
                    <DeleteBtn onClick={() => remove(f.id)} label={t("profile.actions.remove")} />
                  ) : null}
                </span>
              </li>
            ))}
