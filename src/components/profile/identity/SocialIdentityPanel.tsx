// Panel "Social i bio" skonsolidowanej edycji tożsamości (/profile/edit).
...
export function SocialIdentityPanel() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    if (slugBlocked) {
      toast.error(
        t(
          `profile.social.slug${slugStatus === "taken" ? "Taken" : slugStatus === "reserved" ? "Reserved" : slugStatus === "short" ? "TooShort" : "Invalid"}`,
        ),
      );
      return;
    }
        ...
        <CardContent>
          <form className="grid gap-5" onSubmit={save}>
            ...
            <div className="grid gap-2">
              <FieldLabel htmlFor="slug" tip={t("profile.social.tip.slug")}>
                {t("profile.social.slug")}
              </FieldLabel>
              <Input
                id="slug"
                value={data.slug ?? ""}
              ...
              {data.slug ? (
                <div className="flex items-center gap-2 rounded-md border border-border/60 bg-muted/40 px-2.5 py-1.5 text-xs">
                  <span className="text-muted-foreground normal-case tracking-normal font-normal">
                    {t("profile.social.slugPreview")}:
                  </span>
                  <code className="font-mono text-foreground truncate">
                    {origin}/author/{slugify(data.slug) || data.slug}
                  </code>
                </div>
              ) : null}
              <div className="flex items-center justify-between gap-2">
                <p
                  className={`text-[11px] normal-case tracking-normal font-normal ${slugStatus === "ok" ? "text-emerald-600 dark:text-emerald-400" : slugBlocked ? "text-destructive" : "text-muted-foreground"}`}
                >
                ...
                {suggestSource &&
                slugify(suggestSource) &&
                slugify(suggestSource) !== (data.slug ?? "").trim() ? (
                  <button
                    type="button"
                    onClick={suggestSlug}
                    className="text-[11px] normal-case tracking-normal font-medium text-primary hover:underline shrink-0"
                  >
            ...
            <section className="grid gap-3">
              ...
              <div className="profile-grid-2">
                <div className="grid gap-2">
                  <FieldLabel htmlFor="bio_pl" tip={t("profile.social.tip.bioPl")}>
                    {t("profile.social.bioPl")}
                  </FieldLabel>
                  <Textarea
                    id="bio_pl"
                    rows={3}
                ...
                <div className="grid gap-2">
                  <FieldLabel htmlFor="bio_en" tip={t("profile.social.tip.bioEn")}>
                    {t("profile.social.bioEn")}
                  </FieldLabel>
                  <Textarea
                    id="bio_en"
                    rows={3}
