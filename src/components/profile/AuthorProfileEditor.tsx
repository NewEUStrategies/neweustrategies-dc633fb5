// Reużywalny edytor profilu autora - identyczny formularz dla /profile/author
// (mode="self") i /admin/users/$id (mode="admin"). Trzyma pełny zestaw pól:
// avatar, bio (bullets PL/EN), full_bio (hub), org_functions, obszary
// ekspertyzy, kontakt publiczny, kontakt dla mediów, socials, custom_socials,
// media_mentions. Zapis: author_profiles (upsert) + profiles.bio_* (update)
...
const SOCIAL_ICONS: Record<
  string,
  { slug: string; Fallback: ComponentType<{ className?: string } & SVGAttributes<SVGSVGElement>> }
> = {
...
export interface AuthorProfileEditorProps {
  /** ID użytkownika, którego profil edytujemy. */
  userId: string;
  /** Tenant kontekstu (dla insertów w author_profiles). */
  tenantId: string | null;
  /** self = własny profil (/profile/author); admin = edycja przez staff. */
  mode: "self" | "admin";
}

export function AuthorProfileEditor({ userId, tenantId, mode }: AuthorProfileEditorProps) {
  ...
  // OG refresh działa tylko dla własnego profilu (używa auth.uid()).
  ...
  const upload = async (blob: Blob) => {
    ...
    setUploading(true);
    try {
      const path = `${tenantId}/users/${userId}/author-avatar-${Date.now()}.jpg`;
    ...
    <TooltipProvider>
      ...
      <form className="grid gap-6" onSubmit={save}>
        ...
        <section className="grid gap-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-2">
              <FieldLabel htmlFor="ape-job_title">{t("profile.account.jobTitle")}</FieldLabel>
              <Input
                id="ape-job_title"
        ...
        <section className="grid gap-4">
          ...
          <div className="grid gap-2">
            <FieldLabel htmlFor="ape-full_bio_en">{t("expert.fullBioEn")}</FieldLabel>
            <Textarea
              id="ape-full_bio_en"
        ...
        <section className="grid gap-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="grid gap-2">
              <FieldLabel htmlFor="ape-phone">{t("profile.account.phone")}</FieldLabel>
              <Input
                id="ape-phone"
                type="tel"
                value={data.phone ?? ""}
        ...
        <section className="grid gap-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-2 sm:col-span-2">
              <FieldLabel htmlFor="ape-media_contact_name">
                {t("expert.mediaContactName")}
              </FieldLabel>
              <Input
                id="ape-media_contact_name"
            ...
            <div className="grid gap-2">
              <FieldLabel htmlFor="ape-media_contact_email">
                {t("expert.mediaContactEmail")}
              </FieldLabel>
              <Input
                id="ape-media_contact_email"
                type="email"
            ...
            <div className="grid gap-2">
              <FieldLabel htmlFor="ape-media_contact_phone">
                {t("expert.mediaContactPhone")}
              </FieldLabel>
              <Input
                id="ape-media_contact_phone"
                type="tel"
        ...
        {exists && <MediaMentionsSection userId={userId} />}
