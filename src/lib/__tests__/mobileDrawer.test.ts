...
describe("mobileDrawer schemas", () => {
  ...
  it("rejects section_order duplicates", () => {
    expect(() => sectionOrderSchema.parse(["top_tools", "top_tools", "account", "nav"])).toThrow();
  });
  ...
  it("parseDrawerConfig falls back to defaults on garbage input", () => {
    ...
    expect(
      parseDrawerConfig({ section_order: ["top_tools"], top_tools: "x", nav_items: [] }),
    ).toEqual(DEFAULT_DRAWER_CONFIG);
  });
});
