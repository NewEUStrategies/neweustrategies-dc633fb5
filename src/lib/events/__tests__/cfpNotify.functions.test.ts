...
vi.mock("@/integrations/supabase/auth-middleware", () => ({ requireSupabaseAuth: { name: "requireSupabaseAuth" } }));
vi.mock("@/lib/email/transactional.server", () => ({ sendTxEmail }));

const { notifyCfpDecision, confirmCfpSubmissionEmail } = await import("@/lib/events/cfpNotify.functions");
...
function markCalls(rpc: ReturnType<typeof vi.fn>) {
  return rpc.mock.calls.filter((call) => call[0] === "admin_event_cfp_mark_notified").map((call) => call[1]);
}
...
describe("kontrakt wejścia", () => {
  it("obie funkcje wymagają sesji i identyfikatora w kształcie UUID", () => {
    ...
    expect(validateServerFnInput(notifyCfpDecision, { submissionId: ID })).toEqual({ submissionId: ID });
...
describe("notifyCfpDecision", () => {
  it("wysyła mail o decyzji z kluczem ze stemplem decyzji i stempluje wysyłkę", async () => {
    ...
    const result = await callServerFn(notifyCfpDecision, { data: { submissionId: ID }, context: { supabase } });
    expect(result).toEqual({ ok: true });
    expect(supabase.rpc).toHaveBeenCalledWith("admin_event_cfp_notify_payload", { p_submission_id: ID });
    ...
    expect(markCalls(supabase.rpc)).toEqual([{ p_payload: { submission_id: ID, status: "accepted" } }]);
  });
  ...
  it("nieudana wysyłka zapisuje błąd w bazie i wraca jako odmowa", async () => {
    sendTxEmail.mockResolvedValue({ ok: false, skipped: "suppressed", reason: "suppressed:bounce" });
    ...
    expect(
      await callServerFn(notifyCfpDecision, { data: { submissionId: ID }, context: { supabase: client(PAYLOAD) } }),
  ...
  it("stan bez maila, brak adresu, brak ładunku i odmowa bazy - bez wysyłki", async () => {
    const run = (payload: unknown, error: { message: string } | null = null) =>
      callServerFn(notifyCfpDecision, { data: { submissionId: ID }, context: { supabase: client(payload, error) } });
    expect(await run({ ...PAYLOAD, notice: null })).toEqual({ ok: true, skipped: "not_applicable" });
    ...
    expect(await run(null, { message: "forbidden: x" })).toEqual({ ok: false, error: "forbidden: x" });
...
describe("confirmCfpSubmissionEmail", () => {
  it("potwierdza wysłane zgłoszenie z kluczem ze stemplem wysłania", async () => {
    ...
    expect(result).toEqual({ ok: true });
    expect(supabase.rpc).toHaveBeenCalledWith("event_cfp_submission_notice", { p_submission_id: ID });
    expect(sendTxEmail).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("szkic, brak adresu, brak ładunku, odmowa bazy i nieudana wysyłka", async () => {
    ...
    expect(await run({ ...PAYLOAD, status: "draft" })).toEqual({ ok: true, skipped: "not_applicable" });
    expect(await run({ ...PAYLOAD, status: "submitted", email: "" })).toEqual({ ok: false, error: "no_recipient" });
    expect(await run({ ...PAYLOAD, status: "submitted", email: null })).toEqual({ ok: false, error: "no_recipient" });
    expect(await run("x")).toEqual({ ok: false, error: "not_found" });
    expect(await run(null, { message: "auth_required: x" })).toEqual({ ok: false, error: "auth_required: x" });
    expect(sendTxEmail).not.toHaveBeenCalled();

    sendTxEmail.mockResolvedValue({ ok: false, reason: "suppressed:complaint" });
    expect(await run({ ...PAYLOAD, status: "submitted" })).toEqual({ ok: false, error: "suppressed:complaint" });
    sendTxEmail.mockResolvedValue({ ok: false, error: "smtp" });
    expect(await run({ ...PAYLOAD, status: "submitted" })).toEqual({ ok: false, error: "smtp" });
    sendTxEmail.mockResolvedValue({ ok: false, skipped: "no_recipient" });
    expect(await run({ ...PAYLOAD, status: "submitted" })).toEqual({ ok: false, error: "no_recipient" });
    sendTxEmail.mockResolvedValue({ ok: false });
    expect(await run({ ...PAYLOAD, status: "submitted" })).toEqual({ ok: false, error: "send_failed" });
  });
});
