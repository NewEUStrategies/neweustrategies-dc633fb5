...
/**
 * Builder zapytań. Generyk opisuje `data` po rozwiązaniu - domyślnie `unknown`,
 * bo bez wygenerowanych typów nikt nie zna kształtu wiersza.
 *
 * `PromiseLike` jest tu ROZSZERZANY, nie naśladowany: dzięki temu `await q`
 * daje `LooseResult<Row>` bez jednego rzutowania.
 */
export interface LooseQuery<Row = unknown> extends PromiseLike<LooseResult<Row>> {
 ...
 */
export function rowsOf(result: LooseResult): unknown[] {
