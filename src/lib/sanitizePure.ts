/**
 * CZYSTE (bez zależności) helpery sanityzacji: URL-e, id/klasy HTML, scoping
 * customowego CSS, hartowanie <style> i konwersja HTML -> tekst.
 *
 * PO CO OSOBNY MODUŁ. `lib/sanitize.ts` importuje DOMPurify (~82 kB źródeł)
 * na poziomie modułu, a większość konsumentów na ścieżce bootowania
 * (chrome: menu, ticker, style tenantowe, widgety proste) używa WYŁĄCZNIE
 * czystych funkcji regexowych. Jedna krawędź `import { safeUrl } from
 * "@/lib/sanitize"` wciągała DOMPurify do chunku wejściowego KAŻDEJ strony.
 * Moduły na ścieżce bootowania importują stąd; `sanitize.ts` re-eksportuje
 * całość, więc dotychczasowi konsumenci (100+ plików) działają bez zmian.
 *
 * ZASADA: ten plik NIE MOŻE importować niczego poza typami - żadnych
 * bibliotek, żadnych modułów z zależnościami. Pilnuje tego bramka
 * check-entry-purity (marker DOMPurify w grafie bootowania).
 */
