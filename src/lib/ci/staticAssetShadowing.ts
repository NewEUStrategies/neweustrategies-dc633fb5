 ...
 *
 * Dokument HTML odpowiada pod WIĘCEJ niż jednym adresem - to domyślne
 * `html_handling: auto-trailing-slash` w Workers Assets (Vite zachowuje się tak
 * samo), więc bramka musi liczyć wszystkie:
 *   * `index.html` odpowiada TAKŻE pod adresem swojego katalogu - najgroźniejszy
 *     przypadek przesłonięcia, bo zabija SSR całej strony, nie jednej końcówki;
 *   * każdy inny `*.html` odpowiada TAKŻE pod adresem bez rozszerzenia, więc
 *     `public/sitemap.html` przesłania trasę `/sitemap` - adres, którego w
 *     samym pliku nie widać.
 */
