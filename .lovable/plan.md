...
- [x] **10. Stripe** — `subscription_data[trial_period_days]` z `access_plans.trial_days` (klamrowane do 730) w Checkout Session; webhook `customer.subscription.updated` synchronizuje `status`/`current_period_end`/`canceled_at` z `cancel_at_period_end` (trialing/past_due mapowane na `active`, canceled/incomplete_expired na `canceled`); webhook `charge.refunded` oznacza `payment_orders.status='refunded'` i wygasza dostęp — dla subskrypcji `user_subscriptions.status='canceled'` + `current_period_end=now()`, dla one-time `user_purchases.status='refunded'`. Match po `provider_intent_id` (payment_intent). Typecheck OK.
