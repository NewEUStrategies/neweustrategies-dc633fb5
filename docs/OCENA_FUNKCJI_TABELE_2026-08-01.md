# Tabele modułów — funkcja po funkcji: ocena, argumenty, rekomendacje + pozycja vs konkurencja (2026-08-01)

**Data:** 2026-08-01 · **HEAD:** `1701abc` (main po PR #111–#126) · **Gałąź:** `claude/audit-update-2026-07-30-2zmdtd`

Dokument rozbija platformę na **20 modułów × pojedyncze funkcje**. Każda funkcja ma: ocenę 0–10,
argument **✅ dobry**, argument **⚠️ słaby** i **🔧 rekomendację**. To **wydanie 2026-08-01** dokumentu
`OCENA_FUNKCJI_TABELE_2026-07-30.md` (tamten pozostaje nietknięty jako migawka stanu 30.07). Oceny są
spójne z `AUDYT_BRUTALNY_REWIZJA_ZALOZEN_2026-07-30.md` po jego aktualizacji 01.08 (werdykt 7,0 → 7,5).
...
**Co się zmieniło od wydania 30.07 (fala PR #111–#126, zweryfikowana na kodzie, nie na deklaracjach):**
wszystkie P0 i P1 audytu domknięte — scheduler push/digestów (pg_cron + siatka Actions z samozbrojeniem),
unifikacja suppression + dren kolejki + runner z heartbeatem, filtr tenanta w `get_chat_peers` (+ kontrakt
pgTAP), izolacja sandbox/live w płatnościach one-time, zamknięcie furtki Big Five, statyczny gate CI na
anonimowe INSERT-y. Do tego: wycięty martwy panel darowizn (uczciwy link zewnętrzny), SSR trackera,
SSR-paginacja bloga i archiwum autora, inline edytor layoutów ekspertów, licznik meteringu w UI,
konsolidacja kotwic TOC, nowa warstwa „Partner Biznesowy", +2 widgety buildera, **odblokowany pgTAP w CI**
(w ciągu doby złapał pierwszy realny defekt). Sygnały na tym HEAD: `vitest` **3666 pass / 0 fail**,
`tsc` czysto, gate anon-insert ✓; `eslint` czerwony (1440 problemów — dryf przez commity prosto na main,
mimo że lint jest już bramką CI). Jedyny niedomknięty punkt P1: `ON DELETE CASCADE` na dowodach księgowych.
...
Konwencja: ocena „**X → Y**" = zmiana względem wydania 30.07; „(nowa)" = funkcja dodana po 30.07.
...
| RLS coverage | **9** | **0 tabel bez RLS**; parser stanu końcowego (gate) liczy **517 realnych polityk** — metryka odporna na churn | Stare metryki mylące: „915" liczyło instrukcje churnu, „408" pary nazwa+tabela | Używać metryki stanu końcowego w dokumentacji |
...
| Anonimowe INSERT-y | **4 → 8** | **Klasa błędu domknięta gate'em CI** (PR #124): `check-sql-anon-insert.ts` (271 linii) — inwariant A (żadna permisywna polityka INSERT dla `anon`/`public`) + B (6 tabel intake bez klienckiego INSERT); **self-test nie-pustości** (wstrzyknięta polityka failuje gate); ✓ na tym HEAD; równolegle Lovable dokręcił polityki `introduction_requests`/`user_connections`/`profiles` | Gate działa w CI — commity pchane prosto na main weryfikuje dopiero post-hoc | Utrzymać; rozważyć ochronę gałęzi `main` |
...
(checkout_settings, autozapis stron, alerty trackera, odznaki), dryf formatowania na main i otwartą
połowę P1 (CASCADE faktur). 11 modułów w górę, 9 bez zmian, **żaden w dół**.
...
> `OCENA_FUNKCJI_KONKURENCI_2026-07-24.md` (stan do poł. 2026). Oceny NES — stan 01.08 (po fali napraw
> PR #111–#126); w nawiasach delta vs wydanie 30.07.
...
Polska liga to statyczne WordPressy (agregat 1,8–2,4). Żaden nie ma paywalla klasy produkcyjnej,
wyszukiwarki ponad podstawową, czatu, sieci ani profilu użytkownika. **To nie jest ta sama kategoria produktu.**
...
paywallem/członkostwem) miały 30.07 tylko +0,8 w konwersji; po wycięciu wydmuszki darowizn, ożywieniu
runnera newslettera i nowej warstwie Partner Biznesowy jest +1,3–2,3. ECFR/Bruegel dorównują w
microsites/interaktywnych raportach (5,0), ale tracker z SSR i dokończone huby ekspertów odsunęły i to
(+2,0). **Wyszukiwarka, sieć, czat i profil czytelnika pozostają poza ich zasięgiem.**
...
przegrywa już żadnego modułu z żadnym think-tankiem; remis z CSIS jest zasłużony po ich stronie
(ChinaPower, Missile Threat to microsites klasy światowej), a po naszej wisi na brakach trackera
(import, RSS, działające alerty — wciąż sprzedawane w cenniku bez implementacji). RAND ma najlepszą
wyszukiwarkę think-tankową świata (5,6) — wciąż −2,7 do NES. **Kluczowe bez zmian: oni budują to
studiami deweloperów, nie self-service** — a paywall (≤2,0), sieć (~1,0) i czat (0) mają śladowe.
...
czytanie (−0,8), storytelling (Bloomberg Graphics, −2,0), SEO (Reuters, −1,5), newsletter (już tylko
−0,5 po unifikacji suppression i ożywieniu runnera), paywall (−0,7) i konwersję (−1,2). Rewizja 30.07
pokazała, że te różnice były **większe niż deklarowały starsze audyty** — naprawa martwych ścieżek
(runner, suppression, scheduler push) odzyskała większość tej straty. **Ale:** media mają wyszukiwarkę
o połowę słabszą (max FT 4,8), profil czytelnika szczątkowy (3,7), a czat i sieć **zerowe**. To firmy
z setkami inżynierów robiące jedną rzecz świetnie — NES robi 20 rzeczy w jednym systemie.
...
89 widgetów), realtime-infra, analityka BI, CRM, multi-tenant, backend/RLS — żaden think-tank ani serwis
...
Po 31.07 ta warstwa dostała też to, czego jej brakowało od strony procesu: działający pgTAP, trzy
statyczne gate'y SQL i obserwowalny harmonogram zadań tła.

**2. Gdzie NES jest bezkonkurencyjny obserwowalnie:** czat (0 u wszystkich 38 konkurentów), sieć/profil
użytkownika (max 3,7 vs 7,8–8,0), wyszukiwarka (max RAND 5,6 / FT 4,8 vs 8,3). To **przewagi kategorialne**,
...
- **Czytanie i SEO** — globalne media (9,0) mają przewagę szlifu, którego serwis 20-modułowy nie dogoni
  bez dedykowanego zespołu redakcyjnego (−0,8 / −1,5).
- **Newsletter i konwersja** — 30.07 przegrywały przez **martwe ścieżki** (runner, suppression,
  community-cron); po ich naprawie do FT zostało odpowiednio **−0,5 i −1,2** — to już dystans szlifu
  (open/click dedup, segmentacja self-service, checkout bez wybicia do `/profile/billing`), nie
  dystans zepsutej infrastruktury.
...
kategorię, UE o rosnący margines, a najlepsze TT USA o 3–4 punkty, **nie przegrywając już żadnego modułu**
(30.07: jeden, z CSIS). Realną poprzeczką pozostają **globalne media** (FT/Bloomberg/Reuters) — biją NES
w treści, SEO i storytellingu, ale ich przewaga w newsletterze/paywallu/konwersji stopniała do ułamków
punktu, a warstwy społecznościowo-sieciowo-wyszukiwarkowej nie mają wcale. Przewidywanie z wydania 30.07
(„po zamknięciu listy P0 NES wraca w okolice 8,0 i odzyskuje deklarowany dystans") **zrealizowało się w
36 godzin**: średnia ~7,8, kompozyt 7,5, wszystkie P0/P1 domknięte poza CASCADE faktur. Do pełnych 8,0
brakuje domknięcia P2: wydmuszek UI (checkout_settings, autozapis, alerty trackera, odznaki), bundla,
e2e SSR i zatrzymania dryfu formatowania na main.
