...
# Naprawy wprowadzone - runda 6 (2026-07-13): funkcje platformy społeczności

Zakres: siedem obszarów oznaczonych 🟡 w audycie funkcji platformy (katalog osób,
profile, powiadomienia, komentarze, newsletter, formaty, metryki). Wszystkie
zmiany na gałęzi `claude/platform-features-audit-60103k`.

| Obszar                 | Co wdrożono                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Zamknięta luka                                                                                 |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| Katalog osób `/people` | Weryfikacja zawodowa (`profiles.verified_at/verified_by`, RPC `admin_set_profile_verification` + trigger guard przeciw samoweryfikacji), odznaka BadgeCheck na karcie osoby, stronie autora i w adminie użytkownika; `search_people` zwraca `verified` i przyjmuje `p_job_title`/`p_verified_only`; czwarta faseta `job_title`; etykieta fasety firmy → „Instytucja / organizacja"                                                                                                                                                            | brak weryfikacji, odznak i filtrów po instytucji/roli                                          |
| Profile członków       | Dokończona unifikacja bio: widget BIO autora i edytor `/profile/author` przełączone na kanoniczne `profiles.bio_pl/bio_en` (fallback legacy `author_profiles` tylko do odczytu), backfill migracją, współdzielona reguła `preferCanonicalBio` z testami                                                                                                                                                                                                                                                                                       | ostatnie zdesynchronizowane miejsce edycji bio                                                 |
| Powiadomienia          | Kanał Web Push: `push_subscriptions` (own-row RLS) + `push_outbox` z triggerem, doręczanie tickiem (claim SKIP LOCKED, backoff, dead-letter, dezaktywacja endpointów 404/410), `public/sw.js`, hook `usePushSubscription`, przełącznik per urządzenie; e-mail digest opt-in (off/daily/weekly, `claim_due_digest_users`, puste digesty nie wychodzą). Wymaga env `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` (`generateVapidKeys()` z `src/lib/notifications/webpush.server.ts` - pakiet npm `web-push` usunięto 2026-07-25 jako martwą zależność) | kanał wyłącznie in-app                                                                         |
| Komentarze             | Paginacja wątków zamiast okna płaskiej listy (odpowiedzi nigdy nie gubią rodzica na granicy strony), uczciwy licznik COUNT w nagłówku, stabilny tiebreaker id                                                                                                                                                                                                                                                                                                                                                                                 | gubione odpowiedzi + kłamiący licznik (rate-limit/edycja/realtime istniały już po rundach 4-5) |
| Newsletter             | Wysyłka porcjami (≤200 e-maili/wywołanie) z dzierżawą `lease_until` (natychmiastowe wznowienie zamiast heurystyki 20 min), picker „Zaplanuj wysyłkę" w edytorze (dotąd scheduled_at nieustawialne z UI), automatyczny tick pg_cron+pg_net → `POST /api/public/jobs-tick` (sekret w `job_runner_settings`, service-role-only), karta konfiguracji automatu w adminie, pętla porcji i „Wznów wysyłkę" w UI                                                                                                                                      | wysyłka synchroniczna w jednym requeście + martwe planowanie                                   |
| Formaty                | Podcast: `<enclosure>` z realnym `length`/`type` z biblioteki mediów; web stories: równoległy dokument `<amp-story>` pod `/web-stories/$slug/amp` + `rel=amphtml` (kwalifikacja do prezentacji Web Stories Google); live blog: publiczny indeks `/live` (plakietka LIVE, licznik wpisów, SSR, sitemap) - redakcja ma linkowalny adres relacji                                                                                                                                                                                                 | enclosure length=0, brak AMP, relacje bez publicznego URL                                      |
| Metryki                | Dashboard `/admin/audience`: lejek członka (rejestracje → opt-in → aktywni → płacący), aktywność dzienna (silnik Chart), retencja kohortowa tygodniowa; RPC `admin_member_funnel` / `admin_member_retention` / `admin_member_activity_series` z guardem admina tenanta                                                                                                                                                                                                                                                                        | brak dashboardu retencji/lejka                                                                 |

Infrastruktura wspólna rundy: generyczny runner zadań tła (pg_cron + pg_net →
`/api/public/jobs-tick`) obsługuje newsletter, push i digesty; bez pg_net
wszystko degraduje się do opportunistic ticku. Współdzielony
`sendTransactionalEmail` (docelowy jeden nadawca Resend). Naprawione zastane
błędy CI z merge'a rundy czatu (cast `chat_check_upload_quota`, prettier x3).

## Bramki weryfikacyjne rundy 6

- `tsc --noEmit`: 0 błędów
- `bun run lint`: 0 błędów (baseline ostrzeżeń bez zmian)
- `bun run test`: 177 plików / 1570 testów zaliczonych (w tym nowe: canonicalBio,
  podcastRss, ampStory, liveBlogs, pushDigest)
- `bun run build`: zielony (drzewo tras z `/live`, `/web-stories/$slug/amp`,
  `/api/public/jobs-tick`, `/admin/audience`)
- pgTAP: +4 zestawy (people_verification, job_runner_security, push_and_digest,
  member_analytics)

## Ocena po rundzie 6 (szacunek)

| Obszar                                | Po rundzie 5 | Po rundzie 6 |
| ------------------------------------- | ------------ | ------------ |
| Czat / powiadomienia                  | ~8,8         | **~9,0**     |
| Newsletter / monetyzacja              | ~7,8         | **~8,4**     |
| Profil / katalog osób                 | ~8,2         | **~8,6**     |
| Formaty treści (podcast/stories/live) | ~8,3         | **~8,6**     |
| Metryki / analityka                   | -            | **~8,0**     |
| **Platforma ogółem**                  | **~8,2**     | **~8,5**     |

Pozostałe świadome rezerwy: MFA/TOTP, realtime komentarzy oczekuje na
regenerację typów Supabase (casty `as never` przy nowych kolumnach/RPC),
konfiguracja VAPID i `job_runner_settings.base_url` to jednorazowe kroki
operacyjne po wdrożeniu.
