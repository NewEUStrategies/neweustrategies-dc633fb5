...
### MODUŁ 7 — Typy treści specjalne · linie 16,47% → **70,83%** · funkcje 14,60% → **71,19%**

> **ZAKTUALIZOWANE 18.08.2026** (branch `claude/module-7-test-coverage-jbe28y`). Liczby w tabeli
> poniżej to STAN Z AUDYTU, zachowany jako punkt odniesienia. Stan po pracy testowej i pełne
> uzasadnienie każdego przypadku: `docs/WDROZENIE_MODUL7_TESTY_2026-08-18.md`.
>
> Skrót: warstwa `lib/*` modułu jest domknięta (93–100% linii w każdym katalogu), biblioteka
> plików zeszła z absolutnego zera na 99,15%/100%, `tracker/queries.ts` i `tracker/feed.server.ts`
> z 0% na 100%. Poza pracą zostały trzy duże pliki interfejsu (`ExpertLayoutRenderer` 1133 linie,
> `ExpertLayoutInlineEditor` 555, `ExpertMaterialsExplorer` 314) oraz `SpeakerProfileDialog` (254),
> `PolicyPositionsMap` (217) i `QuizBackground` (171) - to one trzymają moduł poniżej 95%.
...
- ~~**MODUŁ 7 — Typy treści specjalne**: linie 16,47%, funkcje 14,60%, plików 0%: 75/109~~ → **70,83% / 71,19%** (18.08.2026, `WDROZENIE_MODUL7_TESTY_2026-08-18.md`)
