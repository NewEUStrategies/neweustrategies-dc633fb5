...
### 9.2 Pliki testowe wyłączone z pomiaru — PRZYCZYNA I NAPRAWA

**Stan pierwotny (rano 2026-08-18):** 39 plików / 464 testów / 950 asercji poza pomiarem.
Wszystkie oprócz jednego dotyczyły DWÓCH powierzchni MODUŁU 3:
`components/admin/builder/**` (panele właściwości, w tym bramka `settingsFidelity.gate`
z 537 testami) oraz `components/builder/organisms/widget-view/**` (render widgetów).

**Diagnoza pierwotna była błędna.** Zakładała „najcięższe importy w repo na czterordzeniowym
sandboksie z zależnościami z publicznego npm zamiast pinów z `bun.lock`". Sprawdzone
i odrzucone — oba założenia:
