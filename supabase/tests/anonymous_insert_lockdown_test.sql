-- pgTAP: zamknięcie publicznych INSERT-ów (migracja 20260730130000).
--
-- Bramka anty-regresyjna dla 3 findings ze skanu anonimowych INSERT-ów
...
-- personalizacja/analityka). Zapisy przechodzą wyłącznie przez utwardzone
-- ścieżki serwerowe (submitContactMessage, /api/public/related-click,
-- triggery SECURITY DEFINER) działające jako service_role/owner.
--
--   1. Granty: anon i authenticated NIE mają INSERT do żadnej z 3 tabel;
--      anon stracił też martwy SELECT na crm_consent_log; staffowy SELECT
--      (authenticated) i pełnia uprawnień service_role zostają.
--   2. Polityki: żadna z 3 tabel nie ma już ŻADNEJ polityki INSERT.
--   3. Zachowanie: INSERT z sesji anon oraz authenticated kończy się 42501
--      na każdej z tabel (nawet z poprawnym tenant_id publicznego wpisu).
