...
-- katalog osobowy tenanta B po podmianie hosta.
--
...
-- Zaakceptowany kontakt viewer <-> contact (galaz czlonkowska).
INSERT INTO public.user_connections (tenant_id, requester_id, addressee_id, status)
SELECT (SELECT home_tenant FROM ppg_ctx),
...
-- ── (17-19) Podmiana x-tenant-host nie otwiera obcego katalogu ──────────────
SELECT set_config('request.jwt.claim.sub', 'c0000000-0000-0000-0000-0000000000b7', true);
SELECT set_config('request.jwt.claims',
  '{"sub":"c0000000-0000-0000-0000-0000000000b7","role":"authenticated"}', true);
...
SELECT is(
  ...
  0,
  'user tenanta A na domenie B NIE czyta discoverable czlonka B (izolacja obszarow)'
);
...
SELECT is(
  ...
  1,
  'na domenie B widac wylacznie to, co B publikuje publicznie (autor wpisu)'
);
...
SELECT is(
  (SELECT count(*)::int FROM public.profiles_public
    WHERE id = 'c0000000-0000-0000-0000-0000000000b2'),
  1,
  'warstwa czlonkowska idzie za tenantem DOMOWYM, nie za naglowkiem'
);

SELECT set_config('request.headers', '', true);
RESET ROLE;

-- ── (20-24) get_my_public_exposure: uczciwy stan dla panelu prywatnosci ─────
SET LOCAL ROLE authenticated;
SELECT set_config('request.jwt.claim.sub', 'c0000000-0000-0000-0000-0000000000b7', true);
SELECT set_config('request.jwt.claims',
  '{"sub":"c0000000-0000-0000-0000-0000000000b7","role":"authenticated"}', true);
