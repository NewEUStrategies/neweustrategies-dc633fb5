...
RETURNS TABLE (
  total_members bigint, members_blocked bigint, members_warning bigint, members_safe bigint,
  total_anon bigint, anon_blocked bigint, avg_used numeric, max_used integer, total_views bigint
)
...
BEGIN
  ...
  WITH members AS (
    SELECT user_id, COUNT(*)::int AS used
      FROM public.metered_views
     WHERE tenant_id = v_tenant AND period_month = v_month AND user_id IS NOT NULL
     GROUP BY user_id
  ),
  anon AS (
    SELECT visitor_id, COUNT(*)::int AS used
     ...
     WHERE tenant_id = v_tenant AND period_month = v_month
       AND user_id IS NULL AND visitor_id IS NOT NULL
     GROUP BY visitor_id
  )
  SELECT
...
RETURNS TABLE (used integer, monthly_limit integer, remaining integer, period_month date)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_tenant uuid := public.current_tenant_id();
  v_limit integer;
...
BEGIN
  IF _user_id <> auth.uid()
    ...
    RAISE EXCEPTION 'forbidden';
  END IF;
  SELECT ms.member_monthly_limit INTO v_limit
    FROM public.metering_settings ms WHERE ms.tenant_id = v_tenant;
  v_limit := COALESCE(v_limit, 5);
  SELECT count(*)::int INTO v_used
    FROM public.metered_views mv
   WHERE mv.tenant_id = v_tenant AND mv.user_id = _user_id AND mv.period_month = v_period;
  v_used := COALESCE(v_used, 0);
  used := v_used; monthly_limit := v_limit; remaining := GREATEST(v_limit - v_used, 0); period_month := v_period;
  RETURN NEXT;
END; $$;

-- 4) bulk_generate_coupons_for_campaign
...
DECLARE
  ...
  _uid UUID := auth.uid();
  _i INTEGER := 0; _created INTEGER := 0;
  _code TEXT;
...
BEGIN
  ...
  IF _c.tenant_id <> current_tenant_id() THEN RAISE EXCEPTION 'wrong_tenant'; END IF;
...
BEGIN
  ...
  END IF;
  UPDATE public.organization_seats os SET last_invited_at = now() WHERE os.id = p_seat;
  RETURN QUERY SELECT v_seat.id, v_seat.invited_email, v_org.name, now()::timestamptz;
END $$;

-- 7) publish_qa_session_summary
...
DECLARE
  ...
  v_q record;
  v_n integer := 0;
  v_body_pl text := ''; v_body_en text := '';
  v_author_pl text; v_author_en text;
  v_slug text; v_post_id uuid; v_parent_page uuid;
...
BEGIN
  ...
  IF NOT v_staff AND v_session.host_user_id <> v_user THEN RAISE EXCEPTION 'qa: not allowed'; END IF;
  IF p_publish AND NOT public.can_publish_content(v_user) THEN
    RAISE EXCEPTION 'qa: publish requires editorial role';
  END IF;
  IF v_session.status NOT IN ('answering', 'closed') THEN RAISE EXCEPTION 'qa: session not summarizable'; END IF;
  IF COALESCE(btrim(v_session.intro_pl), '') <> '' THEN v_body_pl := public.qa_text_to_html(v_session.intro_pl); END IF;
  IF COALESCE(btrim(v_session.intro_en), '') <> '' THEN v_body_en := public.qa_text_to_html(v_session.intro_en); END IF;
  FOR v_q IN
  ...
  LOOP
    v_n := v_n + 1;
    v_author_pl := CASE WHEN v_q.is_anonymous OR COALESCE(btrim(v_q.author_display), '') = '' THEN 'Anonimowo' ELSE public.qa_escape_html(v_q.author_display) END;
    v_author_en := CASE WHEN v_q.is_anonymous OR COALESCE(btrim(v_q.author_display), '') = '' THEN 'Anonymous' ELSE public.qa_escape_html(v_q.author_display) END;
    v_body_pl := v_body_pl || '<h3>Pytanie ' || v_n || '</h3><blockquote>' || public.qa_text_to_html(v_q.body) || '<p><cite>- ' || v_author_pl || '</cite></p></blockquote>' || public.qa_text_to_html(v_q.answer_body);
    v_body_en := v_body_en || '<h3>Question ' || v_n || '</h3><blockquote>' || public.qa_text_to_html(v_q.body) || '<p><cite>- ' || v_author_en || '</cite></p></blockquote>' || public.qa_text_to_html(v_q.answer_body);
  ...
  IF v_post_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.posts p WHERE p.id = v_post_id) THEN v_post_id := NULL; END IF;
  IF v_post_id IS NULL THEN
    SELECT p.id INTO v_post_id FROM public.posts p WHERE p.tenant_id = v_session.tenant_id AND p.slug = v_slug;
  END IF;
  IF v_post_id IS NULL THEN
    SELECT pg.id INTO v_parent_page FROM public.pages pg
     WHERE pg.tenant_id = v_session.tenant_id AND pg.slug = 'blog' AND pg.parent_id IS NULL AND pg.deleted_at IS NULL
     ORDER BY pg.created_at ASC LIMIT 1;
    IF v_parent_page IS NULL THEN
      SELECT pg.id INTO v_parent_page FROM public.pages pg
       WHERE pg.tenant_id = v_session.tenant_id AND pg.parent_id IS NULL AND pg.deleted_at IS NULL
       ORDER BY pg.created_at ASC LIMIT 1;
    END IF;
    IF v_parent_page IS NULL THEN RAISE EXCEPTION 'qa: no parent page for summary post'; END IF;
    INSERT INTO public.posts (
      ...
      v_session.tenant_id, v_slug, v_parent_page, COALESCE(v_session.host_user_id, v_user),
      ...
      v_body_pl, v_body_en, CASE WHEN p_publish THEN now() END
    ) RETURNING id INTO v_post_id;
  ELSE
    SELECT p.status = 'published' INTO v_was_published FROM public.posts p WHERE p.id = v_post_id;
    UPDATE public.posts
       SET title_pl = 'Q&A: ' || v_session.title_pl || ' - podsumowanie',
           ...
           status = CASE WHEN p_publish OR v_was_published THEN 'published'::public.post_status ELSE status END,
           published_at = CASE WHEN p_publish OR v_was_published THEN COALESCE(published_at, now()) ELSE published_at END,
           deleted_at = NULL, updated_at = now()
     WHERE id = v_post_id;
  END IF;
  UPDATE public.qa_sessions SET post_id = v_post_id, updated_at = now() WHERE id = p_session_id;
  SELECT p.status INTO v_status FROM public.posts p WHERE p.id = v_post_id;
  IF p_publish AND NOT v_was_published THEN
    PERFORM public.enqueue_notification(
      u.user_id, 'content',
      'Podsumowanie sesji Q&A jest już dostępne', 'The Q&A session recap is now available',
      v_session.title_pl, v_session.title_en, '/post/' || v_slug, 'BookOpenCheck'
    ) FROM (SELECT DISTINCT unnest(v_notified) AS user_id) u WHERE u.user_id IS NOT NULL;
  END IF;
  RETURN jsonb_build_object('post_id', v_post_id, 'slug', v_slug, 'status', v_status, 'questions', v_n);
END; $$;

-- 8) get_event_access
CREATE OR REPLACE FUNCTION public.get_event_access(p_event_id uuid)
RETURNS TABLE (can_join boolean, join_url text, can_watch boolean, recording_url text, reason text, watch_reason text)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_user uuid := auth.uid();
  v_event public.events%ROWTYPE;
  v_staff boolean := false;
  v_allowed boolean; v_can_watch boolean; v_rsvp text;
BEGIN
  SELECT * INTO v_event FROM public.events
   WHERE id = p_event_id AND tenant_id = public.public_tenant_id();
  IF NOT FOUND OR v_event.status <> 'published' THEN
    RETURN QUERY SELECT false, NULL::text, false, NULL::text, 'not_found', 'not_found'; RETURN;
  END IF;
  IF v_user IS NULL THEN
    RETURN QUERY SELECT false, NULL::text, false, NULL::text, 'auth_required',
      CASE WHEN v_event.recording_url IS NULL THEN 'none' ELSE 'auth_required' END; RETURN;
  END IF;
  v_staff := v_event.tenant_id = public.current_tenant_id()
         AND (public.has_role(v_user, 'admin'::app_role) OR public.has_role(v_user, 'editor'::app_role));
  ...
  ELSIF v_event.visibility = 'members' AND v_event.kind = 'briefing' THEN v_allowed := public.has_tier_feature('pro_briefings');
  ELSIF v_event.visibility = 'members' THEN v_allowed := public.has_tier_rank(GREATEST(COALESCE(v_event.min_tier_rank, 0), 1));
  ELSE v_allowed := public.has_tier_rank(COALESCE(v_event.min_tier_rank, 0)); END IF;
  IF NOT v_allowed THEN
    RETURN QUERY SELECT false, NULL::text, false, NULL::text, 'tier_required',
      CASE WHEN v_event.recording_url IS NULL THEN 'none' ELSE 'tier_required' END; RETURN;
  END IF;
  v_can_watch := v_event.recording_url IS NOT NULL AND (v_staff OR public.has_tier_feature('recordings'));
  RETURN QUERY SELECT
    ...
    CASE WHEN v_event.recording_url IS NULL THEN 'none'
         ...
         ELSE 'tier_required' END;
END; $$;

-- 9) authorize_resource_download
