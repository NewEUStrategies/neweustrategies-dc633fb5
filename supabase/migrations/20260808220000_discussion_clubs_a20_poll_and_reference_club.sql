-- ============================================================================
-- Kluby dyskusyjne - etap A20: domkniecie rodzaju 'poll' + klub referencyjny
--
-- CZESC 1: RODZAJ 'poll' BYL ETYKIETA, NIE FUNKCJA.
--
-- `club_threads.kind` dopuszcza 'poll' od A3, specyfikacja (V1 par. 1.3) mowi
-- wprost, ze ten rodzaj "reuzywa istniejace polls / poll_votes" - a w modelu
-- nie bylo ZADNEJ krawedzi miedzy watkiem a ankieta. Wybor "sondaz" w
-- kompozytorze zmienial wiec chip nad tytulem i nic wiecej: watek dostawal
-- etykiete obiecujaca glosowanie, ktorego nie dalo sie ani utworzyc, ani
-- oddac. To jest ta sama klasa dlugu, co kotwica przed A18 - model danych bez
-- producenta i bez konsumenta.
--
-- Domykamy ja najtansza droga, jaka istnieje: JEDNA kolumna `poll_id`
-- wskazujaca na `polls`, ktore ma juz wlasne glosowanie, wlasne RLS i wlasny
-- widget. Zero nowej mechaniki glosowania w module klubow - i to jest cel,
-- bo drugi silnik ankiet obok istniejacego rozjechalby sie z nim w pierwszym
-- tygodniu.
--
-- CZESC 2: KLUB REFERENCYJNY.
--
-- Modul ma dwadziescia funkcji i zero miejsc, w ktorych widac je razem.
-- Ten seed tworzy JEDEN kompletny klub, ktory uzywa kazdego mechanizmu
-- wdrozonego w A1-A19: pieciu grup o roznych ustawieniach dziedziczenia
-- (w tym grupy z premoderacja, grupy pod regula Chatham House i grupy
-- z harmonogramem), watkow WSZYSTKICH szesciu rodzajow, odpowiedzi na obu
-- dozwolonych poziomach zagniezdzenia, kotwicy w tresci platformy oraz
-- ankiety wpietej w rodzaj 'poll'.
--
-- CZEGO TU NIE MA I DLACZEGO. Zadnych sztucznych licznikow, zadnych
-- wymyslonych czlonkow, zadnych reakcji "na oko". Liczniki utrzymuja triggery
-- i maja pokazywac PRAWDE o tym, co w klubie jest; czlonkostwa wymagaja
-- prawdziwych kont, a reakcje - prawdziwych ludzi. Poprzednie podejscie tego
-- typu zostalo z repozytorium usuniete swiadomie (commit "Usunieto sztuczny
-- seed statystyk") i ta migracja tego nie odwraca: sieje STRUKTURE i TRESC
-- REDAKCYJNA, nie metryki.
--
-- IDEMPOTENCJA. Cala migracja jest bezpieczna przy powtorzeniu: klub jest
-- rozpoznawany po (tenant_id, slug), a kazdy wpis potomny po swoim slugu
-- w obrebie klubu. Ponowne odtworzenie historii migracji nie duplikuje ani
-- jednego wiersza i nie nadpisuje tresci, ktora redakcja mogla poprawic.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) Krawedz watek -> ankieta
-- ----------------------------------------------------------------------------
...
-- Ankieta ma sens WYLACZNIE w watku, ktory jest sondazem. Bez tego warunku
-- kolumna byla by kolejnym polem, ktore "czasem cos znaczy".
ALTER TABLE public.club_threads
...
-- Widok watku musi oddac identyfikator ankiety, inaczej klient nie ma jak
-- narysowac glosowania. Zmiana typu zwracanego, wiec DROP + CREATE.
...
AS $$
  SELECT
    ...
    attr.mode,
    -- Ankieta wychodzi tylko dla watku, ktory jest sondazem - CHECK powyzej
    -- i tak tego pilnuje, ale projekcja nie ma powodu obiecywac wiecej.
...
COMMENT ON FUNCTION public.club_thread_view(uuid, text) IS
  'Widok watku. Warstwa projekcji reguly Chatham House. posted_by_admin_name wychodzi TAKZE w trybie chatham - to informacja o tym, kto WPROWADZIL tresc, nie kto ja napisal. poll_id wychodzi wylacznie dla kind=poll.';
...
-- Zalozenie sondazu z panelu: jedna transakcja tworzy ankiete I watek, ktory
-- ja niesie. Rozdzielenie tego na dwa wywolania zostawialoby przy kazdym
-- bledzie sierote - ankiete bez watku albo watek 'poll' bez ankiety.
...
BEGIN
  ...
  -- Slug z tytulu, z sufiksem przy kolizji - ta sama regula, co
  -- w club_create_thread (A9). Polskie znaki przez unaccent, zeby
  -- "Rozporzadzenie" nie zamienilo sie w ciag myslnikow.
...
COMMENT ON FUNCTION public.admin_club_poll_create(uuid, text, text, text, text, jsonb, timestamptz, uuid) IS
  'Zaklada sondaz klubowy: ankieta w polls + watek kind=poll w JEDNEJ transakcji. Rozdzielenie zostawialoby przy bledzie sierote po jednej ze stron.';
...
-- 2) Klub referencyjny
--
-- Temat nie jest przypadkowy. Klub ma byc czytelny dla redakcji NES i pokrywac
-- obszar, w ktorym platforma ma realne dossier w trackerze - inaczej kotwica
-- (jedyna funkcja, ktorej nie da sie pokazac bez tresci obok) nie mialaby
-- w co trafic.
-- ----------------------------------------------------------------------------
DO $seed$
DECLARE
...
BEGIN
  ...
  -- Wlascicielem jest pierwszy administrator tenantu. Gdy takiego nie ma
  -- (swieza instalacja), klub powstaje BEZ autora - kolumny sa nullowalne
  -- wlasnie dlatego, ze tresc ma przezyc usuniecie konta (V1 par. 7).
  ...
  INSERT INTO public.clubs (
    ...
    'members',
    'request',
    -- `anonymous_allowed`, nie `chatham`: klub ma dzialac pod nazwiskiem, ale
    -- wypowiedz o sprawie wrazliwej ma miec droge wyjscia. Grupa "Kuluary"
    -- nizej nadpisuje to na pelna regule Chatham House.
    'anonymous_allowed',
    'members',
    -- `trusted`: premoderacja wylacznie ponizej progu reputacji. Twarde `pre`
    -- w klubie eksperckim zamienia moderatora w waskie gardlo.
    'trusted',
    'security',
  ...
  -- ---- Grupy ---------------------------------------------------------------
  -- Piec grup, kazda z INNYM ustawieniem dziedziczenia - to jest ta czesc
  -- modelu (V2 par. 2), ktorej nie widac, dopoki wszystkie grupy sa takie same.
  ...
  -- Kotwica: bierzemy REALNY akt prawny tenantu z obszaru bezpieczenstwa.
  -- Gdy tracker jest pusty, watek powstaje bez kotwicy - lepiej bez niej niz
  -- z krawedzia wskazujaca w nicosc.
  ...
  -- ---- Watki: KAZDY z szesciu rodzajow --------------------------------------

  -- 1) ANNOUNCEMENT - przypiete, od prowadzacego. Otwiera klub.
  INSERT INTO public.club_threads (
    tenant_id, club_id, group_id, author_id, posted_by_admin_id,
  ...
  -- 2) DISCUSSION - zwykla debata, dwa poziomy odpowiedzi.
  INSERT INTO public.club_threads (
    tenant_id, club_id, group_id, author_id, posted_by_admin_id,
  ...
  IF v_thread IS NOT NULL
    ...
    -- Odpowiedz na poziomie 1 (drzewo jest przyciete do 2 - trigger pilnuje).
    INSERT INTO public.club_replies (
      tenant_id, club_id, thread_id, parent_id, author_id, posted_by_admin_id, body, status
    ) VALUES (
  ...
  -- 3) QUESTION - z odpowiedzia rozstrzygajaca.
  INSERT INTO public.club_threads (
    tenant_id, club_id, group_id, author_id, posted_by_admin_id,
    slug, title, body, kind, status
  ) VALUES (
  ...
  IF v_thread IS NOT NULL
    ...
    -- Rozstrzygniecie: to jest cala roznica miedzy 'question' a 'discussion'.
    UPDATE public.club_threads
       SET resolved_reply_id = v_reply, status = 'resolved'
     WHERE id = v_thread;
  END IF;

  -- 4) POSITION - z mapa stanowisk (glosy oddaja czlonkowie, nie seed).
  INSERT INTO public.club_threads (
    tenant_id, club_id, group_id, author_id, posted_by_admin_id,
    slug, title, body, kind, status
  ) VALUES (
  ...
  -- 5) RESOURCE - w bibliotece z premoderacja.
  INSERT INTO public.club_threads (
    tenant_id, club_id, group_id, author_id, posted_by_admin_id,
    slug, title, body, kind, status
  ) VALUES (
  ...
  -- 6) POLL - z realna ankieta w `polls`.
  IF NOT EXISTS (
  ...
  -- 7) Watek ZAKOTWICZONY w akcie prawnym - tylko gdy jest w co trafic.
  ...
  -- 8) Watek pod regula Chatham House - w grupie, ktora ja nadpisuje.
  INSERT INTO public.club_threads (
    tenant_id, club_id, group_id, author_id, posted_by_admin_id,
    slug, title, body, kind, status
  ) VALUES (
