-- =============================================================================
-- "Udostepnij pelny artykul" (Share full article) - bramka rejestracji +
-- budzet klikniec na link.
--
-- ZMIANA MECHANIKI (produktowa):
--   1) Kto moze udostepnic: dotad WYLACZNIE platny subskrybent
--      (can_gift_articles). Teraz decyduje ustawienie tenanta `eligibility`:
--        'registered'  - kazda ZAREJESTROWANA osoba tego tenanta (domyslne),
--        'subscribers' - dotychczasowe zachowanie (aktywna subskrypcja / warstwa
--                        z premium_content).
--      Czlonkostwo jest sprawdzane po tenancie DOMOWYM konta
--      (current_tenant_id() = public_tenant_id()), wiec konto z innego tenanta
--      nie generuje linkow do tresci tego serwisu mimo podrobionego naglowka
--      x-tenant-host.
--   2) Ile osob przeczyta z jednego linku: budzet klikniec (domyslnie 5)
--      jest ZAMRAZANY na linku w chwili utworzenia (post_gift_links
--      .max_redemptions), a nie czytany z biezacych ustawien. Obietnica zlozona
--      odbiorcom ("link dziala dla 5 osob") nie zmienia sie pod nimi, gdy admin
--      pozniej przestawi suwak.
--   3) Slot budzetu zuzywa ODBIORCA, nie odswiezenie strony: rejestr
--      post_gift_redemptions deduplikuje po (link, tozsamosc odbiorcy), wiec
--      powrot do artykulu z tego samego urzadzenia/konta nie pali kolejnego
--      klikniecia. Darczynca i czytelnik z wlasnym uprawnieniem (subskrypcja /
--      zakup / miejsce w organizacji) nie zuzywaja budzetu w ogole.
--   4) Limit miesieczny liczy ARTYKULY, nie wiersze linkow
--      (count(DISTINCT post_id) w okresie, takze cofniete). Rotacja linku dla
--      tego samego wpisu nie konsumuje limitu drugi raz, a cofniecie linku
--      go nie zwraca - petla "cofnij i wygeneruj" przestaje byc darmowym
--      zrodlem nowych budzetow. Dodatkowo nowy link dla wpisu juz udostepnionego
--      w tym okresie DZIEDZICZY zuzycie poprzedniego (przeniesienie licznika),
--      wiec rotacja po wygasnieciu nie resetuje budzetu klikniec.
--
-- Bezpieczenstwo pozostaje serwerowe: klient nigdy nie widzi body inaczej niz
-- przez SECURITY DEFINER redeem_gift_link, a tresc chroniona haslem nigdy nie
-- wychodzi linkiem (sekret autora).
-- =============================================================================

-- -----------------------------------------------------------------------------
-- (1) Ustawienia tenanta: kto moze udostepniac + domyslny budzet klikniec.
-- -----------------------------------------------------------------------------
...
-- Domyslny budzet 50 pochodzil z utwardzenia 20260724090600 (cap "zeby nie bylo
-- nieskonczonosci"). Mechanika produktowa mowi teraz "do 5 klikniec".
ALTER TABLE public.gift_article_settings
  ALTER COLUMN max_redemptions_per_link SET DEFAULT 5;

-- Istniejacy tenant: przestaw wylacznie wiersze, ktore zostaly przy STARYM
-- domyslnym (50). Swiadomie ustawiona inna wartosc zostaje nietknieta.
UPDATE public.gift_article_settings SET max_redemptions_per_link = 5
 WHERE max_redemptions_per_link = 50;

-- Bramka rejestracji jest intencja wdrozenia, nie tylko domyslna dla nowych
-- tenantow - istniejace wiersze przechodza na 'registered'.
...
-- Backfill: istniejace linki dostaja efektywny budzet swojego tenanta (po
-- zmianie wyzej - domyslnie 5). Bez tego stare wiersze mialyby 0 = bez limitu,
-- czyli dokladnie ta luke, ktora ta migracja zamyka.
...
-- -----------------------------------------------------------------------------
-- (3) Rejestr odbiorcow - deduplikacja slotow i material analityczny.
--
-- PRYWATNOSC: tozsamosc odbiorcy NIE jest ujawniana darczyncy. Sharer widzi
-- wylacznie zagregowany licznik z RPC; wiersze czyta tylko redakcja (RLS).
-- recipient_key dla anonima to pseudonim (uuid goscia z localStorage), ten sam,
-- ktorym posluguje sie metering - nie jest to identyfikator osoby.
-- -----------------------------------------------------------------------------
...
-- Zapis idzie WYLACZNIE przez SECURITY DEFINER redeem_gift_link - zadna
-- polityka INSERT dla roli klienta (patrz bramka check:sql-anon-insert).
...
-- -----------------------------------------------------------------------------
-- (4) Audyt: nowy typ zdarzenia "exhausted" (odbicie od wyczerpanego budzetu).
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- (5) Uprawnienie do udostepniania.
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Czlonkostwo w tenancie PRZEGLADANEGO serwisu liczone z tenanta DOMOWEGO
  -- konta (profiles.tenant_id), nie z naglowka - naglowek jest do podrobienia.
...
-- -----------------------------------------------------------------------------
-- (6) Stan dla popovera - dochodzi budzet klikniec i tryb uprawnienia.
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  IF NOT public.can_share_full_article() THEN
    -- 'subscribers': brak subskrypcji -> CTA planow. 'registered': konto z
    -- innego tenanta -> traktujemy jak brak uprawnienia do logowania w tym
    -- serwisie (requires_auth), bo plan niczego tu nie zmieni.
    RETURN QUERY SELECT
      true,
      false,
  ...
  -- Limit miesieczny liczy ARTYKULY udostepnione w okresie (takze przez linki
  -- cofniete/wygasle) - patrz naglowek migracji.
...
-- -----------------------------------------------------------------------------
-- (7) Utworzenie linku - bramka `eligibility`, zamrozony budzet, przeniesienie
--     licznika przy rotacji, limit miesieczny liczony po artykulach.
-- -----------------------------------------------------------------------------
...
DECLARE
  ...
  v_settings public.gift_article_settings%ROWTYPE;
  v_enabled boolean := true;
  v_limit integer := 10;   -- bezpieczne fallbacki (brak wiersza ustawien)
  v_ttl integer := 30;
  v_cap integer := 5;
  v_used integer := 0;
  v_carry integer := 0;
...
BEGIN
  ...
  -- Funkcja dotyczy tresci ZA PAYWALLEM. Link do wpisu publicznego niczego nie
  -- odblokowuje, wiec zamiast wydawac martwy kod odmawiamy wprost; tresc na
  -- haslo pozostaje wykluczona (sekret autora nie chodzi linkiem).
  IF NOT EXISTS (
  ...
  -- samego darczyncy (atomowy limit miesieczny).
  ...
  -- Zywy link: get-or-create jest idempotentne (ponowne udostepnienie tego
  -- samego artykulu nie konsumuje limitu i nie resetuje budzetu klikniec).
  ...
  -- Limit miesieczny sprawdzamy dopiero dla NOWEGO artykulu: wpis juz
  -- udostepniony w tym okresie jest w v_used, wiec rotacja go nie blokuje.
  IF v_limit > 0
     AND v_used >= v_limit
     AND NOT EXISTS (
  ...
  -- Przeniesienie zuzycia: rotacja linku dla tego samego artykulu w tym samym
  -- okresie nie daje swiezych 5 klikniec.
...
-- -----------------------------------------------------------------------------
-- (8) Realizacja linku - budzet klikniec z dedupem odbiorcy + powod odmowy.
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Tresc na haslo nigdy nie wychodzi linkiem podarunkowym.
  IF EXISTS (
    SELECT 1 FROM public.content_access ca
     WHERE ca.entity_type = 'post'
       AND ca.entity_id = _post_id
  ...
  IF v_uid IS NOT DISTINCT FROM v_link.created_by THEN
    -- Darczynca zawsze widzi swoj artykul i nigdy nie pali slotu.
    v_reason := 'owner';
  ELSIF public.has_content_access('post'::public.access_entity_type, _post_id) THEN
    -- Czytelnik z wlasnym uprawnieniem tez nie marnuje slotu odbiorcy.
    v_reason := 'entitled';
  ELSE
    v_key := CASE
    ...
    -- Serializacja budzetu: rownolegle wejscia nie moga przeskoczyc capu.
...
-- -----------------------------------------------------------------------------
-- (9) Panel redakcji: budzet per link + unikalni odbiorcy.
-- -----------------------------------------------------------------------------
