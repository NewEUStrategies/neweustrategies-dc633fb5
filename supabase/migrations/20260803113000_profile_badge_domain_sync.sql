-- ============================================================================
-- ODNAKI PROFILOWE: jeden kontrakt, bezpieczne mutacje, zdarzenia domenowe
-- i automatyczne nadawanie za reputację.
--
-- Zasady:
--   * dozwolone są dokładnie: verified, expert, staff, contributor;
--   * klient nigdy nie podaje tenant_id przy mutacji administracyjnej;
--   * baza odrzuca każdą relację badge -> profil z innego tenantu;
--   * INSERT i DELETE przechodzą wyłącznie przez RPC admina lub funkcje
--     systemowe SECURITY DEFINER;
--   * grant i revoke emitują wersjonowane zdarzenia domenowe;
--   * tylko contributor może zostać nadany z reputacji: 150 punktów
--     aktywności w ruchomym oknie 90 dni, bez punktów z samych odznak.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. Pochodzenie nadania i indeksy operacyjne.
-- ----------------------------------------------------------------------------
...
-- Twarda ochrona przed relacją cross-tenant. Obowiązuje także service_role
-- i emitery systemowe, więc błędna integracja nie może obejść izolacji RLS.
...
-- Nadanie ręczne dostaje aktora z sesji. Nadanie automatyczne pozostaje bez
-- fałszywego granted_by, ale nadal wysyła użytkownikowi powiadomienie.
...
-- ----------------------------------------------------------------------------
-- 2. Zdarzenia domenowe dla nadania i odebrania.
-- Aktor = odbiorca odznaki. Dzięki temu użytkownik widzi własne zdarzenie
-- przez RLS i odświeża profil, a staff widzi je przez politykę tenantową.
-- Payload nie zawiera notatki administracyjnej.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3. Granica bezpieczeństwa odczytu i zapisu.
-- Publicznie dostępne są wyłącznie pola prezentacyjne. Notatka, źródło i
-- identyfikator nadającego są dostępne tylko przez kontrolowane RPC admina.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4. Automatyczna odznaka contributor z aktywności reputacyjnej.
-- Punkty odznak są celowo wyłączone, więc nadanie nie podnosi samo własnej
-- podstawy kwalifikacji. Funkcja pracuje dla jednego użytkownika i tenantu.
-- ----------------------------------------------------------------------------
...
-- Jeden emiter obsługuje wszystkie źródła aktywności. Każde wywołanie liczy
-- wyłącznie wynik jednego użytkownika, a UNIQUE zapewnia idempotencję wyścigów.
...
-- Akceptacja materiału zapisuje źródło już w INSERT odznaki. Dzięki temu
-- zdarzenie AFTER INSERT niesie prawidłowe contributor_submission, a nie
-- przejściowe manual. Pozostała logika recenzji zachowuje dotychczasowy
-- kontrakt powiadomień.
...
-- Zbiorcza ścieżka naprawcza dla aktywności czasowej (np. wydarzenie, które
-- właśnie się rozpoczęło) i importów. Wywołuje ją community-cron.
