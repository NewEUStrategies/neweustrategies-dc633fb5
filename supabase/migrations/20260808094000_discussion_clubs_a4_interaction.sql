-- ============================================================================
-- DISCUSSION CLUB - ETAP A4: INTERAKCJA
--
-- Reakcje semantyczne, stanowiska, subskrypcje, wzmianki i KOMPLET producentow
-- powiadomien.
--
-- REAKCJE TO DANE, NIE EKSPRESJA (V1 §4.2):
-- To jest swiadome odejscie od wzorca czatu, gdzie message_reactions.emoji
-- jest dowolnym stringiem. W czacie reakcja to ekspresja; w klubie
-- dyskusyjnym reakcja to DANE. Dowolne emoji daja szum nieprzydatny ani do
-- rankingu, ani do reputacji, ani do mapy stanowisk. Stad slownik ZAMKNIETY:
--   grupa JAKOSC     - insightful, evidence, question, thanks (niezalezne)
--   grupa STANOWISKO - agree, disagree (WYKLUCZAJACE sie wzajemnie)
-- Rozlacznosc grupy "stanowisko" egzekwuje TRIGGER, nie klient: sam
-- UNIQUE (target, user, kind) pozwolilby na agree I disagree naraz.
--
-- UWAGA WIAZACA Z AUDYTEM (§7): platforma ma piec funkcji, ktore dzialaja,
-- ale sa CICHE - rezerwacja slotu, prosba o wprowadzenie, rekomendacja,
-- potwierdzenie umiejetnosci i odwiedziny profilu nie wstawialy wiersza do
-- `notifications`. Discussion Club wchodzi z KOMPLETEM producentow od
-- pierwszego dnia, zeby nie powtorzyc tego wzorca.
--
-- Nowy rodzaj powiadomienia wymaga TRZECH zmian, inaczej enqueue_notification
-- cicho zwroci NULL (V1 §6.4):
--   1. notifications_kind_check - dopisac 'club',
--   2. notification_preferences.enabled_club,
--   3. galaz WHEN 'club' w enqueue_notification.
-- Robimy wszystkie trzy.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) Reakcje - slownik ZAMKNIETY
-- ----------------------------------------------------------------------------
...
COMMENT ON TABLE public.club_reactions IS
  'Reakcje semantyczne. Slownik ZAMKNIETY, bo w klubie reakcja to dane (ranking, reputacja, mapa stanowisk), a nie ekspresja jak emoji w czacie.';

-- ----------------------------------------------------------------------------
-- 2) Stanowiska (tematy kind='position')
-- ----------------------------------------------------------------------------
...
COMMENT ON TABLE public.club_stances IS
  'Stanowiska w temacie kind=position. Jedno na osobe i temat - zmiana zdania nadpisuje, nie dopisuje.';

-- ----------------------------------------------------------------------------
-- 3) Subskrypcje watkow (nadpisuja notify_level klubu)
-- ----------------------------------------------------------------------------
...
COMMENT ON TABLE public.club_thread_subscriptions IS
  'Subskrypcja watku. Nadpisuje notify_level klubu w OBIE strony: mozna sledzic watek w wyciszonym klubie i wyciszyc watek w sledzonym.';
...
-- ----------------------------------------------------------------------------
-- 5) Rozlacznosc grupy "stanowisko"
--
-- UNIQUE (target, user, kind) sam z siebie pozwala tej samej osobie postawic
-- `agree` I `disagree` na tym samym wpisie. Trigger to blokuje - i robi to
-- przez PODMIANE, nie przez blad: klikniecie "nie zgadzam sie" po wczesniejszym
-- "zgadzam sie" jest zmiana zdania, a nie pomylka do zgloszenia.
-- ----------------------------------------------------------------------------
...
-- Licznik reakcji na celu plus przeliczenie rankingu watku. Ranking liczy
-- WYLACZNIE reakcje jakosciowe - agree/disagree sa z niego celowo wylaczone.
...
BEGIN
  ...
  ELSE
    ...
    -- Reakcja na odpowiedzi tez zmienia ranking watku: jakosc dyskusji to
    -- takze jakosc odpowiedzi, nie tylko posta otwierajacego.
...
-- ----------------------------------------------------------------------------
-- 6) Powiadomienia: rodzaj 'club'
--
-- Trzy zmiany wymagane przez kontrakt enqueue_notification (V1 §6.4).
-- Bez KTOREJKOLWIEK z nich funkcja cicho zwroci NULL, a producent bedzie
-- wygladal na dzialajacy.
-- ----------------------------------------------------------------------------
...
-- Galaz preferencji. Odtwarzamy CALA funkcje, tak jak robi to kazda migracja
-- dokladajaca rodzaj powiadomienia - operowanie stringiem na
-- pg_get_functiondef() bylo by krucha sztuczka, ktora psuje sie po pierwszej
-- zmianie formatowania.
...
BEGIN
  ...
  -- Deduplikacja 5-minutowa: dwie odpowiedzi w tym samym watku w ciagu minuty
  -- to jeden sygnal, nie dwa. Ta reguła istniala juz wczesniej; nie ruszamy jej.
...
-- ----------------------------------------------------------------------------
-- 7) Producenci powiadomien - KOMPLET od pierwszego dnia
--
-- Wspolny helper, bo kazdy producent robi to samo: sprawdza, czy odbiorca to
-- nie autor zdarzenia, i wola enqueue_notification z linkiem do watku.
-- ----------------------------------------------------------------------------
...
AS $$
BEGIN
  -- Nikt nie chce powiadomienia o wlasnym dzialaniu.
...
COMMENT ON FUNCTION public.club_notify(uuid, uuid, text, text, text, text, text) IS
  'Wspolny producent powiadomien modulu. Odsiewa powiadomienie o wlasnym dzialaniu; preferencje odbiorcy respektuje enqueue_notification.';
...
-- Producent: nowa odpowiedz -> autor watku + subskrybenci.
...
BEGIN
  IF NEW.status <> 'visible' THEN
    RETURN NULL;   -- wpis w kolejce premoderacji jeszcze nie istnieje dla innych
  END IF;
  ...
  -- 2. Subskrybenci watku (bez autora watku - juz dostal, i bez wyciszonych).
  FOR v_rec IN
...
-- Producent: reakcja `insightful` na moim wpisie. PROGOWANA - powiadomienie
-- po KAZDEJ reakcji zamienia sie w szum, po ktorym uzytkownik wylacza caly
-- kanal. Progi 1, 5, 25 daja trzy sygnaly zamiast dwudziestu pieciu.
...
-- Producent: zaproszenie do klubu.
...
-- Producent: przyjecie do klubu (pending -> active).
...
-- ----------------------------------------------------------------------------
-- 8) RPC: reakcje, stanowiska, subskrypcje
-- ----------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.club_react(text, uuid, text) IS
  'Dodaje reakcje ze slownika zamknietego. Rozlacznosc agree/disagree egzekwuje trigger przez PODMIANE - zmiana zdania nie jest bledem.';
...
-- Odczyt WSADOWY dla calej widocznej partii - nigdy N+1, dokladnie jak
-- useBadgesForUsers i useConnectionStatuses na /people.
...
AS $$
  ...
  WHERE r.target_type = p_target_type
    ...
    -- Widocznosc liczy club_capabilities dla klubu wiersza; bez tego reakcje
    -- z klubu secret wyplywalyby przez sam licznik.
...
COMMENT ON FUNCTION public.club_reactions_for(text, uuid[]) IS
  'Reakcje dla partii celow jednym zapytaniem - nigdy N+1. Respektuje widocznosc klubu, wiec licznik nie wycieka z klubu secret.';
...
COMMENT ON FUNCTION public.club_stance_summary(uuid) IS
  'Mapa stanowisk watku. Zwraca liczby, nie osoby - kto jak zaglosowal, nie jest informacja publiczna.';
...
-- Autor watku jest subskrybowany z automatu - inaczej nie dostalby
-- powiadomienia o odpowiedzi na wlasny temat po tym, jak ktos inny
-- zasubskrybowal, a on nie.
...
-- Kto odpowiedzial, ten sledzi - dopoki jawnie nie wyciszy.
