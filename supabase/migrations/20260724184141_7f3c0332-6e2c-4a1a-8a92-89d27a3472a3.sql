...
RETURNS TABLE(
  ...
  cover_image_url text, published_at timestamptz, author_id uuid,
  author_name text, author_avatar text, author_slug text
)
LANGUAGE sql STABLE SET search_path = public AS $$
  SELECT p.id, p.slug, p.title_pl, p.title_en, p.excerpt_pl, p.excerpt_en,
         p.cover_image_url, p.published_at, p.author_id,
         pp.display_name, pp.avatar_url, pp.slug
    FROM public.posts p
