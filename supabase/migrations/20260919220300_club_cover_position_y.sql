-- Kadrowanie okładki klubu w pionie: `clubs.cover_position_y` i komplet RPC,
-- które tę kolumnę zwracają lub zapisują (bliźniak pasa drizzle
-- `0028_club_cover_position_y`).
--
-- PO CO BLIŹNIAK. `src/lib/clubs/coverPosition.functions.ts` woła RPC
-- `club_set_cover_position`, a pas supabase nie zna ani tej funkcji, ani
-- kolumny. W przeglądarce to PGRST202 „function not found" w KAŻDYM
-- środowisku odtworzonym z `supabase/migrations/` - CI e2e, staging, nowa baza
-- - i dokładnie to zgłasza `bun run check:rpc-contract`. Pozostałe pięć funkcji
-- jedzie razem, bo `club_view`, `club_list`, `club_list_by_specialization`,
-- `club_activity_feed` i `club_update_settings` mają w sygnaturze nową kolumnę;
-- rozdzielenie ich zostawiłoby pas supabase z widokiem bez `cover_position_y`.
--
-- DLACZEGO `DROP FUNCTION` PRZED `CREATE OR REPLACE`. Wszystkie pięć zmienia
-- listę kolumn w `RETURNS TABLE`, a tego `CREATE OR REPLACE` nie potrafi
-- (42P13 „cannot change return type of existing function"). Sygnatury
-- argumentów zostają te same, więc `DROP FUNCTION IF EXISTS` trafia dokładnie
-- w istniejącą funkcję i nie zostawia sieroty.
--
-- POWTÓRNE WYKONANIE JEST BEZPIECZNE: `ADD COLUMN IF NOT EXISTS` oraz
