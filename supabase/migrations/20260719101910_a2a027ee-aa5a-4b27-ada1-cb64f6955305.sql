...
COMMENT ON COLUMN public.categories.logo_url IS
  'URL logotypu organizacji (kind = organization); wyświetlany w wynikach wyszukiwania.';
...
COMMENT ON FUNCTION public.search_people_orgs(text, int) IS
  'Sekcja "Osoby i organizacje" wyszukiwarki: autorzy/eksperci z publicznym dorobkiem + organizacje (z logo).';
