-- Ustawienia rejestracji wydarzenia + przelaczniki modulow.
--
-- DWA BRAKI Z JEDNEJ LISTY, ZAMYKANE RAZEM.
--
-- 1) USTAWIENIA REJESTRACJI byly edytowalne WYLACZNIE w starym dialogu
--    `/admin/community/events`. Studio pytalo o tytul, termin i miejsce, a tryb
--    zapisow, limit miejsc, prog warstwy, cene wejsciowki i adres transmisji
--    stalyby sie nieosiagalne z nowej powierzchni - czyli dwa formularze na te
--    same kolumny, z ktorych jeden nie widzi polowy pol. To jest dokladnie ten
--    dlug, ktory studio mialo zlikwidowac.
--
-- 2) ADRES TRANSMISJI I NAGRANIA jest odciety od klienta grantem kolumnowym
--    (`20260803191905`), wiec redaktor nie moze go ODCZYTAC zwyklym zapytaniem -
--    tylko zapisac na slepo. `admin_event_detail` stoi za `assert_editor_tenant`,
--    wiec to jest jedyne miejsce, w ktorym te dwa adresy moga wrocic na ekran
--    bez rozszerzania grantu dla wszystkich zalogowanych.
--
-- 3) PRZELACZNIKI MODULOW (`events.features`) mowia, KTORYCH MODULOW UZYWA TO
--    WYDARZENIE. Webinar nie ma gieldy spotkan ani odprawy na miejscu, a
--    kongres ma jedno i drugie - i sidebar studia nie ma powodu pokazywac obu
--    w obu przypadkach.
--
--    TO NIE JEST WIDOCZNOSC PUBLICZNA. Ta ma juz wlasne zrodlo prawdy
--    (`event_page_sections` + `event_sections`), a drugi przelacznik na te sama
--    rzecz znaczy dwa miejsca, w ktorych mozna ja wylaczyc, i tylko jedno,
--    ktore ktos pamieta. Przelacznik modulu chowa sekcje w PANELU; tego, co
--    widzi uczestnik, nie dotyka.

-- ---------------------------------------------------------------------------
-- 1. Odczyt: adres transmisji i nagrania wracaja na ekran redaktora
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  SELECT
    ...
    -- Adresy WRACAJA, ale wylacznie tutaj: funkcja stoi za asercja roli
    -- redaktora w tenancie domowym, a grant kolumnowy dla anon i authenticated
    -- zostaje nietkniety.
...
-- ---------------------------------------------------------------------------
-- 2. Zapis ustawien rejestracji i dostepu
-- ---------------------------------------------------------------------------
--
-- ROZSZERZAMY ISTNIEJACY KONTRAKT, nie dodajemy drugiego RPC na te sama tabele.
-- `admin_event_general_save` juz ma regule „klucz nieobecny = pole nietkniete",
-- wiec ekran ustawien rejestracji wysyla swoje dziesiec pol i nie rusza tytulu
-- ani adresu publicznego. Drugi RPC znaczylby dwie listy walidacji na te same
-- warunki bazy.
...
BEGIN
  ...
  IF v_reg_mode = 'external' THEN
    ...
    END IF;
    IF v_external !~* '^https://[^[:space:]]+$' OR char_length(v_external) > 2048 THEN
      RAISE EXCEPTION 'external_url_invalid: url must start with https and be under 2048 chars';
    END IF;
  ELSE
    -- Adres zostaje ZAPISANY, ale przestaje obowiazywac. Zerowanie go przy
    -- kazdej zmianie trybu kasowaloby prace redaktora, ktory tylko sprawdzal,
    -- jak wyglada inny tryb.
    NULL;
  END IF;
  ...
  IF v_capacity IS NOT NULL AND v_capacity < 0 THEN
    RAISE EXCEPTION 'invalid_capacity: capacity cannot be negative';
  END IF;
  ...
  IF v_currency !~ '^[A-Z]{3}$' THEN
    RAISE EXCEPTION 'invalid_currency: currency must be a three-letter code';
  END IF;
...
-- ---------------------------------------------------------------------------
-- 3. Przelaczniki modulow wydarzenia
-- ---------------------------------------------------------------------------
--
-- ZBIOR KLUCZY JEST ZAMKNIETY. `features` to jsonb, wiec bez bialej listy
-- ktokolwiek z rola redaktora moglby wstrzyknac dowolny klucz, a ekran
-- pokazywalby przelacznik, ktorego nikt nie obsluguje. Klucz nieobecny =
-- modul WLACZONY: nowy modul nie ma znikac wydarzeniom zapisanym przed jego
-- powstaniem.
...
BEGIN
  ...
  FOREACH v_key IN ARRAY v_keys LOOP
    ...
    -- wyslac jeden przelacznik i nie wlaczyc przy okazji szesciu pozostalych.
