...
COMMENT ON COLUMN public.event_people.email_visible IS
  'Decyzja OSOBY: czy jej e-mail widza inni uczestnicy wydarzenia. Domyslnie false - milczenie znaczy NIE.';
COMMENT ON COLUMN public.event_people.phone_visible IS
  'Decyzja OSOBY: czy jej telefon widza inni uczestnicy wydarzenia. Domyslnie false.';
COMMENT ON COLUMN public.event_people.social_links IS
  'Slownik {linkedin,x,facebook,instagram,youtube,website} -> URL http(s). Walidacja w event_my_event_profile_set.';

ALTER TABLE public.event_people
  DROP CONSTRAINT IF EXISTS event_people_social_links_object;
ALTER TABLE public.event_people
  ADD CONSTRAINT event_people_social_links_object
  ...
  IF v_blocked IS NULL THEN
    WITH listable AS (
      SELECT
        ...
        CASE WHEN pr.hide_avatar THEN NULL ELSE COALESCE(pe.photo_url, pr.avatar_url) END AS avatar_url,
        pr.slug AS profile_slug,
        CASE WHEN pe.email_visible THEN COALESCE(NULLIF(btrim(pe.email), ''), NULLIF(btrim(pr.contact_email), '')) END AS email,
        CASE WHEN pe.phone_visible THEN COALESCE(NULLIF(btrim(pe.phone), ''), NULLIF(btrim(pr.phone), '')) END AS phone,
        NULLIF(btrim(COALESCE(pe.industry, '')), '') AS industry,
        COALESCE(NULLIF(btrim(pe.specialization), ''), NULLIF(btrim(pr.specialization), '')) AS specialization,
        COALESCE(NULLIF(btrim(pe.seeking_pl), ''), NULLIF(btrim(pr.seeking_pl), '')) AS seeking_pl,
        COALESCE(NULLIF(btrim(pe.seeking_en), ''), NULLIF(btrim(pr.seeking_en), '')) AS seeking_en,
        COALESCE(NULLIF(btrim(pe.offering_pl), ''), NULLIF(btrim(pr.offering_pl), '')) AS offering_pl,
        COALESCE(NULLIF(btrim(pe.offering_en), ''), NULLIF(btrim(pr.offering_en), '')) AS offering_en,
        COALESCE(pe.social_links, '{}'::jsonb) AS social_links,
        COALESCE(NULLIF(btrim(pe.bio_pl), ''), NULLIF(btrim(pr.bio_pl), '')) AS bio_pl,
        COALESCE(NULLIF(btrim(pe.bio_en), ''), NULLIF(btrim(pr.bio_en), '')) AS bio_en
      FROM public.event_registrations r
      JOIN public.event_people pe
        ON pe.id = r.person_id AND pe.tenant_id = r.tenant_id
        ...
        (
          SELECT jsonb_agg(
            jsonb_build_object(
              ...
              'seeking_pl', pg.seeking_pl,
              'seeking_en', pg.seeking_en,
              'offering_pl', pg.offering_pl,
              'offering_en', pg.offering_en,
              'social_links', pg.social_links,
              'bio_pl', pg.bio_pl,
              'bio_en', pg.bio_en,
              'groups', (
...
COMMENT ON FUNCTION public.event_attendees(jsonb) IS
  'Katalog uczestnikow opublikowanego wydarzenia dla ZAPISANEGO wolajacego. Kontakt (e-mail/telefon) wychodzi wylacznie, gdy wlasciciel wlaczyl event_people.email_visible / phone_visible.';
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'profile', CASE WHEN v_person.id IS NULL THEN NULL ELSE jsonb_build_object(
      ...
      'email_visible', v_person.email_visible,
      'phone_visible', v_person.phone_visible,
      'job_title', v_person.job_title,
      'company_text', v_person.company_text,
      'industry', v_person.industry,
      'specialization', v_person.specialization,
      'seeking_pl', v_person.seeking_pl,
      'seeking_en', v_person.seeking_en,
      'offering_pl', v_person.offering_pl,
      'offering_en', v_person.offering_en,
    ...
    'account', CASE WHEN v_acc.id IS NULL THEN NULL ELSE jsonb_build_object(
      'first_name', v_acc.first_name,
      'last_name', v_acc.last_name,
      'email', COALESCE(v_acc.contact_email, v_acc.email),
      'phone', v_acc.phone,
      'job_title', v_acc.job_title,
      'company_text', v_acc.current_company,
      'specialization', v_acc.specialization,
      'seeking_pl', v_acc.seeking_pl,
      'seeking_en', v_acc.seeking_en,
      'offering_pl', v_acc.offering_pl,
      'offering_en', v_acc.offering_en,
      ...
      'social_links', jsonb_strip_nulls(jsonb_build_object(
        'linkedin', v_acc.linkedin_url,
        'x', v_acc.twitter_url,
        'facebook', v_acc.facebook_url,
        'instagram', v_acc.instagram_url,
...
CREATE OR REPLACE FUNCTION public.event_my_event_profile_sync_account(p_payload jsonb DEFAULT '{}'::jsonb)
RETURNS jsonb
LANGUAGE plpgsql
VOLATILE SECURITY DEFINER
SET search_path TO 'public', 'pg_temp'
...
BEGIN
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'auth_required: sign in to sync your event profile';
  END IF;
  IF v_tenant IS NULL THEN
    RAISE EXCEPTION 'invalid_tenant: unknown host';
  END IF;
  ...
  IF v_acc.id IS NULL THEN
    RAISE EXCEPTION 'no_account_profile: platform profile is missing';
  END IF;

  UPDATE public.event_people p SET
    first_name = COALESCE(NULLIF(btrim(COALESCE(v_acc.first_name, '')), ''), p.first_name),
    last_name = COALESCE(NULLIF(btrim(COALESCE(v_acc.last_name, '')), ''), p.last_name),
    email = COALESCE(NULLIF(btrim(COALESCE(v_acc.contact_email, v_acc.email, '')), ''), p.email),
    phone = COALESCE(NULLIF(btrim(COALESCE(v_acc.phone, '')), ''), p.phone),
    job_title = COALESCE(NULLIF(btrim(COALESCE(v_acc.job_title, '')), ''), p.job_title),
    company_text = COALESCE(NULLIF(btrim(COALESCE(v_acc.current_company, '')), ''), p.company_text),
    specialization = COALESCE(NULLIF(btrim(COALESCE(v_acc.specialization, '')), ''), p.specialization),
    seeking_pl = COALESCE(NULLIF(btrim(COALESCE(v_acc.seeking_pl, '')), ''), p.seeking_pl),
    seeking_en = COALESCE(NULLIF(btrim(COALESCE(v_acc.seeking_en, '')), ''), p.seeking_en),
    offering_pl = COALESCE(NULLIF(btrim(COALESCE(v_acc.offering_pl, '')), ''), p.offering_pl),
    offering_en = COALESCE(NULLIF(btrim(COALESCE(v_acc.offering_en, '')), ''), p.offering_en),
    bio_pl = COALESCE(NULLIF(btrim(COALESCE(v_acc.bio_pl, v_acc.bio, '')), ''), p.bio_pl),
    bio_en = COALESCE(NULLIF(btrim(COALESCE(v_acc.bio_en, '')), ''), p.bio_en),
    photo_url = CASE WHEN v_acc.hide_avatar THEN p.photo_url
                     ELSE COALESCE(NULLIF(btrim(COALESCE(v_acc.avatar_url, '')), ''), p.photo_url) END,
    social_links = COALESCE(p.social_links, '{}'::jsonb) || jsonb_strip_nulls(jsonb_build_object(
      'linkedin', NULLIF(btrim(COALESCE(v_acc.linkedin_url, '')), ''),
      'x', NULLIF(btrim(COALESCE(v_acc.twitter_url, '')), ''),
      'facebook', NULLIF(btrim(COALESCE(v_acc.facebook_url, '')), ''),
      'instagram', NULLIF(btrim(COALESCE(v_acc.instagram_url, '')), ''),
      'website', NULLIF(btrim(COALESCE(v_acc.website_url, '')), '')
    )),
    updated_at = now()
  WHERE p.id = v_person_id AND p.tenant_id = v_tenant AND p.user_id = v_uid;
...
REVOKE ALL ON FUNCTION public.event_my_event_profile_sync_account(jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.event_my_event_profile_sync_account(jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.event_my_event_profile_sync_account(jsonb) TO service_role;