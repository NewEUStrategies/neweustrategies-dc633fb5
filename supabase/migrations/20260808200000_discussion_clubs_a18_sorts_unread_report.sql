-- ============================================================================
-- Kluby dyskusyjne - etap A18: sorty, nieprzeczytane, zgloszenia, kotwice
--
-- Audyt wdrozenia A1-A17 znalazl cztery luki, w ktorych KLIENT deklaruje
-- funkcje, a BAZA jej nie ma. Kazda z nich jest osobnym rodzajem dlugu:
--
--   1. SORTY, KTORYCH NIE MA. `CLUB_THREAD_SORTS` w typach klienta wymienia
--      hot / new / unanswered / top / mine, a `club_threads_list` zna dwa
--      porzadki. Warstwa API mapowala wszystko poza 'new' na 'hot', wiec trzy
--      sorty byly martwa litera - w tym `unanswered`, o ktorym specyfikacja
--      (V1 par. 5.2) pisze wprost, ze ma byc wyeksponowany, bo watek bez
--      odpowiedzi to porazka klubu, a nie neutralny stan. Dokladamy tez
--      `subscribed`: bez niego subskrypcja watku jest przelacznikiem, ktory
--      wlacza powiadomienia i NIC wiecej - nie da sie z niej zrobic listy.
--
--   2. SORT ODPOWIEDZI 'stance'. V1 par. 4.4 wymienia trzy porzadki
--      (chronological / best / stance); wdrozone byly dwa. Grupowanie wg
--      stanowiska to jedyny widok, ktory pokazuje MAPE SPORU zamiast kolejnosci
--      wpisywania - a to jest cala roznica miedzy deliberacja a forum.
--      Wymaga nowej kolumny w projekcji, wiec funkcja jest zdejmowana i
--      tworzona od nowa (zmiana typu zwracanego; ten sam wzorzec, co A14).
--
--   3. LICZNIK NIEPRZECZYTANYCH. V1 par. 6.3 zapowiada
--      `user_pending_counters.club_unread` utrzymywany triggerem. Nie powstal,
--      wiec plakietka klubow w nawigacji nie mialaby z czego zyc, a
--      `club_members.last_read_at` bylo kolumna, ktorej nikt nie zapisuje.
--
--      Model: licznik na CZLONKOSTWIE (`club_members.unread_count`), a licznik
--      globalny to jego suma. Dwa powody, dla ktorych nie liczymy COUNT(*):
--      lista klubow renderuje sie na kazdym wejsciu do huba, a "oznacz jako
--      przeczytane" musi umiec odjac DOKLADNIE tyle, ile dolozyl jeden klub -
--      z jednego skalara globalnego nie da sie tego wyliczyc.
--
--      Bump idzie JEDNA instrukcja na wpis (CTE + INSERT ... ON CONFLICT), a
--      nie triggerem wierszowym na `club_members`: klub o pieciuset czlonkach
--      nie moze placic pieciuset odpaleniami triggera za jedna odpowiedz.
--
--   4. ZGLOSZENIA. V1 par. 7 stawia sprawe jednoznacznie: wejscie "Zglos" ma
--      byc przy kazdym wpisie OD PIERWSZEGO DNIA (czat nie ma go od szesciu
--      wydan i to jest przestroga, nie wzorzec). Istniejacy `report_user`
--      przyjmuje IDENTYFIKATOR AUTORA - a pod regula Chatham House klient go
--      nie dostaje i dostac nie moze. Dlatego zgloszenie klubowe wskazuje
--      TRESC, a autora rozwiazuje baza. To nie jest obejscie anonimowosci:
--      to jedyny ksztalt, w ktorym zgloszenie i anonimowosc moga wspolistniec.
--
-- Piata pozycja to `club_anchor_suggest`: kotwiczenie watku w akcie prawnym /
-- wpisie / wydarzeniu jest w modelu danych od A1, w szwie `cross_references`
-- od A12 i na stronie aktu prawnego od A12 - ale NIE MA ZADNEJ SCIEZKI, ktora
-- pozwalalaby kotwice ustawic. Kompozytor nie mial czego zapytac o podpowiedzi,
-- wiec `ClubAnchorThreads` na stronie aktu prawnego z definicji swiecil pustka.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 0) Indeksy pod nowe sorty
--
-- `unanswered` i `mine` to filtry, nie tylko porzadki - bez indeksu czesciowego
-- kazdy z nich czytalby cala liste watkow klubu, zeby odrzucic wiekszosc.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 1) club_threads_list: szesc realnych sortow
--
-- Kursor jest tekstem `przypiecie|klucz sortu|id` porownywanym malejaco, wiec
-- KLUCZ MUSI ODPOWIADAC PORZADKOWI - inaczej paginacja gubi i duplikuje
-- wiersze. Kazdy sort dostaje wiec wlasny czlon srodkowy o STALEJ szerokosci
-- (padding zerami), bo porownanie jest leksykalne, nie liczbowe.
--
-- Przypiecie podbija wylacznie porzadki CZYTELNICZE (hot / new). W `mine`
-- i `subscribed` przypiete ogloszenie prowadzacego nie ma czego robic na
-- gorze - to sa listy odpowiadajace na pytanie "gdzie ja jestem", a nie
-- "co klub uwaza za wazne".
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 2) club_replies_list: porzadek 'stance' i stanowisko autora w projekcji
--
-- Stanowisko autora wychodzi WYLACZNIE tam, gdzie wychodzi jego nazwisko.
-- W klubie pod regula Chatham House (albo we wpisie anonimowym) kolumna jest
-- pusta: para (alias, stanowisko) powtarzana przez kilka watkow jest sladem
-- korelacyjnym, a caly sens osolenia aliasu per watek polega na tym, zeby
-- takiego sladu nie bylo.
--
-- Zmiana typu zwracanego, wiec CREATE OR REPLACE nie wystarczy. DROP stoi
-- w tym samym pliku, co CREATE - bramka odtwarzalnosci uznaje to za legalne.
-- ----------------------------------------------------------------------------
...
AS $$
  ...
  rows AS (
    SELECT
      ...
      -- Stanowisko ma sens wylacznie w watku typu 'position' i wylacznie przy
      -- autorstwie jawnym (patrz naglowek sekcji).
    ...
    WHERE r.thread_id = p_thread_id
      AND cap.can_read
      -- Status WATKU, nie tylko odpowiedzi. club_moderate('thread', ..., 'hide')
      -- zmienia wylacznie club_threads.status i nie dotyka odpowiedzi, wiec bez
      -- tego warunku ukrycie watku zabieralo naglowek, a tresc zostawiala
      -- czytelna kazdemu czlonkowi - dokladnie odwrotnie, niz brzmi ta akcja.
  ...
  ORDER BY
    ...
    -- 'stance' grupuje: najpierw poparcie, potem sprzeciw, potem wstrzymanie,
    -- na koncu glosy bez zadeklarowanego stanowiska. To jest MAPA SPORU -
    -- czytelnik widzi dwa obozy, a nie kolejnosc wpisywania.
...
-- ----------------------------------------------------------------------------
-- 3) Nieprzeczytane: licznik na czlonkostwie + suma w user_pending_counters
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Jedna instrukcja na wpis: aktualizacja czlonkostw i licznikow globalnych
  -- w tej samej CTE. Wariant z triggerem wierszowym na club_members kosztowalby
  -- N odpalen na jedna odpowiedz.
  WITH bumped AS (
    UPDATE public.club_members m
     ...
     WHERE m.club_id = p_club_id
       ...
       -- Wyciszony klub nie zapala plakietki. Powiadomienia i licznik musza
       -- mowic to samo, inaczej "nic mi nie przyszlo" kloci sie z czerwona
       -- kropka w nawigacji.
...
AS $$
BEGIN
  -- Wpis w kolejce premoderacji nie jest jeszcze niczyja lektura.
  IF NEW.status = 'open' THEN
    PERFORM public.club_bump_unread(NEW.club_id, COALESCE(NEW.author_id, NEW.posted_by_admin_id));
  END IF;
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- Awaria licznika nie moze wywrocic publikacji watku - ta sama doktryna,
  -- co w szwach A12.
  RETURN NEW;
END;
$$;
...
-- ----------------------------------------------------------------------------
-- 4) Zgloszenie tresci klubowej
--
-- Wskazujemy TRESC, nie osobe: pod regula Chatham House klient nie zna autora,
-- a zgloszenie ma dzialac tak samo w kazdym trybie atrybucji. Autor jest
-- rozwiazywany po stronie bazy i nigdy nie wraca do wolajacego.
--
-- Zgloszenie ma DWA adresatow, bo ma dwie funkcje: `user_reports` to kolejka
-- staffu tenantu (ten sam wzorzec, co w sieci kontaktow), a
-- `club_moderation_log` to dziennik widoczny dla PROWADZACYCH KLUBU - klub,
-- ktory nie widzi zgloszen na wlasnym podworku, nie moze nim zarzadzac.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Zglaszac moze wylacznie ktos, kto ma prawo TO PRZECZYTAC. Bez tego
  -- zgloszenie bylo by wyrocznia istnienia tresci w klubie zamknietym.
  SELECT * INTO v_caps FROM public.club_capabilities(v_club, v_group, v_uid);
  IF NOT COALESCE(v_caps.can_read, false) THEN
    RAISE EXCEPTION 'clubs: forbidden' USING ERRCODE = '42501';
  END IF;

  -- Zgloszenie wlasnego wpisu nie jest zgloszeniem.
  IF v_author IS NOT NULL AND v_author = v_uid THEN
    RAISE EXCEPTION 'clubs: invalid target' USING ERRCODE = '22023';
  END IF;

  -- Kolejka staffu. Wpis osierocony (autor usunal konto) nie ma kogo zglosic,
  -- ale nadal ma byc widoczny dla prowadzacych - dlatego dziennik nizej jest
  -- bezwarunkowy, a ten INSERT nie.
...
-- Dziennik moderacji dopuszcza teraz akcje 'report'. Lista jest przepisana
-- w calosci z A10 - ograniczenie CHECK nie ma skladni "dodaj wartosc", wiec
-- kazde rozszerzenie musi powtorzyc stan poprzedni, inaczej cicho zabiera
-- akcje, ktore juz dzialaja.
...
-- ----------------------------------------------------------------------------
-- 5) Podpowiedzi kotwic dla kompozytora
--
-- Kotwica jest w modelu od A1 i w grafie powiazan od A12, ale nie bylo zadnej
-- sciezki, ktora pozwalalaby ja USTAWIC - wiec karta "dyskutowane w klubach"
-- na stronie aktu prawnego z definicji swiecila pustka. To jest ta funkcja.
--
-- Zwracamy wylacznie tresc PUBLICZNA tenantu wolajacego: kotwica jest linkiem,
-- ktory zobaczy kazdy czytelnik watku, wiec nie moze wskazywac na szkic.
-- ----------------------------------------------------------------------------
  ...
  (
    SELECT 'event'::text, e.id::text, e.title_pl, e.title_en, e.slug
     ...
     WHERE q.len >= 2
       ...
       -- Wydarzenie dostepne wylacznie dla warstwy platnej nadal moze byc
       -- kotwica: kotwica jest LINKIEM, a bramka warstwy siedzi na stronie
       -- wydarzenia, nie w podpowiedzi.
...
COMMENT ON FUNCTION public.club_anchor_suggest(text, text, integer) IS
  'Podpowiedzi kotwicy dla kompozytora watku: akty prawne, opublikowane wpisy i wydarzenia tenantu wolajacego. Bez tego kolumna anchor_type byla martwa, a karta dyskusji na stronie aktu prawnego nie miala z czego zyc.';
