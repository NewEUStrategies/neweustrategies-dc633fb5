...
COMMENT ON FUNCTION public.admin_event_registration_notify_payload(jsonb) IS
  'Dane JEDNEGO zgloszenia potrzebne do wyslania maila: adres, imie, JEZYK odbiorcy (profil -> newsletter -> pl), najemca, status w chwili odczytu oraz tytul i termin wydarzenia. Bramka: assert_editor_tenant().';
...
COMMENT ON FUNCTION public.event_registration_notify_payload(jsonb) IS
  'Dane WLASNEGO zgloszenia po kluczu manage_token: adres, imie, jezyk, najemca, status oraz tytul i termin wydarzenia. Uwierzytelnienie tym samym sekretem, co event_registration_cancel. Uzasadnienie odmowy NIE wychodzi ta droga.';