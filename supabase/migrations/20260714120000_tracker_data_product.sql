-- ============================================================================
-- Tracker UE jako produkt danych (P1, runda 2) - rekomendacje 4 i 5 z
-- docs/OCENA_KONKURENCYJNA_2026-07-13.md:
--   * dedykowany rodzaj powiadomień 'tracker' (osobny przełącznik + grupowanie
--     w digestcie - alert legislacyjny czyta się jak brief, nie jak "content"),
--   * powiązane akty (eu_policy_links) - relacje między dossier w duchu
--     Legislative Compass,
--   * RPC statystyk trackera pod dashboard.
--
-- Wszystko idempotentne.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) Dedykowany rodzaj powiadomień 'tracker'
-- ----------------------------------------------------------------------------
ALTER TABLE public.notification_preferences
  ADD COLUMN IF NOT EXISTS enabled_tracker boolean NOT NULL DEFAULT true;

-- Rozszerz CHECK rodzajów o 'tracker' (odtwórz constraint z pełną listą).
...
-- enqueue_notification: dopnij gałąź 'tracker' do mapy preferencji.
-- (Wierna kopia najnowszego ciała z 20260713140000 + jedna linia CASE.)
...
-- Trigger trackera: emituj alert obserwującym jako rodzaj 'tracker'
-- (reszta ciała bez zmian względem 20260713096000).
...
-- ----------------------------------------------------------------------------
-- 2) Powiązane akty (relacje skierowane między dossier)
-- ----------------------------------------------------------------------------
...
-- tenant_id/created_by przypina baza; oba dossier muszą należeć do tego samego
-- najemcy (relacja cross-tenant nie ma sensu i łamałaby izolację odczytu).
...
-- Publiczny odczyt tylko gdy OBA dossier są opublikowane (krawędź do szkicu
-- nie wycieka jego istnienia).
...
-- ----------------------------------------------------------------------------
-- 3) Statystyki trackera (dashboard) - agregaty publiczne, tenant-scoped.
--    Zwraca liczności per etap i per obszar TYLKO dla opublikowanych dossier
--    najemcy hosta; brak wycieku wierszy (same liczby).
-- ----------------------------------------------------------------------------
