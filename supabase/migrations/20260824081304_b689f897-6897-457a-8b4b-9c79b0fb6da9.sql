...
COMMENT ON COLUMN public.events.format IS
  'GDZIE sie dzieje: onsite / online / hybrid.';
COMMENT ON COLUMN public.events.registration_mode IS
  'JAK sie zapisac: rsvp / form / external / none.';
...
COMMENT ON COLUMN public.events.guest_mode IS
  'Co widzi osoba NIEZAREJESTROWANA na wydarzenie: hidden / teaser / full.';
COMMENT ON COLUMN public.events.published_at IS
  'Stempel pierwszej publikacji, odporny na retencje `domain_events`.';
COMMENT ON COLUMN public.events.cancelled_at IS
  'Stempel odwolania, ustawiany triggerem przy przejsciu statusu.';
COMMENT ON COLUMN public.events.root_page_id IS
  'Strona glowna frontu wydarzenia (poddrzewo `pages`).';
COMMENT ON COLUMN public.events.branding IS
  'Marka wydarzenia (kolory, logotypy) niezalezna od motywu serwisu.';
...
COMMENT ON FUNCTION public.event_types_active() IS
  'Aktywne rodzaje wydarzen tenanta wolajacego (anon: tenant z naglowka hosta).';
...
COMMENT ON FUNCTION public.admin_event_types_list() IS
  'Katalog rodzajow wydarzen dla panelu, z licznikiem uzycia. Bramka: assert_admin_tenant().';
