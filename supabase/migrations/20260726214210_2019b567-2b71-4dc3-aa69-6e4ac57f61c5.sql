...
AS $function$
  ...
  scored AS (
    ...
    UNION ALL
    SELECT 'page'::text, pg.id, pg.slug,
      coalesce(pg.title_pl, pg.title_en, pg.slug),
      coalesce(pg.title_en, pg.title_pl, pg.slug),
      NULL::uuid,
      public._suggest_score(nq.q, pg.title_pl, pg.title_en)
    FROM public.pages pg, ctx, nq
    WHERE nq.qlen >= 2
      AND pg.tenant_id = ctx.tid
      AND pg.status = 'published'

    UNION ALL
    SELECT 'company'::text, mo.id, mo.slug,
      mo.name, mo.name,
      NULL::uuid,
      public._suggest_score(nq.q, mo.name, mo.name)
    FROM public.member_organizations mo, ctx, nq
    WHERE nq.qlen >= 2
      AND mo.tenant_id = ctx.tid
      AND coalesce(mo.status, 'active') <> 'archived'
  )
