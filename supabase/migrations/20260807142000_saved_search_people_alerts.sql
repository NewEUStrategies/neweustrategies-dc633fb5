-- ============================================================================
-- LUKA #3: ALERTY ZAPISANYCH WYSZUKIWAŃ CZYTAJĄ WYŁĄCZNIE `posts`.
--
-- Stan przed: run_saved_search_alerts (20260720170000) skanuje opublikowane
-- wpisy ponad znak wodny i wysyła kind 'saved_search'. Cała rura działa,
-- ale zna JEDNĄ encję. Nie dało się dostać sygnału "dołączył ktoś, kogo
-- szukasz" - a to najbardziej naturalna pętla powrotu na platformie, której
-- produktem są ludzie, nie tylko teksty. Katalog osób nie miał nawet stanu
-- w URL-u, więc nie było CZEGO zapisać.
--
-- Zakres:
--   1. `saved_searches.entity` ('posts' | 'people') - jeden zapis, dwie
--      encje, ten sam rodzaj powiadomienia (ta sama intencja użytkownika =
--      ten sam przełącznik w ustawieniach).
--   2. Drugi znak wodny: `last_seen_profile_at` (osobny od
--      `last_seen_published_at` - encje mają różne osie czasu).
--   3. Trigger znaku wodnego stempluje właściwą kolumnę wg encji.
--   4. `run_saved_search_alerts` v2: pętla rozgałęzia się po `entity`.
--      Gałąź `posts` jest przeniesiona 1:1 (dowiedziona produkcją).
--      Gałąź `people` liczy DWA zdarzenia warte sygnału:
--        * NOWY profil w katalogu (`created_at` ponad znakiem wodnym),
--        * ZADEKLAROWANA intencja (`intent_updated_at` ponad znakiem wodnym) -
--          "szukam partnerów do konsorcjum Horizon" wpisane dziś przez osobę,
--          która jest w bazie od roku, jest dokładnie tak samo interesujące
--          jak nowa rejestracja.
--      Świadomie NIE reagujemy na samo `updated_at`: podmiana avatara nie
--      jest zdarzeniem, o którym ktokolwiek chce wiedzieć.
--   5. Widoczność w gałęzi `people` jest identyczna z `search_people`:
--      wyłącznie `discoverable` profile z tenanta AUTORA ZAPISU, nigdy sam
--      autor. pg_cron nie ma `auth.uid()`, więc tenant bierzemy z wiersza
--      zapisu (`ss.tenant_id`), a nie z kontekstu żądania.
--
-- Parametry gałęzi `people` są lustrem stanu URL /people
-- (q / specialization / company / location / role / open / verified),
-- dokładnie jak gałąź `posts` jest lustrem urlToFilters wyszukiwarki treści.
--
-- Wszystko idempotentne.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) Encja zapisu + drugi znak wodny
-- ----------------------------------------------------------------------------
...
COMMENT ON COLUMN public.saved_searches.entity IS
  'Co ten zapis obserwuje: ''posts'' (wyszukiwarka treści) albo ''people'' (katalog osób). Rozgałęzia run_saved_search_alerts.';
COMMENT ON COLUMN public.saved_searches.last_seen_profile_at IS
  'Znak wodny gałęzi ''people'': najnowszy created_at / intent_updated_at, który już policzono w wysłanym alercie.';
...
-- ----------------------------------------------------------------------------
-- 2) Trigger znaku wodnego: właściwa kolumna wg encji
--
-- Reguła bez zmian: włączenie alertu bez znaku wodnego = alert "od teraz",
-- nigdy zalew powiadomień o historycznym stanie katalogu.
-- ----------------------------------------------------------------------------
  ...
  LOOP
    BEGIN
      IF s.entity = 'people' THEN
        ...
        -- `open` przyjeżdża jako CSV kodów intencji; nieznane kody odpadają,
        -- pusta lista = brak filtra (semantyka nes_post_matches_term_group).
        ...
        IF coalesce(v_count, 0) > 0 THEN
          ...
          v_body_pl := CASE
            ...
            ELSE v_count::text || ' nowych osób dla zapisanego wyszukiwania'
          END;
          v_body_en := CASE
      ...
      ELSE
        -- ── WYSZUKIWARKA TREŚCI (bez zmian względem 20260720170000) ─────────
...
COMMENT ON FUNCTION public.run_saved_search_alerts(integer) IS
  'Alerty zapisanych wyszukiwań dla DWÓCH encji: ''posts'' (nowe publikacje ponad znak wodny) i ''people'' (nowy profil albo świeżo zadeklarowana intencja w katalogu tenanta autora zapisu).';
