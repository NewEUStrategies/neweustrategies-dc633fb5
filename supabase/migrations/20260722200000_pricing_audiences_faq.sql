...
--
-- Prezentacja wzorowana na prasie cyfrowej (NYT / FT / Politico Pro), a wybor
-- na wzorcach Netflix/Apple (kotwica "Najpopularniejszy", framing ceny
-- miesiecznej przy rozliczeniu rocznym). Model:
--
--   pricing_audiences    segmenty odbiorcow per tenant (individual / business /
--                        academic / team) - /pricing grupuje po nich warstwy
--                        czlonkostwa (membership_tiers.audience_key).
--   membership_tiers     + audience_key  (segment; miekka referencja jak
--                          access_plans.tier_key - celowo bez FK, usuniecie
--                          segmentu nie kasuje warstw),
--                        + badge_pl/en   (etykieta na karcie),
--                        + highlight     (wyroznienie wizualne / kotwica),
--                        + contact_url   (nadpisanie CTA "porozmawiajmy" dla
--                          warstw sprzedawanych offline; NULL = wbudowany
--                          formularz kontaktowy na /pricing).
--                        Benefity [{pl,en}] zyskuja OPCJONALNE pola detail_pl/
--                        detail_en (rozwiniecie w stylu NYT) i group_pl/group_en
--                        (naglowek sekcji w stylu FT) - format wstecznie zgodny,
--                        starsze parsery dodatkowe pola ignoruja.
--   pricing_faq_items    FAQ cennika per tenant, opcjonalnie per segment
--                        (audience_key NULL = pytanie globalne).
--
-- Wszystko idempotentne; seed per tenant + trigger dla nowych tenantow.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) Rozszerzenia marketingowe warstw czlonkostwa.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3) FAQ cennika (globalne lub per segment).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4) Katalog warstw: rozszerzenie kanonicznego seedu (wersja z migracji
--    20260721120000 - szesc warstw 1:1, lacznie z features premium_content /
--    member_library / working_groups) o oferty akademickie i zespolowa oraz
--    o przypisanie segmentu (audience_key). Idempotencja per klucz - dane
--    istniejacych tenantow pozostaja nietkniete.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 5) Seed segmentow (idempotentny per klucz).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 6) Seed FAQ - "seed once": tylko dla tenantow bez zadnych pozycji, zeby
--    ponowne uruchomienie nigdy nie dublowalo ani nie nadpisywalo edycji.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 7) Orkiestracja seedu per tenant: segmenty -> warstwy -> mapowanie legacy
--    (istniejace warstwy dostaja segment po kluczu) -> kotwica "Najpopularniejszy"
--    na warstwie member -> FAQ. Wolana dla istniejacych tenantow i triggerem
--    dla nowych; nie nadpisuje zadnych edycji admina.
-- ----------------------------------------------------------------------------
