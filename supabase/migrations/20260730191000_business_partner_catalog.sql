-- ============================================================================
-- Partner Biznesowy: samoobsługowa subskrypcja dla firm w cyklach
-- 2 tygodnie / miesiąc / kwartał.
--
-- Kontekst: dotychczasowy obszar reklamowy (sprzedaż ekspozycji) zostaje
-- przekształcony w subskrypcję biznesową - model zgodny z Acceptable Use
-- Policy operatora płatności (Paddle jako Merchant of Record wspiera
-- subskrypcje treści/usług cyfrowych; sprzedaż reklam i sponsoringu jest
-- poza wspieranymi kategoriami). Widoczność partnera pozostaje świadczeniem
-- członkowskim (jak w warstwie Strategic Partner), nie odrębnym produktem.
--
-- Forward-only (nie zmieniamy zaaplikowanych plików - sumy kontrolne):
--   1) pricing_catalog_business_rows() - kanoniczny wiersz nowej warstwy
--      w formacie identycznym z pricing_catalog_v3_rows(); strażnik
--      tierCatalogParity.test.ts skanuje OBA pliki.
--   2) seed_membership_tiers() czyta v3 UNION business - nowe tenanty
--      dostają warstwę automatycznie (seed_pricing_defaults bez zmian).
--   3) seed_pricing_plans_v3() dostaje trzy plany 'business'
--      (two_weeks / month / quarter) obok dotychczasowych VALUES.
--   4) Idempotentny backfill istniejących tenantów.
--
-- Ceny startowe (edytowalne w panelu /admin/pricing): 590 zł / 2 tyg.,
-- 990 zł / mies., 2490 zł / kwartał (≈830 zł/mies., realna oszczędność).
-- ============================================================================

-- 1) Kanoniczny wiersz warstwy Partner Biznesowy (format kolumn 1:1 z v3).
...
-- 2) Seed warstw czyta katalog v3 + business (idempotentnie per key).
...
-- 3) Plany sprzedażowe: dotychczasowe VALUES + Partner Biznesowy w trzech
--    cyklach. Nazwa funkcji zostaje (seed_pricing_defaults woła ją po nazwie).
...
-- 4) Backfill istniejących tenantów (idempotentny - guardy WHERE NOT EXISTS).
DO $$
DECLARE
  r record;
BEGIN
