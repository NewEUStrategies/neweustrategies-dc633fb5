-- ============================================================================
-- SPÓJNOŚĆ MIĘDZY MODUŁAMI, część 2/5: jeden graf powiązań + wzmianki @slug.
--
-- cross_references to uniwersalny indeks relacji między encjami RÓŻNYCH
-- modułów (comment <-> post, notatka CRM <-> lead, wzmianka <-> profil, ...).
-- Zamiast joinów pisanych osobno w każdym module, panele "Powiązane" czytają
-- jeden graf przez RPC get_linked_items (obie strony relacji + gotowe
-- etykiety, zero N+1 po stronie klienta).
--
-- Backfill triggerami: INSERT na comments / crm_lead_notes automatycznie
-- dopisuje krawędź do grafu; istniejące dane są migrowane jednorazowo niżej.
--
-- Wzmianki: parsowane PO STRONIE BAZY (trigger na comments, messages,
-- crm_lead_notes), więc działają w każdym kliencie bez zmian UI:
--   @slug-profilu -> krawędź 'mention' w grafie + notyfikacja przez
--   enqueue_notification (szanuje preferencje odbiorcy) + zdarzenie
--   mention.created.v1 na szynie.
--
...
-- Graf czyta staff swojego tenanta (panele admina); autor krawędzi widzi swoje.
...
-- Ręczne linkowanie (panel admina); krawędzie systemowe wstawiają triggery
-- SECURITY DEFINER, więc nie potrzebują polityki.
...
-- ----------------------------------------------------------------------------
-- Bezpieczny zapis krawędzi (idempotentny, nie wywraca zapisu źródłowego).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- get_linked_items: obie strony grafu dla encji + etykiety rozwiązane w bazie
-- (tytuł posta, e-mail leada, nazwa profilu, skrót komentarza/notatki).
-- Tylko staff - graf zawiera odnośniki do treści roboczych i CRM.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  CASE p_type
    ...
    WHEN 'message' THEN
      v_label := NULL; -- prywatność: treści czatu nie wyciekają do etykiet
...
-- ----------------------------------------------------------------------------
-- Wzmianki @slug - wspólny procesor. Rozwiązuje slugi profili w obrębie
-- tenanta, pomija autowzmianki, zapisuje krawędź grafu, notyfikuje odbiorcę
-- i emituje mention.created.v1.
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Warunek przed @ odsiewa adresy e-mail ("user@example.com" nie jest
  -- wzmianką); ARE Postgresa wspiera grupy nieprzechwytujące.
  FOR v_slug IN
...
-- ----------------------------------------------------------------------------
-- Triggery: graf + wzmianki przy INSERT-ach źródłowych.
-- ----------------------------------------------------------------------------

-- Komentarz: krawędź comment -> post + wzmianki w treści (href do posta).
...
-- Notatka CRM: krawędź note -> lead + wzmianki (href do panelu CRM).
...
-- Wiadomość czatu: tylko wzmianki (bez krawędzi message -> conversation,
-- graf nie ma indeksować całego czatu; etykiety wiadomości i tak są ukryte).
...
-- ----------------------------------------------------------------------------
-- Backfill istniejących danych do grafu (jednorazowy, idempotentny).
-- ----------------------------------------------------------------------------
INSERT INTO public.cross_references (
  tenant_id, source_type, source_id, target_type, target_id, relation, created_by
)
