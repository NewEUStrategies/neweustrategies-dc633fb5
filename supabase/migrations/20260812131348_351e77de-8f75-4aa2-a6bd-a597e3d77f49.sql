-- club_capabilities: odczyt grupy respektuje jej widoczność, nie tylko prawo pisania.
--
-- Flaga v_group_open gasła dla stanów 'draft'/'archived'/'frozen', okna przed
-- opens_at i po closes_at, ale bramkowała wyłącznie can_post_thread/can_reply/
-- can_react. Wyrażenie v_read przepuszczało każdego członka klubu (v_role <>
-- 'non_member'), więc treść grupy roboczej - w tym wątki i odpowiedzi widoczne
-- tylko dla zarządzających - była czytelna dla wszystkich członków klubu.
--
-- Rozdzielenie na dwie flagi jest tu istotą rzeczy, bo stany grupy znaczą dwie
-- różne rzeczy: 'frozen', 'archived' i zamknięte okno to "można czytać, nie można
-- pisać", a 'draft' i okno przed opens_at to "grupa jeszcze nie istnieje dla
-- członka". Tylko drugi zbiór gasi v_group_visible; pierwszy zostaje czytelny.
-- Asercje w scripts/pg-harness/runtime_test.sql (sekcja 39 [A9]) pilnują obu
-- kierunków naraz i to one wykryły ten wyciek.
