...
-- CENNIK 3.0 + RETENCJA.
--
-- 1) Katalog ofert wg briefu wlasciciela (kopia redakcyjna JEST zrodlem
--    prawdy - ta migracja celowo NADPISUJE tresci warstw i segmentow):
--      Dla Ciebie:          Essential (0 zl) / Plus 59|590 [Najpopularniejszy]
--                           / Pro 129|1290 / VIP (rozmowa)
--      Dla firm:            Enterprise / Strategic Partner [Najpopularniejszy]
--                           / Partner Generalny / President's Circle (tylko na
--                           zaproszenie - bez ceny i bez przycisku)
--      Program Akademicki:  Student i Doktorant 19/mies. / Kadra Akademicka
--                           29/mies. / Organizacja Non-profit (preferencyjnie)
--      Dla zespolow:        Zespol od 99/mies. za miejsce (2-20 miejsc)
--    Nowe kolumny prezentacji: membership_tiers.per_seat / price_note_* /
--    cta_mode ('auto'|'contact'|'none'), pricing_audiences.trust_* (pasek
--    zaufania segmentu). Ceny wewnetrzne ofert enterprise NIE sa publikowane.
--    Uwaga do warunkow (nie do copy): VIP sprzedaje obecnosc w sieci
--    i wprowadzenia - nigdy licencji na baze kontaktow.
--
-- 2) RETENCJA (zatrzymanie odchodzacych): ankieta powodu + kontrofertka
--    rabatowa konfigurowana w panelu (retention_settings), powody odejscia
--    (retention_reasons, CRUD w panelu), zapis odpowiedzi i decyzji
--    (retention_feedback, tylko server-side). Zaakceptowana kontrofertka
--    generuje personalny kupon w ISTNIEJACYM module b2b_coupons - realizacja
--    w checkoutcie ta sama sciezka co kupony B2B.
--
-- Jedno zrodlo katalogu: pricing_catalog_v3_rows(); seed nowych tenantow
-- i force-update istniejacych czytaja te sama liste. Wszystko idempotentne.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) Kolumny prezentacji.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 2) Katalog v3 - jedno zrodlo prawdy dla seedu i aktualizacji.
-- ----------------------------------------------------------------------------
...
-- Seed brakujacych warstw (nowe tenanty i nowe klucze) - z katalogu v3.
...
-- Segmenty v3 (seed brakujacych; tresc istniejacych aktualizuje apply_v3).
...
-- ----------------------------------------------------------------------------
-- 3) Plany sprzedazowe wg briefu (Plus 59|590, Pro 129|1290, Student 19,
--    Kadra 29, Zespol 99/miejsce). Idempotentnie per (tier_key, interval) -
--    istniejace plany i ich ceny pozostaja nietkniete.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4) Force-update istniejacych tenantow do katalogu v3 (kopia wlasciciela
--    nadpisuje tresc; features scala sie przez ||, zeby nie gubic recznych
--    flag bramek nadanych przez admina).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 5) RETENCJA: ustawienia kontrofertki (per tenant), katalog powodow odejscia
--    i zapis odpowiedzi. Feedback pisze WYLACZNIE serwer (service_role) -
--    klient nie ma zadnej sciezki zapisu; admin czyta w panelu.
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.retention_settings (
  ...
  -- Liczba platnosci, przy ktorych kupon moze zostac uzyty (max_redemptions
  -- kuponu retencyjnego) - "x% upustu na x okresow".
...
-- Seed retencji: ustawienia (upsert-if-missing) + powody (seed once).
...
-- ----------------------------------------------------------------------------
-- 6) Orkiestracja: seed_pricing_defaults obejmuje teraz takze plany v3
--    i retencje (nowe tenanty dostaja komplet przez istniejacy trigger);
--    istniejace tenanty przechodza force-update katalogu v3.
-- ----------------------------------------------------------------------------
