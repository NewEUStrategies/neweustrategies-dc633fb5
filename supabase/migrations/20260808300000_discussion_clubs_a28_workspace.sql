-- ============================================================================
-- KLUBY DYSKUSYJNE - ETAP A28: PRZESTRZEN ROBOCZA KLUBU
--
-- CZEGO BRAKOWALO. Po A1-A27 klub umie ROZMAWIAC (watki, odpowiedzi, reakcje,
-- stanowiska, moderacja, wyszukiwarka) i nie umie NIC POZA TYM. Tymczasem
-- deklaracja kazdego z tych klubow - "trwala deliberacja w rytmie procesu
-- legislacyjnego, nie w rytmie newsow" - obiecuje trzy rzeczy, ktorych model
-- danych nie mial:
--
--   1) DOKUMENTY. Rozmowa o zapasach amunicji w regionie bez wspolnego zestawu
--      zrodel to rozmowa, w ktorej kazdy cytuje co innego. Watek `resource`
--      pozwalal wkleic link W TRESCI - czyli pochowac go w 40 odpowiedziach.
--   2) HARMONOGRAM I KALENDARZ. "Rytm procesu legislacyjnego" jest twierdzeniem
--      o DATACH. Bez etapow i terminow klub ma rytm newsow z ladniejsza nazwa.
--   3) POMIAR. Klub, ktory nie widzi wlasnej dynamiki, nie ma jak stwierdzic,
--      ze umiera - a pasek "7 odpowiedzi w tym tygodniu" liczyl sie ze strony
--      pobranych watkow, wiec mierzyl WIDOK, nie KLUB.
--
-- TRZY DECYZJE, KTORE TRZEBA ZROZUMIEC PRZED CZYTANIEM RESZTY:
--
-- A) TA SAMA BRAMKA, CO CALY MODUL. Zero nowych regul dostepu. Odczyt wymaga
--    `can_read`, kuratorstwo `can_moderate`, obecnosc - AKTYWNEGO czlonkostwa.
--    Wszystko liczone przez public.club_capabilities(), nigdy inline (inwariant
--    z A1; lekcja profiles_guard_verification z audytu 2026-08-06).
--
-- B) ZERO DRUGIEGO SILNIKA. Kalendarz klubu NIE jest druga implementacja
--    `events`: tamte to wydarzenia PLATFORMY (bilety, prelegenci, RSVP z
--    limitem miejsc), te sa wewnetrzna siatka terminow jednego klubu. Krawedz
--    do platformy istnieje i jest jedna: `club_events.anchor_event_id`. To ta
--    sama doktryna, co `poll_id` w A20 - wpinamy sie w istniejacy mechanizm
--    zamiast budowac rownolegly.
--
-- C) ANALITYKA JEST LICZONA, NIE PRZECHOWYWANA. Zadnej tabeli agregatow, zero
--    zadan cron. Dwa RPC czytaja `club_threads`/`club_replies` po indeksach,
--    ktore i tak istnieja od A13. Tabela agregatow rozjechalaby sie z prawda
--    przy pierwszej moderacji usuwajacej watek.
--
-- Wszystko idempotentne. Tabele sa RPC-only: klient nie ma zadnych grantow DML,
-- cala powierzchnia mutacji idzie przez SECURITY DEFINER.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) DOKUMENTY KLUBU
--
-- `visibility` jest OSOBNA osia od widocznosci klubu, dokladnie jak w A1
-- widocznosc bywa osobna od polityki wstepu: 'club' widzi kazdy, kto ma
-- `can_read`, 'moderators' - wylacznie prowadzenie. Bez tej osi kazda notatka
-- robocza prowadzacego musialaby wyladowac poza klubem.
--
-- Dokument moze, ale nie musi, wisiec przy watku (`thread_id`) i przy dziale
-- (`group_id`). Oba sa NULLABLE, bo biblioteka klubu istnieje takze wtedy,
-- gdy zadna rozmowa jeszcze sie nie zaczela - to jest caly sens materialu
-- WEJSCIOWEGO do deliberacji.
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.club_documents (
  ...
  -- Plik ALBO link zewnetrzny. Jedno z dwoch musi byc - dokument bez tresci
  -- jest wpisem w spisie tresci, nie dokumentem.
  file_url        text,
...
-- ----------------------------------------------------------------------------
-- 2) KALENDARZ / HARMONOGRAM KLUBU
--
-- Jedna tabela na spotkania I terminy, nie dwie. Roznica miedzy "posiedzenie
-- klubu 12.09" a "koniec konsultacji 30.09" jest roznica RODZAJU (`kind`),
-- nie bytu: obie rzeczy maja date, obie stoja w tym samym kalendarzu i obie
-- odpowiadaja na to samo pytanie czlonka - "co mnie czeka w tym klubie".
-- Dwie tabele znaczylyby dwa zapytania i dwa miejsca do rozjechania.
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.club_events (
  ...
  -- JEDYNA krawedz do wydarzen platformy. Klub nie kopiuje `events`, tylko
  -- wskazuje wpis, ktory ma wlasne bilety, prelegentow i strone.
  ...
  -- Koniec przed poczatkiem to nie jest "dziwny wpis", tylko wpis, ktory
  -- rozwali kazde sortowanie i kazdy widok tygodnia.
...
-- Kalendarz pyta ZAWSZE o zakres dat w obrebie klubu - to jest ten indeks.
...
-- Obecnosc. Trzy stany, bo "moze" jest prawdziwa odpowiedzia i wypchniecie
-- jej do braku wiersza kasuje informacje.
...
-- ----------------------------------------------------------------------------
-- 3) HARMONOGRAM PRAC (ETAPY)
--
-- To NIE jest kalendarz. Kalendarz odpowiada "co i kiedy", harmonogram - "na
-- ktorym etapie jestesmy". Klub, ktory towarzyszy procesowi legislacyjnemu,
-- ma etapy o roznej dlugosci (konsultacje, pierwsze czytanie, trilog), a nie
-- punkty w czasie. Dlatego zakres dat, `state` i JAWNA kolejnosc.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4) RLS: deny-all. Tabele sa RPC-only, tak jak cala reszta modulu.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 5) Triggery: pinowanie tenanta, updated_at, licznik obecnosci
--
-- `club_child_pin_tenant()` (A1) wyprowadza tenanta z `NEW.club_id` i ignoruje
-- wartosc podana przez wolajacego - dokladnie to, czego te trzy tabele
-- potrzebuja. RSVP nie ma `club_id`, wiec ma wlasna funkcje idaca przez
-- wydarzenie.
-- ----------------------------------------------------------------------------
...
-- Licznik obecnosci utrzymuje trigger, bo lista kalendarza rysuje go w KAZDYM
-- wierszu - dokladnie ten sam powod, co przy `member_count` w A1.
...
-- ----------------------------------------------------------------------------
-- 6) Wspolna bramka kuratorska modulu przestrzeni roboczej
--
-- Trzy tabele x cztery operacje = dwanascie miejsc, w ktorych ta sama regula
-- moglaby sie rozjechac. Dlatego jest JEDNA funkcja, dokladnie jak
-- `is_club_admin()` w A1 - i tak samo nigdy nie rozwijana inline.
-- ----------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.club_require_curator(uuid) IS
  'Bramka kuratorska przestrzeni roboczej klubu (dokumenty, kalendarz, harmonogram): wymaga can_moderate. Zwraca tenant_id klubu. Nigdy nie rozwijac inline - jedna regula, dwanascie wywolan.';
...
-- Upsert idzie jsonb, tak jak reszta mutacji administracyjnych modulu: pola
-- nieobecne w patchu zostaja nietkniete, wiec klient nie musi odsylac calego
-- wiersza, zeby przypiac dokument.
...
BEGIN
  ...
  -- Aktualizacja. Wiersz MUSI nalezec do klubu z bramki - inaczej kurator
  -- klubu A edytowalby dokument klubu B, podajac jego id.
...
COMMENT ON FUNCTION public.club_document_upsert(uuid, jsonb) IS
  'Dodanie/edycja dokumentu klubu. Patch jsonb: klucz nieobecny znaczy "nie ruszaj pola". UPDATE zwiazany z p_club_id, wiec kurator nie siegnie do cudzego klubu po id.';
...
-- Licznik pobran. Osobne RPC, bo pobranie jest czynnoscia CZYTELNIKA, a nie
-- kuratora - bramka to `can_read`, nie `can_moderate`.
...
COMMENT ON FUNCTION public.club_document_register_download(uuid) IS
  'Zlicza pobranie. Zwraca false zamiast rzucac - nieudany licznik nie moze przerwac pobierania pliku po stronie klienta.';
...
COMMENT ON FUNCTION public.club_events_list(uuid, timestamptz, timestamptz, text, integer) IS
  'Kalendarz klubu w zakresie dat. Zakres domyka sie po ends_at, wiec wydarzenie trwajace przez granice okna nie znika. meeting_url wychodzi tylko uczestnikom.';
...
-- Obecnosc. Bramka to AKTYWNE CZLONKOSTWO, a nie `can_read`: deklaracja
-- obecnosci osoby spoza klubu jest szumem dla prowadzacego, ktory na jej
-- podstawie rezerwuje salę.
...
COMMENT ON FUNCTION public.club_event_rsvp(uuid, text) IS
  'Deklaracja obecnosci. Wymaga AKTYWNEGO czlonkostwa, nie samego can_read - deklaracja osoby spoza klubu jest szumem dla prowadzacego, ktory na jej podstawie rezerwuje sale.';
...
-- ----------------------------------------------------------------------------
-- 9) HARMONOGRAM PRAC - odczyt i mutacje
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 10) POMIAR - szereg czasowy i przekroje
--
-- Liczone na zywo, bez tabeli agregatow (decyzja C z naglowka). Oba RPC
-- respektuja `can_read`, wiec anonim przy klubie zamknietym dostaje pusty
-- zbior, a nie ksztalt jego dynamiki.
-- ----------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.club_activity_series(uuid, integer) IS
  'Szereg dzienny klubu, luki wypelnione zerami przez generate_series - wykres bez dziur wymaga KAZDEGO dnia, takze pustego.';
...
AS $$
  ...
  -- Mediana czasu do PIERWSZEJ odpowiedzi. Srednia klamalaby na jednym watku
  -- odkopanym po roku - dlatego percentyl, nie avg.
  first_reply AS (
    SELECT t.id,
  ...
  SELECT
    ...
    -- Przekroje ida jako jsonb, bo maja ZMIENNA liczbe wierszy, a kolumna
    -- na kazdy rodzaj watku rozjechalaby sie przy pierwszym nowym rodzaju.
    COALESCE((
    ...
    -- Ranking WYLACZNIE dla klubu atrybuowanego. W trybie Chatham House
    -- "kto najwiecej pisze" jest dokladnie ta informacja, ktorej ten tryb
    -- obiecuje nie ujawniac - a lista dziesieciu nazwisk deanonimizuje
    -- rozmowe skuteczniej niz podpis pod pojedynczym wpisem.
    COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
...
COMMENT ON FUNCTION public.club_workspace_stats(uuid, integer) IS
  'Przekroj dynamiki klubu. Ranking autorow wychodzi WYLACZNIE dla attribution_mode=attributed - w trybie chatham lista najaktywniejszych deanonimizuje rozmowe.';
