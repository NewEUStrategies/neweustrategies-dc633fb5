...
COMMENT ON FUNCTION public.club_invite_by_email(uuid, text, text, uuid) IS
  'Sciezka B: zaproszenie e-mailowe przez user_invitations. Rola PLATFORMY zawsze user; rola klubowa wylacznie w metadata.club_role.';
...
COMMENT ON FUNCTION public.club_accept_rules(uuid) IS
  'Akceptacja zasad klubu. Znacznik czasu jest podstawa do przetwarzania tresci przy usunieciu konta (anonimizacja) - dlatego nie nadpisujemy istniejacej daty.';
...
COMMENT ON FUNCTION public.admin_club_invitations(uuid) IS
  'Historia zaproszen klubu: sciezka bezposrednia i e-mailowa w jednej liscie.';
