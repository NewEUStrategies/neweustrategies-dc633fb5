...
--      czekaja. Goscie JUZ przyjeci zostaja nietknieci (patrz nizej).
...
--      zamowienie, gdy wynik przychodzi bez zamowienia.
...
--      kogo i czy bilet wyszedl.
--   g) `admin_event_ticket_resend` - organizator wysyla bilet ponownie.
--
...
DECLARE
  ...
  v_left integer;
BEGIN
  -- PRZYJECIE. Tylko z oczekiwania - prowadzacy przywrocony z odrzucenia albo
  -- anulowania nie ma gosci w oczekiwaniu (odrzucenie zamknelo ich razem z nim).
  IF OLD.status IN ('draft', 'pending', 'waitlist') AND NEW.status = 'approved' THEN
    FOR g IN
      ...
      WHERE r.tenant_id = NEW.tenant_id
        ...
        -- go trigger platnosci razem z wplata prowadzacego.
  ...
  -- anulowanie - daty (`cancelled_dated`).
  ELSIF NEW.status IN ('rejected', 'cancelled') THEN
    UPDATE public.event_registrations r SET
      ...
      decision_source = COALESCE(NEW.decision_source, 'system'),
      cancelled_at = CASE
        WHEN NEW.status = 'cancelled' THEN COALESCE(r.cancelled_at, now())
        ELSE r.cancelled_at
      END,
      waitlist_position = NULL,
      qr_token_hash = NULL,
...
COMMENT ON FUNCTION public._tg_event_group_follow_lead_status() IS
  'Kaskada statusu prowadzacego grupy na gosci: zatwierdzenie przyjmuje gosci rozliczonych (albo stawia ich w kolejce, gdy brak miejsca), odrzucenie i anulowanie zamyka gosci, ktorzy jeszcze czekaja. Nieoplaconych gosci przyjmuje trigger platnosci, przyjetych nie rusza.';
...
-- a gosc nie moze zgubic wlasnego) i zawezenie do najemcy prowadzacego.
...
--
-- Wiersze: goscie, prowadzacy z aktywnymi goscmi i kazdy przyjety zapis (tylko
-- tam znacznik biletu i rozliczenie cos znacza). Bramka jak lista zgloszen -
-- `assert_editor_tenant()` deleguje do `assert_event_admin_tenant()`, a nowy
-- kod wola nastepce wprost.
CREATE OR REPLACE FUNCTION public.admin_event_registration_group_links(p_event_id uuid)
RETURNS TABLE (
...
BEGIN
  ...
  WHERE r.tenant_id = v_tenant
    ...
    AND (
      r.group_lead_registration_id IS NOT NULL
      OR COALESCE(gc.cnt, 0) > 0
      OR r.status IN ('approved', 'attended')
    )
  ORDER BY r.created_at, r.id;
END;
$$;

REVOKE ALL ON FUNCTION public.admin_event_registration_group_links(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_event_registration_group_links(uuid) TO authenticated, service_role;

COMMENT ON FUNCTION public.admin_event_registration_group_links(uuid) IS
  'Panel zgloszen: powiazania grupy (gosc -> prowadzacy z imieniem i nazwiskiem, prowadzacy -> liczba aktywnych gosci), rozliczenie i znacznik wyslanego biletu. Wiersze gosci, prowadzacych z goscmi i przyjetych zapisow. Bramka: assert_event_admin_tenant().';
...
COMMENT ON FUNCTION public.admin_event_ticket_resend(uuid, boolean) IS
  'Ponowna wysylka biletu: kasuje znacznik wysylki przyjetych i rozliczonych wierszy (zgloszenie albo cala jego grupa) i oddaje identyfikator do wydania przez serwer. Wiersz nieprzyjety albo nierozliczony: ticket_not_issuable. Bramka: assert_event_admin_tenant().';
