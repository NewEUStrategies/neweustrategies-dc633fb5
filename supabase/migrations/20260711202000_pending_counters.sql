-- ============================================================================
-- SPÓJNOŚĆ MIĘDZY MODUŁAMI, część 3/5: materializacja liczników.
--
-- Zamiast N zapytań COUNT(*) na każdy badge (dzwonek notyfikacji, czat,
-- kolejki moderacji), triggery utrzymują dwa małe słowniki liczników:
--   * user_pending_counters  - per użytkownik: notifications_unread, chat_unread;
--   * tenant_pending_counters - per tenant (kolejki staffu): comments_pending,
--     crm_leads_new.
-- Frontend czyta JEDEN wiersz/kilka wierszy i subskrybuje Realtime - badge'e
-- aktualizują się na żywo bez odpytywania tabel źródłowych.
--
-- Dryf: funkcje recompute_* przeliczają z tabel źródłowych (wołane przy
-- seedowaniu i dostępne jako zawór bezpieczeństwa).
--
...
-- ----------------------------------------------------------------------------
-- Bumpy - wołane wyłącznie z triggerów SECURITY DEFINER. Nigdy nie wywracają
-- zapisu źródłowego; wartość nie schodzi poniżej zera.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Przeliczenia z tabel źródłowych (seed + zawór bezpieczeństwa przy dryfie).
-- ----------------------------------------------------------------------------
...
-- Użytkownik może przeliczyć wyłącznie własne liczniki.
...
-- chat_unread (delta z materializowanego unread_count uczestnika)
...
-- comments_pending (kolejka moderacji staffu, per tenant)
...
-- crm_leads_new (świeże leady czekające na kontakt, per tenant)
...
-- ----------------------------------------------------------------------------
-- Seed wartości początkowych z danych istniejących.
-- ----------------------------------------------------------------------------
DO $$
DECLARE
  v_user uuid;
