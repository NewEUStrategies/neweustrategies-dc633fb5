-- ============================================================================
-- SPÓJNOŚĆ MIĘDZY MODUŁAMI, część 5/5: workflowy między modułami.
--
-- Deklaratywny silnik "przepisów" napędzany szyną zdarzeń:
--   workflow_definitions: trigger_event_type + condition (containment @> na
--   payloadzie) + steps (lista akcji). Trigger AFTER INSERT na domain_events
--   wykonuje pasujące definicje; każdy przebieg ląduje w workflow_runs
--   (correlation_id spina przebieg ze zdarzeniem źródłowym - explainability).
--
-- Katalog akcji v1 (celowo bez akcji emitującej zdarzenia - zero ryzyka pętli;
-- głębokość łańcucha ogranicza dodatkowo pg_trigger_depth):
--   notify_user       - notyfikacja dla użytkownika wskazanego w payloadzie,
--   notify_staff      - notyfikacja dla ról staffu tenanta,
--   notify_followers  - notyfikacja dla obserwujących autora (user_follows),
--   create_crm_lead   - upsert leada CRM z danych payloadu (crm_upsert_lead),
--   add_cross_reference - krawędź w grafie powiązań (część 2/5).
--
-- workflow_templates to globalny katalog gotowych przepisów cross-module;
-- install_workflow_template instaluje przepis w tenancie wołającego. Cztery
-- flagowe przepisy są instalowane od razu we wszystkich istniejących tenantach:
--   1. newsletter confirmed  -> lead w CRM,
--   2. post published        -> notyfikacje obserwujących autora,
--   3. CRM lead won          -> notyfikacja staffu,
--   4. comment pending       -> notyfikacja moderacji.
--
...
-- ----------------------------------------------------------------------------
-- Pomocnik: tekst z payloadu po kluczu wskazanym w parametrach kroku.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Wykonanie jednego kroku. Rzuca wyjątkiem przy nieznanej akcji - silnik
-- łapie go per definicja i zapisuje failed run.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Silnik: trigger na szynie. Awaria jednej definicji nie blokuje ani innych
-- definicji, ani zapisu źródłowego (failed run + dalej).
-- ----------------------------------------------------------------------------
...
DECLARE
  ...
  v_done integer;
BEGIN
  -- Ogranicznik głębokości łańcucha (workflow -> INSERT -> zdarzenie -> workflow).
...
-- ----------------------------------------------------------------------------
-- Instalacja przepisu z katalogu w tenancie wołającego (staff).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Katalog przepisów cross-module.
-- ----------------------------------------------------------------------------
...
-- Flagowe przepisy startują od razu we wszystkich istniejących tenantach
-- (idempotentnie), a trigger na tenants instaluje je automatycznie każdemu
-- nowemu tenantowi - katalog działa od pierwszego dnia bez ręcznej akcji.
INSERT INTO public.workflow_definitions (
