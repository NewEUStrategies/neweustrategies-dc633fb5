-- Paginacja SERWEROWA archiwum autora (/author/$slug): RPC get_expert_materials
-- zwraca JEDNA strone przefiltrowanych materialow eksperta + dokladny total,
-- zamiast zmuszac klienta do filtrowania i krojenia pobranego kompletu.
-- Dopelnia rekomendacje audytu 2026-07-30 (MODUL 5, "Archiwum autora"):
-- "Paginacja serwerowa z URL" - stan strony/filtrow zyje w URL trasy, a baza
-- liczy total i wycina strone (LIMIT/OFFSET) po stronie SQL.
--
-- Kontrakty (te same, co get_expert_hub - obie funkcje musza widziec ten sam
-- zbior):
--   * SECURITY INVOKER + STABLE: czyta wylacznie przez RLS wolajacego (anon
--     widzi dokladnie to, co przy zapytaniach bezposrednich; tenant scoping
--     przez public_tenant_id() dziala bez zmian, takze na slugach filtrow -
--     programs/regions maja tenant-scoped public read).
--   * Zbior zrodlowy 1:1 z get_expert_hub: posty (autor glowny + wspolautor,
--     dedupe - autor glowny wygrywa), podcasty, wydarzenia (host + prelegent,
--     dedupe - host wygrywa), wylacznie status='published', deleted_at IS NULL.
--   * Mapowanie wierszy zostaje w TS (normalize.ts) - items niosa surowe
--     wiersze w kolejnosci strony, pivoty taksonomii tylko dla postow strony.
--   * Filtry AND (parytet z applyMaterialFilters): kind / program / region /
--     kategoria / tag (slugi) / rok (UTC). Nieznany slug lub kind = pusty
--     wynik, nigdy wyjatek. Filtr taga/kategorii dotyczy tylko postow -
--     podcasty i wydarzenia nie maja tych pivotow, wiec odpadaja.
--   * Strona poza zakresem: items puste, total prawdziwy (klient sam
--     przekierowuje na ostatnia strone).
--   * Porzadek deterministyczny: data DESC NULLS LAST, id ASC (stabilna
--     paginacja bez duplikatow/przeskokow miedzy stronami).
--   * Brak profilu => jsonb z found=false (TS mapuje na 404), nigdy wyjatek.
...
DECLARE
  ...
  -- Sufit 60 spina sie z ARCHIVE_PAGE_SIZE taksonomii; klient uzywa 9.
...
BEGIN
  ...
  -- Rezolucja profilu: slug, a dla UUID fallback po id (jak w get_expert_hub).
  ...
  -- Walidacja/rezolucja filtrow. Nieznana wartosc degraduje do pustego
  -- wyniku - RPC broni sie sam, niezaleznie od walidacji parsera URL.
  ...
  -- Zunifikowany, ODFILTROWANY zbior kandydatow (id, kind, data) + total +
  -- okno strony w jednym przebiegu. LEFT JOIN pivotu zawezonego do eksperta
  -- daje najwyzej jeden wiersz (PK zlozony), wiec dedupe post/event jest
  -- strukturalny, nie przez DISTINCT.
  WITH cand AS (
    SELECT p.id,
           CASE
  ...
  -- Pelne wiersze wylacznie dla pozycji strony, w kolejnosci okna. Kolumny
  -- 1:1 z get_expert_hub - TS mapuje obie sciezki jedna asemblacja.
  ...
  -- Pivoty taksonomii TYLKO dla postow biezacej strony (karty pokazuja
  -- kategorie/tagi, wiec strona niesie komplet swoich metadanych).
...
-- Sciezka krytyczna paginacji: wspolautorstwa i prelekcje sa juz pokryte
-- (idx_event_speakers_user, PK post_authors), a posty eksperta wyciaga
-- istniejacy indeks author_id. Dokladamy brakujacy indeks pod liste postow
-- autora sortowana po dacie (najczestszy przebieg kandydatow).
