...
--
-- BLIZNIAK drizzle/migrations/0050_event_speaker_card_tracks.sql - ten sam SQL
-- wykonywalny (pilnuje tego `src/lib/ci/migrationLaneParity.ts`).
--
-- PO CO
--   Siatka prelegentow (`/events/<slug>/speakers`) dostaje karte w ukladzie
--   „naglowka profilu": domyslnie widac zdjecie, nazwisko, role i instytucje,
--   a dopiero KLIK rozwija karte do pelnego kadru zdjecia z podpisem na
--   gradiencie i przyciskiem akcji. Redakcja musi miec czym te karte wypelnic -
--   stad piec kolumn nakladki scenicznej. Karta pokazuje tez, w jakich
--   SCIEZKACH prelegent wystepuje - i ta informacja NIE jest wpisywana recznie:
--   wynika z obsady sesji (`event_session_speakers` -> `event_sessions.track_id`).
--   Przypisanie prelegenta do sesji w sciezce AUTOMATYCZNIE dopisuje mu te
--   sciezke; zdjecie go z sesji - automatycznie ja zdejmuje. Drugiego rejestru
--   „prelegent <-> sciezka" nie ma i nie bedzie: dwa zrodla jednego faktu
--   rozjezdzaja sie przy pierwszej zmianie programu.
--
-- CO ROBI
--   1) `speaker_profiles`: `card_photo_url` (kadr rozwinietej karty), etykieta
--      przycisku PL/EN, adres przycisku i kolor przycisku. Kolumny siedza na
--      NAKLADCE SCENICZNEJ, bo ta wskazuje konto ALBO osobe z kartoteki -
--      jedna definicja karty dla obu rodzajow prelegenta.
--   2) `_event_speaker_tracks(...)` - JEDEN rachunek sciezek prelegenta,
--      wspolny dla projekcji publicznej i listy panelu.
--   3) `event_speakers_public` - piec pol karty (za ta sama bramka
--      `sp.is_public`, co tekst sceniczny) i `tracks jsonb`. Zmiana RETURNS
--      TABLE wymaga DROP + CREATE (42P13 przy CREATE OR REPLACE).
--   4) `admin_event_speakers_list` - pola karty, naglowek sceniczny, `tracks`
--      i `sessions` (obsada), zeby panel pokazal sciezki przypisane
--      automatycznie, a podglad studia narysowal prelegentow w programie.
--   5) `admin_event_speaker_upsert` - przyjmuje pola karty juz przy zakladaniu
--      prelegenta (PATCH po OBECNOSCI klucza, wiec da sie je tez wyczyscic).
--   6) `admin_event_speaker_card_save` - edycja karty ISTNIEJACEGO prelegenta
--      (z kontem i bez), po `speaker_profile_id`. Osobna funkcja, bo upsert
--      w trybie osoby stempluje `consent_data_processing_at` - edycja karty nie
--      moze byc zdarzeniem zgody.
--   7) `admin_event_session_detail` - obsada sesji w panelu dolacza `profiles`
--      ORAZ `event_people` LEFT JOIN-em. Dotad JOIN po `profiles` gubil osobe
--      bez konta (ten sam blad, ktory 20260923130000 naprawil w agendzie), wiec
--      edytor obsady nie widzial prelegentow wpisanych recznie. RETURNS bez
--      zmian - CREATE OR REPLACE.
--
-- CZEGO NIE ZMIENIA
--   * `event_agenda` - obsada sesji juz niesie osobe bez konta; sciezki
--     prelegenta w programie liczy front z TEJ SAMEJ listy sesji;
--   * grantow tabeli `speaker_profiles` (anon i authenticated nie maja do niej
--     dostepu; nowe kolumny czytaja wylacznie projekcje SECURITY DEFINER);
--   * bramek najemcy: plaszczyzna tresci stoi na `public_tenant_id()` bez
--     `has_role()`, plaszczyzna panelu na `assert_event_admin_tenant()`
--     (`assert_editor_tenant()` to jej alias).
--
