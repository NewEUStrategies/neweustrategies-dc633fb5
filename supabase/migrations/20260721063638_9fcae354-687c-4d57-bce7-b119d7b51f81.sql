-- Monetyzacja (3 filary): metering paywalla, ustawienia checkoutu (kupony/Tax/NIP/faktury), samoobsluga B2B + site licence przez membership_tiers.
...
RETURNS TABLE (
  enabled boolean, monthly_limit integer, used integer, remaining integer,
  requires_registration boolean, show_counter boolean
)
...
BEGIN
  ...
  RETURN QUERY SELECT
    true, v_limit, v_used, GREATEST(v_limit - v_used, 0),
...
CREATE OR REPLACE FUNCTION public.consume_metered_view(
  _entity_type public.access_entity_type, _entity_id uuid, _visitor_id uuid DEFAULT NULL
)
RETURNS TABLE (
  granted boolean, consumed boolean, used integer, monthly_limit integer,
  remaining integer, requires_registration boolean, show_counter boolean,
  content_pl text, content_en text, builder_data jsonb, blocks_data jsonb
)
...
DECLARE
  ...
  v_content_pl text; v_content_en text;
  v_builder jsonb; v_blocks jsonb;
...
BEGIN
  ...
  IF NOT v_already THEN
    ...
    IF v_insert_count > 0 THEN v_used := v_used + 1; END IF;
  END IF;
  ...
  RETURN QUERY SELECT
    true, (NOT v_already AND v_insert_count > 0),
    v_used, v_limit, GREATEST(v_limit - v_used, 0), false, v_show_counter,
    v_content_pl, v_content_en, v_builder, v_blocks;
END $$;
...
CREATE OR REPLACE FUNCTION public.has_content_access(
  _entity_type access_entity_type, _entity_id uuid
) RETURNS boolean
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public
AS $$
DECLARE
...
BEGIN
  ...
  IF NOT FOUND OR v_mode = 'public' THEN RETURN true; END IF;
  IF v_uid IS NULL THEN RETURN false; END IF;

  IF v_mode = 'members' THEN
    RETURN EXISTS (
      SELECT 1 FROM public.profiles p WHERE p.id = v_uid AND p.tenant_id = v_tenant
    );
  END IF;

  IF EXISTS (
    SELECT 1 FROM public.user_purchases
     WHERE user_id = v_uid AND entity_type = _entity_type
       AND entity_id = _entity_id AND status = 'active'
  ) THEN RETURN true; END IF;

  IF array_length(v_plans, 1) IS NOT NULL AND EXISTS (
    SELECT 1 FROM public.user_subscriptions
     WHERE user_id = v_uid AND plan_id = ANY (v_plans) AND status = 'active'
       AND (current_period_end IS NULL OR current_period_end > now())
  ) THEN RETURN true; END IF;

  IF public.user_has_tier_feature(v_uid, 'premium_content') THEN RETURN true; END IF;
