-- Wyszukiwarka premium: osoby i organizacje + zaawansowane tryby zapytań (PR #40).
-- Zakres: nowy wymiar taksonomii "organization", nes_search_tsquery_adv,
-- search_posts v4 / search_facets v2 (parametry _match, _in), search_people_orgs.
...
COMMENT ON FUNCTION public.nes_search_tsquery_adv(text, text) IS
  'Zaawansowany parser zapytań: "frazy" (<->), -wykluczenia (& !), tryby all/any/phrase.';
...
COMMENT ON FUNCTION public.search_people_orgs(text, int) IS
  'Sekcja "Osoby i organizacje" wyszukiwarki: autorzy/eksperci z publicznym dorobkiem + organizacje.';
