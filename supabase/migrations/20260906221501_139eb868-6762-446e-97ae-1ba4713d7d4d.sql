...
DROP POLICY IF EXISTS "invitation_recipient_accept_own" ON public.user_invitations;
CREATE POLICY "invitation_recipient_accept_own"
...
USING (
  ...
  AND status IN ('pending'::public.invitation_status, 'sent'::public.invitation_status, 'failed'::public.invitation_status)
)
WITH CHECK (
  auth_user_id = auth.uid()
  AND tenant_id = public._caller_tenant()
