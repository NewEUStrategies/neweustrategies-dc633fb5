...
BEGIN
  ...
  WITH tenant_members AS (
    SELECT p.id, p.created_at, p.discoverable FROM public.profiles p WHERE p.tenant_id = v_tenant
  ),
  activity AS (
    SELECT h.user_id, h.read_at AS ts FROM public.user_read_history h WHERE h.read_at >= v_since
    UNION ALL
    SELECT c.user_id, c.created_at FROM public.comments c WHERE c.tenant_id = v_tenant AND c.created_at >= v_since
    UNION ALL
    SELECT m.sender_id, m.created_at FROM public.messages m WHERE m.tenant_id = v_tenant AND m.created_at >= v_since
    UNION ALL
    ...
    UNION ALL
    SELECT v.user_id, v.viewed_at FROM public.post_views v WHERE v.user_id IS NOT NULL AND v.tenant_id = v_tenant AND v.viewed_at >= v_since
  ),
  member_activity AS (
    SELECT a.user_id, a.ts FROM activity a WHERE a.user_id IN (SELECT tm.id FROM tenant_members tm)
  )
  SELECT
    ...
    (SELECT count(DISTINCT h.user_id) FROM public.user_read_history h WHERE h.read_at >= v_since AND h.user_id IN (SELECT tm.id FROM tenant_members tm)),
    (SELECT count(DISTINCT c.user_id) FROM public.comments c WHERE c.tenant_id = v_tenant AND c.created_at >= v_since),
    (SELECT count(DISTINCT m.sender_id) FROM public.messages m WHERE m.tenant_id = v_tenant AND m.created_at >= v_since),
    (SELECT count(*) FROM public.newsletter_subscribers ns WHERE ns.tenant_id = v_tenant AND ns.status = 'subscribed'),
    (SELECT count(DISTINCT us.user_id) FROM public.user_subscriptions us WHERE us.tenant_id = v_tenant AND us.status = 'active');
END;
$$;
...
RETURNS TABLE (cohort_start date, cohort_size bigint, week_offset integer, active_members bigint)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public
AS $$
DECLARE
  v_tenant uuid := public.assert_admin_tenant();
...
BEGIN
  ...
  WITH cohorts AS (
    SELECT p.id AS user_id, date_trunc('week', p.created_at)::date AS cohort
      FROM public.profiles p WHERE p.tenant_id = v_tenant AND p.created_at >= v_since
  ),
  activity AS (
    SELECT h.user_id, h.read_at AS ts FROM public.user_read_history h WHERE h.read_at >= v_since
    UNION ALL
  ...
  cohort_activity AS (
    SELECT co.cohort, a.user_id,
           floor(extract(epoch FROM (a.ts - co.cohort::timestamptz)) / 604800)::integer AS wk
      FROM activity a JOIN cohorts co ON co.user_id = a.user_id
  ...
  SELECT co.cohort AS cohort_start,
    ...
    FROM cohorts co
    CROSS JOIN LATERAL (
      SELECT generate_series(0, GREATEST(0, floor(extract(epoch FROM (now() - co.cohort::timestamptz)) / 604800)::integer)) AS week_offset
    ) wk
    LEFT JOIN LATERAL (
      SELECT count(DISTINCT ca.user_id) AS active FROM cohort_activity ca WHERE ca.cohort = co.cohort AND ca.wk = wk.week_offset
    ) act ON true
...
BEGIN
  ...
  SELECT d.d AS day,
         (SELECT count(DISTINCT a.user_id) FROM activity a WHERE a.ts::date = d.d AND a.user_id IN (SELECT tm.id FROM tenant_members tm)) AS active_members,
         (SELECT count(*) FROM tenant_members tm WHERE tm.created_at::date = d.d) AS new_members
    FROM days d
   ORDER BY d.d ASC;
END;
$$;
REVOKE ALL ON FUNCTION public.admin_member_activity_series(integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_member_activity_series(integer) TO authenticated, service_role;
