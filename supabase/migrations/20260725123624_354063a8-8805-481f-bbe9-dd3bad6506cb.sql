-- ============================================================================
-- Suppression list + telemetria dostarczalności (bounce / complaint). PR #96
-- Idempotentne.
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS email_suppressions_active_idx
  ON public.email_suppressions (tenant_id, email_norm) WHERE released_at IS NULL;
...
CREATE OR REPLACE FUNCTION public.email_record_suppression(
  p_tenant uuid, p_email text, p_reason text,
  p_source text DEFAULT 'resend_webhook', p_provider text DEFAULT 'resend',
  p_provider_message_id text DEFAULT NULL, p_event_id text DEFAULT NULL,
  p_campaign uuid DEFAULT NULL, p_subscriber uuid DEFAULT NULL,
  p_diagnostic text DEFAULT NULL, p_meta jsonb DEFAULT '{}'::jsonb
...
DECLARE
  ...
  v_scope text; v_expires timestamptz;
  v_occurrences integer := 1; v_escalated boolean := false; v_id uuid;
BEGIN
  ...
  END IF;
  SELECT * INTO v_existing FROM public.email_suppressions
   WHERE tenant_id = p_tenant AND email_norm = v_email FOR UPDATE;
  ...
  IF v_reason = 'soft_bounce' THEN
    IF v_occurrences >= c_soft_limit THEN
      v_reason := 'hard_bounce'; v_scope := 'permanent'; v_expires := NULL; v_escalated := true;
    ELSE
      v_scope := 'transient';
      v_expires := now() + make_interval(days => LEAST(power(2, GREATEST(v_occurrences - 1, 0))::integer, 8));
    END IF;
  ELSE
    v_scope := 'permanent'; v_expires := NULL;
  END IF;
  IF v_has_existing AND v_existing.released_at IS NULL
     AND public.email_suppression_severity(v_existing.reason) > public.email_suppression_severity(v_reason) THEN
    ...
    RETURN jsonb_build_object('ok', true, 'id', v_id, 'reason', v_existing.reason,
  ...
  ON CONFLICT (tenant_id, email_norm) DO UPDATE
     SET reason = EXCLUDED.reason, scope = EXCLUDED.scope, source = EXCLUDED.source,
         ...
         released_at = NULL, released_by = NULL, meta = es.meta || EXCLUDED.meta
  RETURNING id INTO v_id;
  RETURN jsonb_build_object('ok', true, 'id', v_id, 'reason', v_reason, 'scope', v_scope,
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $fn$
  SELECT es.email_norm, es.reason, es.scope, es.expires_at
   ...
   WHERE es.tenant_id = p_tenant AND es.released_at IS NULL
...
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $fn$
  SELECT EXISTS (SELECT 1 FROM public.email_suppressions es
...
CREATE OR REPLACE FUNCTION public.email_apply_delivery_event(
  p_provider text, p_event_id text, p_event_type text, p_kind text, p_email text,
  p_provider_message_id text DEFAULT NULL, p_bounce_class text DEFAULT NULL,
  p_diagnostic text DEFAULT NULL, p_occurred_at timestamptz DEFAULT now(),
  p_tenant_hint uuid DEFAULT NULL, p_campaign_hint uuid DEFAULT NULL,
  p_subscriber_hint uuid DEFAULT NULL, p_payload jsonb DEFAULT '{}'::jsonb
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $fn$
DECLARE
  v_email text := lower(btrim(COALESCE(p_email, '')));
  v_provider text := COALESCE(NULLIF(btrim(p_provider), ''), 'resend');
  v_tenant uuid := p_tenant_hint; v_campaign uuid := p_campaign_hint;
  v_subscriber uuid := p_subscriber_hint;
  v_rec record; v_match_count integer := 0; v_event_id uuid;
  v_reason text; v_suppression jsonb := NULL; v_state text;
BEGIN
  ...
  IF p_provider_message_id IS NOT NULL AND btrim(p_provider_message_id) <> '' THEN
    SELECT r.tenant_id, r.campaign_id, r.subscriber_id, r.id INTO v_rec
      FROM public.newsletter_campaign_recipients r
     WHERE r.provider_message_id = p_provider_message_id LIMIT 1;
    IF FOUND THEN
  ...
  IF v_tenant IS NULL AND v_email <> '' THEN
    SELECT count(*) INTO v_match_count FROM public.newsletter_subscribers ns
     WHERE lower(ns.email) = v_email;
    IF v_match_count = 1 THEN
      SELECT ns.tenant_id, ns.id INTO v_tenant, v_subscriber
        FROM public.newsletter_subscribers ns WHERE lower(ns.email) = v_email;
    END IF;
  END IF;
  ...
  END IF;
  v_state := CASE p_kind
    WHEN 'delivered' THEN 'delivered' WHEN 'bounced' THEN 'bounced'
    WHEN 'complained' THEN 'complained' WHEN 'delayed' THEN 'delayed'
    WHEN 'failed' THEN 'failed' ELSE NULL
  END;
  IF v_state IS NOT NULL AND p_provider_message_id IS NOT NULL THEN
    UPDATE public.newsletter_campaign_recipients r
     ...
     WHERE r.provider_message_id = p_provider_message_id
       AND (v_state <> 'delivered' OR r.delivery_state NOT IN ('bounced','complained','failed'));
  END IF;
  v_reason := CASE
  ...
  IF v_reason IS NOT NULL AND v_tenant IS NOT NULL AND v_email <> '' THEN
    v_suppression := public.email_record_suppression(
      p_tenant => v_tenant, p_email => v_email, p_reason => v_reason,
      p_source => 'resend_webhook', p_provider => v_provider,
      p_provider_message_id => p_provider_message_id, p_event_id => p_event_id,
      p_campaign => v_campaign, p_subscriber => v_subscriber,
      p_diagnostic => p_diagnostic,
      p_meta => jsonb_build_object('event_type', p_event_type));
  END IF;
  RETURN jsonb_build_object('ok', true, 'duplicate', false,
    'tenant_id', v_tenant, 'campaign_id', v_campaign, 'subscriber_id', v_subscriber,
    'kind', p_kind, 'suppression', v_suppression);
END;
$fn$;
...
CREATE OR REPLACE FUNCTION public.email_suppression_add(
  p_email text, p_reason text DEFAULT 'manual', p_note text DEFAULT NULL
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $fn$
DECLARE
  v_tenant uuid; v_email text := lower(btrim(COALESCE(p_email, ''))); v_id uuid;
BEGIN
  ...
  ON CONFLICT (tenant_id, email_norm) DO UPDATE
     SET reason = CASE
         ...
         scope = 'permanent', expires_at = NULL, released_at = NULL, released_by = NULL,
         note = COALESCE(NULLIF(EXCLUDED.note, ''), es.note), last_seen_at = now()
  RETURNING id INTO v_id;
...
CREATE OR REPLACE FUNCTION public.email_suppression_release(
  p_id uuid, p_resubscribe boolean DEFAULT false
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $fn$
DECLARE v_tenant uuid; v_row public.email_suppressions%ROWTYPE;
BEGIN
  IF NOT public.is_staff() THEN RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501'; END IF;
  v_tenant := public.current_tenant_id();
  ...
  END IF;
  UPDATE public.email_suppressions
     SET released_at = now(), released_by = auth.uid(), expires_at = now()
   WHERE id = p_id;
...
CREATE OR REPLACE FUNCTION public.email_deliverability_counts(
  p_tenant uuid, p_days integer DEFAULT 30
) RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public AS $fn$
DECLARE
  v_days integer := LEAST(GREATEST(COALESCE(p_days, 30), 1), 365);
  v_since timestamptz;
  v_sent_log bigint := 0; v_suppressed_log bigint := 0;
  v_sent bigint := 0; v_delivered bigint := 0; v_bounced bigint := 0;
  v_hard bigint := 0; v_soft bigint := 0; v_complained bigint := 0;
  v_failed bigint := 0; v_delayed bigint := 0; v_active bigint := 0;
BEGIN
  IF p_tenant IS NULL THEN RETURN jsonb_build_object('ok', false, 'error', 'no_tenant'); END IF;
  ...
  SELECT
    ...
    count(*) FILTER (WHERE kind = 'bounced' AND COALESCE(bounce_class, 'unknown') NOT IN ('hard','block')),
  ...
  FROM public.email_delivery_events WHERE tenant_id = p_tenant AND occurred_at >= v_since;
  SELECT
  ...
  SELECT count(*) INTO v_active FROM public.email_suppressions
   WHERE tenant_id = p_tenant AND released_at IS NULL
     AND (expires_at IS NULL OR expires_at > now());
  RETURN jsonb_build_object('ok', true, 'days', v_days,
    'sent', v_sent, 'delivered', v_delivered, 'bounced', v_bounced,
    'hard_bounced', v_hard, 'soft_bounced', v_soft, 'complained', v_complained,
    'failed', v_failed, 'delayed', v_delayed,
    'suppressed_sends', v_suppressed_log, 'active_suppressions', v_active);
END;
$fn$;
...
DECLARE
  v_tenant uuid;
  v_days integer := LEAST(GREATEST(COALESCE(p_days, 30), 1), 365);
  v_since timestamptz; v_counts jsonb; v_series jsonb; v_reasons jsonb; v_campaigns jsonb;
BEGIN
  IF NOT public.is_staff() THEN RAISE EXCEPTION 'forbidden' USING ERRCODE = '42501'; END IF;
  v_tenant := public.current_tenant_id();
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    INTO v_reasons
    FROM (SELECT reason, scope, count(*) AS c FROM public.email_suppressions
       WHERE tenant_id = v_tenant AND released_at IS NULL
         AND (expires_at IS NULL OR expires_at > now())
       GROUP BY reason, scope) g;
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'day', to_char(d.day, 'YYYY-MM-DD'),
           'sent', COALESCE(e.sent, 0), 'delivered', COALESCE(e.delivered, 0),
           'bounced', COALESCE(e.bounced, 0), 'complained', COALESCE(e.complained, 0)) ORDER BY d.day), '[]'::jsonb)
    INTO v_series
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           'id', c.id, 'name', c.name, 'finished_at', c.finished_at,
           'sent', COALESCE(r.sent, 0), 'delivered', COALESCE(r.delivered, 0),
           'bounced', COALESCE(r.bounced, 0), 'complained', COALESCE(r.complained, 0),
           'suppressed', COALESCE(r.suppressed, 0)) ORDER BY c.created_at DESC), '[]'::jsonb)
    INTO v_campaigns
    FROM (SELECT id, name, finished_at, created_at FROM public.newsletter_campaigns
       WHERE tenant_id = v_tenant AND status IN ('sent','sending','failed')
       ORDER BY created_at DESC LIMIT 10) c
    LEFT JOIN (
      SELECT campaign_id,
        ...
        FROM public.newsletter_campaign_recipients
       WHERE tenant_id = v_tenant GROUP BY campaign_id
    ) r ON r.campaign_id = c.id;
  RETURN v_counts || jsonb_build_object(
    'suppression_reasons', v_reasons, 'series', v_series,
    'campaigns', v_campaigns, 'generated_at', now());
END;
$fn$;
...
SELECT s.tenant_id, s.email_norm, 'invalid', 'permanent', 'system', 'resend',
  ...
  FROM (
    SELECT r.tenant_id, lower(r.email) AS email_norm,
