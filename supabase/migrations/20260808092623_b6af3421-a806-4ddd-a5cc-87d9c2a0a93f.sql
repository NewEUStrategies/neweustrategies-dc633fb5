-- 1. Kolumna obszaru tematycznego na wątkach
ALTER TABLE public.club_threads ADD COLUMN IF NOT EXISTS topic text;
CREATE INDEX IF NOT EXISTS club_threads_topic_idx
  ON public.club_threads (club_id, topic)
  WHERE topic IS NOT NULL;

-- 2. Słownik obszarów tematycznych (jedno miejsce prawdy dla walidacji)
...
AS $$
  SELECT _topic IS NULL OR _topic IN (
    'geopolitics','transport','energy','cybersecurity','technology',
    'finance','economy','diplomacy','international_relations','culture'
  )
$$;
GRANT EXECUTE ON FUNCTION public.club_topic_valid(text) TO anon, authenticated, service_role;

-- 3. Zakładanie wątku przez członka: obszar tematyczny + zamknięcie odpowiedzi
...
CREATE FUNCTION public.club_create_thread(
  ...
  p_anchor_type text DEFAULT NULL::text,
  p_anchor_id text DEFAULT NULL::text,
...
BEGIN
  ...
  INSERT INTO public.club_threads (
    ...
    is_anonymous, anchor_type, anchor_id, topic, locked_at
  ) VALUES (
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.club_create_thread(uuid,text,text,text,boolean,text,text,text,boolean,text) TO authenticated, service_role;

-- 4. Zakładanie wątku z panelu administracyjnego
DROP FUNCTION IF EXISTS public.admin_club_thread_create(uuid,text,text,uuid,text,boolean);
CREATE FUNCTION public.admin_club_thread_create(
  p_group_id uuid,
  p_title text,
  p_body text,
...
-- 5. Widok wątku zwraca obszar tematyczny
...
-- 6. Lista wątków: zwraca obszar tematyczny i pozwala po nim filtrować
