...
--
-- domain_events to pierwszoklasowa warstwa integracyjna (nie tylko log):
--   * każdy moduł (content, komentarze, czat, CRM, newsletter) emituje
--     zdarzenia `<agregat>.<czasownik>.v1` triggerami AFTER na swojej tabeli;
--   * konsumenci (frontend przez Realtime, workflowy - część 4/5, router
--     integracji - część 5/5) słuchają JEDNEJ tabeli zamiast N tabel źródłowych;
--   * correlation_id spina "co się wydarzyło po moim kliknięciu": frontend
--     wysyła nagłówek x-correlation-id, PostgREST wystawia go w GUC
--     request.headers, a emitery zapisują go przy zdarzeniu. RPC
--     get_correlated_events zwraca pełny ślad (explainability + debug).
--
-- Bezpieczeństwo:
--   * INSERT wyłącznie przez SECURITY DEFINER (emit_domain_event) - klient nie
--     może sfałszować zdarzenia (workflowy i integracje ufają tej tabeli);
--   * SELECT: staff czyta zdarzenia swojego tenanta; zwykły użytkownik widzi
--     tylko zdarzenia, których sam jest aktorem (to wystarcza do potwierdzeń
--     optymistycznych mutacji po correlation_id).
--
...
-- Realtime: konsumenci frontendu subskrybują postgres_changes na tej tabeli
-- (RLS filtruje strumień per użytkownik).
DO $$ BEGIN
  BEGIN
...
-- ----------------------------------------------------------------------------
-- correlation_id z nagłówka żądania. PostgREST wystawia nagłówki w GUC
-- request.headers (json). Nagłówek jest kontrolowany przez klienta, więc
-- traktujemy go wyłącznie jako NIEZAUFANE metadane śledzenia (nigdy autoryzacja).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Jedyny producent zdarzeń. SECURITY DEFINER + REVOKE od klientów: wołają go
-- wyłącznie triggery SECURITY DEFINER i service_role. Nigdy nie wyrzuca
-- wyjątku - awaria szyny nie może wywrócić zapisu źródłowego.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Pełny ślad korelacji: wszystkie zdarzenia z danym correlation_id, w kolejności
-- powstania. SECURITY INVOKER - RLS tabeli decyduje, co wolno zobaczyć.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- EMITERY per moduł. Każdy: AFTER trigger, SECURITY DEFINER, nigdy nie blokuje
-- zapisu źródłowego (emit_domain_event połyka błędy).
-- ----------------------------------------------------------------------------

-- CONTENT: posty - utworzenie, zmiana statusu (w tym publikacja), miękkie
-- usunięcie. Celowo NIE emitujemy każdego autosave'a (spam na szynie).
...
-- KOMENTARZE: utworzenie + zmiana statusu moderacji (status-agnostycznie,
-- stary/nowy w payloadzie - workflowy filtrują warunkiem).
...
-- CZAT: wysłanie wiadomości. Payload bez treści (prywatność) - identyfikatory
-- wystarczają do potwierdzeń optymistycznych i liczników.
...
-- CRM: nowy lead, zmiana etapu (won/lost napędzają workflowy), inne zmiany.
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    ELSE
      -- Ruch CRM jest niskowolumenowy (w przeciwieństwie do autosave'ów
      -- postów), więc zwykłe aktualizacje też emitujemy - panel admina
      -- odświeża się np. po dopisaniu źródła przez formularz kontaktowy.
      PERFORM public.emit_domain_event(
...
-- NOTATKI CRM: powstanie notatki (zasila graf powiązań i wzmianki - część 2/5).
...
-- NEWSLETTER: zapis, potwierdzenie (double opt-in), wypis. Payload z danymi
-- kontaktu, bo workflow "subskrybent -> lead CRM" ich potrzebuje.
...
-- ----------------------------------------------------------------------------
-- Retencja: szyna nie jest archiwum. 90 dni wystarcza na debug/explainability;
-- integracje i workflowy konsumują na bieżąco.
-- ----------------------------------------------------------------------------
