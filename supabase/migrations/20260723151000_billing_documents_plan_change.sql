-- ============================================================================
-- Dokumenty rozliczeniowe w naszym systemie + zmiana planu na szynie zdarzeń.
--
-- 1) billing_documents - rejestr dokumentów rozliczeniowych klienta (faktury
--    Stripe z pierwszego zakupu I KAŻDEGO odnowienia, paragony płatności
--    jednorazowych, korekty po refundzie). Dotąd jedynym śladem był
--    payment_orders.invoice_url z checkoutu - odnowienia subskrypcji nie
--    zostawiały żadnego dokumentu widocznego dla użytkownika. Zasila go
--    webhook Stripe (service_role); użytkownik widzi własne dokumenty
--    (/profile/orders), staff dokumenty swojego tenanta. Przechowujemy
--    metadane + trwałe linki Stripe (podgląd online i PDF) - treść dokumentu
--    pozostaje u operatora płatności, rejestr i dostęp są nasze.
-- 2) Emitery billing_document.issued/updated.v1 - profil odświeża listę na
--    żywo (aktor = właściciel), a workflowy mogą reagować na wystawienie
--    dokumentu (np. "faktura -> notyfikacja").
-- 3) tg_user_subscriptions_emit_events uczy się ZMIANY PLANU (upgrade/
--    downgrade przez changeSubscriptionPlan): subscription.updated.v1 niesie
--    old_plan_id/new_plan_id i tier_key nowego planu, więc warstwa, paywall
--    i pulpity odświeżają się dokładnie tak samo jak przy anulowaniu.
--
...
CREATE TABLE IF NOT EXISTS public.billing_documents (
  ...
  -- Celowo bez FK do auth.users: dokumenty księgowe muszą przetrwać
  -- usunięcie konta (obowiązek retencji dokumentacji sprzedaży).
...
-- ----------------------------------------------------------------------------
-- 2) Emitery dokumentów na szynie (aktor = właściciel dokumentu).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 3) Zmiana planu na szynie: subscription.updated.v1 emitowane też przy
--    zmianie plan_id (payload: old/new plan + tier_key nowego planu).
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    ELSIF NEW.plan_id IS DISTINCT FROM OLD.plan_id THEN
      -- Upgrade/downgrade planu (changeSubscriptionPlan): warstwa użytkownika
      -- mogła się zmienić - konsumenci odświeżają dokładnie te same klucze.
      PERFORM public.emit_domain_event(
