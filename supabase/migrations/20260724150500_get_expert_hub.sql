-- Hub eksperta jednym round-tripem: caly ladunek strony /author/$slug
-- (profil + nakladka autora + odznaki + programy + obszary + wzmianki +
-- materialy z pivotami + ZAWEZONE taksonomie faset + layout tenanta profilu)
-- w jednym jsonb. Wczesniej ta sama strona kosztowala ~22 round-tripy HTTP
-- do PostgREST w 4-5 sekwencyjnych falach - najciezsza publiczna trasa.
--
-- Kontrakty:
--   * SECURITY INVOKER + STABLE: funkcja czyta wylacznie przez RLS wolajacego
--     (anon widzi dokladnie to, co przy zapytaniach bezposrednich; tenant
--     scoping przez public_tenant_id() dziala bez zmian).
--   * Kolumny i filtry odwzorowuja 1:1 selecty z src/lib/experts/queries.ts -
--     mapowanie wierszy zostaje w TS (normalize.ts), obie sciezki dziela
--     jedna asemblacje.
--   * Taksonomie faset sa zawezone w SQL do wartosci wystepujacych w
--     materialach eksperta - koniec z pobieraniem 4 pelnych tabel katalogu.
--   * Brak profilu => jsonb z profile=null (TS mapuje na 404), nigdy wyjatek.
...
DECLARE
  ...
  v_expert uuid;
  v_tenant uuid;
  v_author_profile jsonb;
  v_badges jsonb;
  v_program_members jsonb;
  v_expertise_areas jsonb;
  v_media_mentions jsonb;
  v_primary_posts jsonb;
  v_coauthor_posts jsonb;
  v_podcasts jsonb;
  v_host_events jsonb;
  v_speaker_events jsonb;
  v_post_ids uuid[];
  v_post_categories jsonb;
  v_post_programs jsonb;
  v_post_regions jsonb;
  v_post_tags jsonb;
  v_program_ids uuid[];
  v_region_ids uuid[];
  v_layout jsonb;
BEGIN
  -- Rezolucja profilu: slug, a dla UUID fallback po id (jak w TS).
               ...
               SELECT to_jsonb(pr)
                 FROM (
                   SELECT p2.id, p2.slug, p2.name_pl, p2.name_en, p2.kind,
                     ...
                     WHERE p2.id = pm.program_id
                 ) pr
             )
           )
           ORDER BY pm.sort_order ASC
         ), '[]'::jsonb) INTO v_program_members
    FROM public.program_members pm
    WHERE pm.user_id = v_expert;
  ...
  SELECT COALESCE(jsonb_agg(to_jsonb(he)), '[]'::jsonb) INTO v_host_events
    FROM (
      SELECT id, slug, title_pl, title_en, description_pl, description_en, cover_url,
             starts_at, program_id, region_id, host_user_id
        FROM public.events
        WHERE host_user_id = v_expert AND status = 'published'
    ) he;

  SELECT COALESCE(jsonb_agg(to_jsonb(se)), '[]'::jsonb) INTO v_speaker_events
    FROM (
  ...
  -- Pelny zbior postow eksperta (glowny autor + wspolautorstwa) dla pivotow.
  ...
  SELECT COALESCE(jsonb_agg(to_jsonb(pc)), '[]'::jsonb) INTO v_post_categories
    FROM (
      SELECT post_id, category_id FROM public.post_categories
        WHERE post_id = ANY (v_post_ids)
    ) pc;
  SELECT COALESCE(jsonb_agg(to_jsonb(pg)), '[]'::jsonb) INTO v_post_programs
    FROM (
      SELECT post_id, program_id FROM public.post_programs
        WHERE post_id = ANY (v_post_ids)
    ) pg;
  SELECT COALESCE(jsonb_agg(to_jsonb(pr)), '[]'::jsonb) INTO v_post_regions
    FROM (
      SELECT post_id, region_id FROM public.post_regions
        WHERE post_id = ANY (v_post_ids)
    ) pr;
  SELECT COALESCE(jsonb_agg(to_jsonb(pt)), '[]'::jsonb) INTO v_post_tags
    FROM (
      SELECT post_id, tag_id FROM public.post_tags
        WHERE post_id = ANY (v_post_ids)
    ) pt;

  -- Taksonomie faset zawezone do wartosci obecnych w materialach: pivoty
  -- postow + program/region podcastow i wydarzen eksperta.
  SELECT array_agg(DISTINCT x.program_id) INTO v_program_ids
    FROM (
      SELECT pp2.program_id
        FROM public.post_programs pp2
        WHERE pp2.post_id = ANY (v_post_ids)
      UNION
      SELECT pod2.program_id
        FROM public.podcasts pod2
        WHERE pod2.author_id = v_expert AND pod2.status = 'published'
          AND pod2.deleted_at IS NULL AND pod2.program_id IS NOT NULL
      UNION
      SELECT e2.program_id
        FROM public.events e2
        LEFT JOIN public.event_speakers es2
          ON es2.event_id = e2.id AND es2.user_id = v_expert
        WHERE e2.status = 'published' AND e2.program_id IS NOT NULL
          AND (e2.host_user_id = v_expert OR es2.user_id IS NOT NULL)
    ) x
    WHERE x.program_id IS NOT NULL;
  v_program_ids := COALESCE(v_program_ids, ARRAY[]::uuid[]);

  SELECT array_agg(DISTINCT x.region_id) INTO v_region_ids
    FROM (
      SELECT pr2.region_id
        FROM public.post_regions pr2
        WHERE pr2.post_id = ANY (v_post_ids)
      UNION
      SELECT pod3.region_id
        FROM public.podcasts pod3
      ...
      SELECT e3.region_id
        ...
        LEFT JOIN public.event_speakers es3
          ON es3.event_id = e3.id AND es3.user_id = v_expert
  ...
  SELECT to_jsonb(l) INTO v_layout
    FROM (
      SELECT * FROM public.expert_layout_settings
          ...
          SELECT id, slug, name_pl, name_en
            ...
            WHERE id IN (
              SELECT DISTINCT pc2.category_id
                ...
                WHERE pc2.post_id = ANY (v_post_ids)
            )
        ) tc
    ),
    'tags', (
          ...
          SELECT id, slug, name
            ...
            WHERE id IN (
              SELECT DISTINCT pt2.tag_id
                ...
                WHERE pt2.post_id = ANY (v_post_ids)
            )
        ) tt
    ),
