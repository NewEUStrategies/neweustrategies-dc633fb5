...
const STR_MAP: Record<string, string> = {
  ...
  "https://www.youtube.com/watch?v=…  •  https://vimeo.com/…  •  https://x.com/…": 'i18n.editor("embed","urlPh")',
  ...
  "Wstępnie sformatowany tekst (zachowuje spacje i nowe linie)…": 'i18n.editor("preformatted","textPh")',
};
...
for (const file of files) {
  ...
  for (const [needle, replacement] of Object.entries(TEXT_MAP)) {
    ...
    src = src.replace(reText, () => { touched = true; return `>${replacement}<`; });
  }

  if (!touched) { skipped.push(file); continue; }
  ...
  if (!src.includes('from "@/lib/blocks/i18n"')) {
    ...
    if (importBlockMatch) {
      src = src.slice(0, importBlockMatch[0].length) + importLine + src.slice(importBlockMatch[0].length);
    } else {
      src = importLine + src;
    }
  }
  ...
  src = src.replace(/export function (\w+)\(([^)]*)\)\s*(?::\s*[^{]+)?\{/g, (match, _name, _args) => {
  ...
  src = src.replace(/(\n\s*const i18n = useBlocksI18n\(\);)(\s*\n\s*const i18n = useBlocksI18n\(\);)+/g, "$1");
