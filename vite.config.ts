...
export default defineConfig({
  ...
  plugins: [
    // Dev-only: wyłącz HMR na trasie głównej (`src/routes/__root.tsx`). Gdy
    // Vite próbuje partial-HMR-em podmienić root layout w trakcie strumienia
    // SSR, TanStack anuluje zapytania w locie (`CancelledError` na
    // `menu-with-items:main`), a przeglądarka zostaje z połowicznym HTML-em
    // i zawiesza się do twardego przeładowania. Zamiast partial HMR wymuszamy
    // pełny reload przez WS - strumień domyka się czysto, klient startuje od
    // nowa. Nie dotyczy buildu produkcyjnego.
    {
