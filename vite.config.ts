...
import { bundleStatsPlugin } from "./scripts/lib/bundleStatsPlugin";
...
export default defineConfig({
  ...
  vite: {
    // Zrzut składu chunków klienta - AKTYWNY WYŁĄCZNIE przy `BUNDLE_STATS=1`
    // (bez zmiennej `bundleStatsPlugin()` zwraca `false`, więc artefakt
    // produkcyjny jest bit-w-bit identyczny). Czytnik: `bun run analyze:bundle`.
